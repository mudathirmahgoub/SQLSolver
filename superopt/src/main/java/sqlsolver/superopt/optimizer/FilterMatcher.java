...
class FilterMatcher
{
  ...
  {
    if (opChain.size() > nodeChain.size())
      return emptyList();
    if (isSimpleFullCover())
      return fullCover(baseMatch); // fast path

    final List<SubMatcher> matchers = mkSubMatchers(baseMatch);
    if (!isSubMatchersValid(matchers))
      return emptyList();
  ...
  {
    return !isTrailing && !isLeading && opChain.size() == 1
  ...
  private List<SubMatcher> mkSubMatchers(Match baseMatch)
  {
    ...
    {
      if (!done[i])
        matchers.add(mkSubMatcherFor(i, done, model));
    }
  ...
  private SubMatcher mkSubMatcherFor(int opIdx, boolean[] done, Model model)
  {
    ...
    for (int i = 0, bound = opChain.size(); i < bound; i++)
    {
      ...
      if (!(otherOp instanceof AttrsFilter))
        continue;
    ...
    if (model.isAssigned(attrsSym))
      return new GroupedFilterSubMatcher(buddies.toArray(), 0);
    if (isFreeAttr && !op.kind().isSubquery())
      return new FreeFilterSubMatcher(opIdx, !isTrailing && !isLeading);
    else
      return new GroupedFilterSubMatcher(buddies.toArray(), op.kind().isSubquery() ? 2 : 1);
  }
  ...
  private class GroupedFilterSubMatcher extends SubMatcher
  {
    ...
    {
      if (opOrdinal >= indices.length)
        return next.match(whatIf);
      if (nextNodeIdx >= nodeChain.size())
        return false;
  ...
  private class FreeFilterSubMatcher extends SubMatcher
  {
    ...
    private boolean greedyMatch(Match match)
    {
      ...
      for (int i = 0, bound = nodeChain.size(); i < bound; ++i)
        if (!assignments.isUsed(i))
          toMergeFilters.add(i);

      if (toMergeFilters.isEmpty())
        return false;
    ...
    private boolean nonGreedyMatch(Match match)
    {
      ...
      for (int i = 0, bound = nodeChain.size(); i < bound; ++i)
      {
        if (assignments.isUsed(i) || checked[i])
          continue;
  ...
  private class Terminator extends SubMatcher
  {
    ...
    boolean match(Match whatIf)
    {
      ...
      if (isLeading)
        for (Match m : matches) m.setMatchRootNode(newChain.at(0));
  ...
  private List<Match> tryMatch(Op op, int nodeId, Match match)
  {
    if (!match.matchOne(op, nodeId))
      return emptyList();
    if (!op.kind().isSubquery())
      return singletonList(match);
    else
      return Match.match(match, op.predecessors()[1], match.sourcePlan().childOf(nodeId, 1));
  }
}
