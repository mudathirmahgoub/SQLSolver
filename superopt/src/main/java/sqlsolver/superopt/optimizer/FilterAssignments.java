...
class FilterAssignments
{
  ...
  FilterChain mkChain(boolean appendUnused)
  {
    ...
    if (!appendUnused)
      addUnused(filterNodes);

    for (final int[] assignment : assignments)
    {
      assert assignment != null && assignment.length > 0;
      if (assignment.length == 1)
        filterNodes.add(filterChain.at(assignment[0]));
      else
        filterNodes.add(mkMerged(plan, assignment));
    }
  ...
  {
    for (int i = 0, bound = used.length; i < bound; i++)
      if (!used[i])
        dest.add(filterChain.at(i));
  }
  ...
  private int mkMerged(PlanContext plan, int... indices)
  {
    ...
    for (Expression expr : exprs)
    {
      ...
      if (mergedAst == null)
        ...
        mergedAst = mkBinary(tempCtx, AND, mergedAst, copiedAst);
  ...
  private static Expression exprOf(PlanContext plan, int nodeId)
  {
    ...
    if (filter.kind() == Filter)
      return ((SimpleFilterNode) filter).predicate();
    else
