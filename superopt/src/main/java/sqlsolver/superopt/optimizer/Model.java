...
class Model
{
  ...
  void reset()
  {
    if (assignments.isInitialized())
      assignments.get().clear();
  }
  ...
  private boolean checkConstraint(Constraint constraint)
  {
    ...
    {
      case AttrsSub: return checkAttrsSub(constraint);
      case Unique:
        return checkUnique(constraint);
        //      case NotNull:
        //        return checkNotNull(constraint);
      case Reference: return checkReference(constraint);
      default: return true;
    }
  }
  ...
  private boolean checkUnique(Constraint unique)
  {
    ...
    if (attrs == null)
      return true; // not assigned yet, pass

    final List<Column> columns = tryResolveColumns(plan, attrs);
    if (columns == null)
      return false; // some attrs has no backed column
  ...
  private boolean checkNotNull(Constraint notNull)
  {
    ...
    if (none(columns, column -> isParticipateIn(column, NOT_NULL)))
      return false;
  ...
  private boolean checkReference(Constraint reference)
  {
    ...
    if (!isRootRef(plan, rootRef))
      rootRef = deRef(plan, rootRef);
    assert rootRef != null;
    ...
    while (path != surface)
    {
      ...
      if (plan.kindOf(parent).isFilter())
        return false;
      if (plan.kindOf(parent) == PlanKind.Agg
          && ((AggNode) plan.nodeAt(parent)).havingExpr() != null)
        return false;
      path = parent;
    }
