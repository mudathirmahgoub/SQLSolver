...
class ReduceSort
{
  ...
  int reduce(int planRoot)
  {
    ...
    if (!sortNodes.isInitialized())
      return planRoot;

    final TIntSet toRemove = sortNodes.get();
    if (enforcers.isInitialized())
      toRemove.removeAll(enforcers.get());
    if (toRemove.isEmpty())
      return planRoot;
  ...
  private SortSpec resolveSortChain(int node)
  {
    ...
    if (kind == Join || kind == SetOp)
    {
      ...
      if (preserve0 && preserve1)
        basedOn = new SortSpec[] {sort0, sort1};
      else if (preserve0)
        basedOn = new SortSpec[] {sort0};
      else if (preserve1)
        basedOn = new SortSpec[] {sort1};
      else
        return null;
    ...
    if (kind == Agg)
      if (sort0 == null)
        return null;
      else if (isCountAgg(node))
        return new SortSpec(NO_SUCH_NODE, false, sort0.basedOn);
      else
        return sort0;
    ...
    if (kind == Sort)
    {
      ...
      if (sort0 == null)
        return new SortSpec(node, false, null);
      else if (sort0.limited)
        return new SortSpec(node, false, new SortSpec[] {sort0});
      else
        return new SortSpec(node, false, sort0.basedOn);
    }
  ...
  private void gatherSortEnforcer(SortSpec sort)
  {
    if (sort == null)
      return;
    if (sort.enforcer != NO_SUCH_NODE)
      addEnforcer(sort.enforcer);
    if (sort.basedOn != null)
      for (SortSpec base : sort.basedOn) gatherSortEnforcer(base);
  }
