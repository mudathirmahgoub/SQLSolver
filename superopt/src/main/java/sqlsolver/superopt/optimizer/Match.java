...
class Match
{
  ...
  Match setLastMatchPoint(int lastMatchedNode, Op lastMatchOp)
  {
    ...
    if (matchStartNode == NO_SUCH_NODE)
      matchStartNode = lastMatchedNode;
    return this;
  }
  ...
  boolean assembleModifiedPlan()
  {
    ...
    if (!reBinder.rebindToRoot(modifiedPoint))
      return false;

    final int parent1 = sourcePlan.parentOf(matchRootNode);
    final int newRootId;
    if (parent1 == NO_SUCH_NODE)
      if (matchRootNode == matchStartNode)
        newRootId = modifiedPoint;
      else
        newRootId = matchRootNode;
    else
  ...
  boolean matchOne(Op op, int nodeId)
  {
    ...
    switch (op.kind())
    {
      case INPUT: result = matchInput((Input) op, nodeId); break;
      case INNER_JOIN:
      case LEFT_JOIN: result = matchJoin((Join) op, nodeId); break;
      case SIMPLE_FILTER: result = matchFilter((SimpleFilter) op, nodeId); break;
      case IN_SUB_FILTER: result = matchInSub((InSubFilter) op, nodeId); break;
      case PROJ: result = matchProj((Proj) op, nodeId); break;
      case UNION: result = matchSetOp((Union) op, nodeId); break;
      case AGG: result = matchAgg((Agg) op, nodeId); break;
      default: throw new IllegalArgumentException("unknown operator: " + op.kind());
    }

    if (result)
      setLastMatchPoint(nodeId, op);
    return result;
  }
  ...
  {
    if (sourcePlan.kindOf(nodeId) != PlanKind.Join)
      return false;
    ...
    if (opKind == OpKind.LEFT_JOIN && joinKind != JoinKind.LEFT_JOIN)
      return false;
    if (opKind == OpKind.INNER_JOIN && joinKind != JoinKind.INNER_JOIN)
      return false;
    if (!infoCache.isEquiJoin(nodeId))
      return false;

    final var keys = infoCache.getJoinKeyOf(nodeId);
    return model.assign(joinOp.lhsAttrs(), keys.getLeft())
        && model.assign(joinOp.rhsAttrs(), keys.getRight()) && model.checkConstraints();
  }
  ...
  private boolean matchFilter(SimpleFilter filter, int nodeId)
  {
    ...
    if (nodeKind.isSubqueryFilter())
      predicate = sourcePlan.infoCache().getSubqueryExprOf(nodeId);
    else
      predicate = ((SimpleFilterNode) sourcePlan.nodeAt(nodeId)).predicate();

    final Values attrs = sourcePlan.valuesReg().valueRefsOf(predicate);

    return model.assign(filter.predicate(), predicate) && model.assign(filter.attrs(), attrs)
        && model.checkConstraints();
  }

  private boolean matchInSub(InSubFilter inSub, int nodeId)
  {
    if (sourcePlan.kindOf(nodeId) != PlanKind.InSub)
      return false;

    final InSubNode inSubNode = (InSubNode) sourcePlan.nodeAt(nodeId);
    if (!inSubNode.isPlain())
      return false;
  ...
  {
    if (sourcePlan.kindOf(nodeId) != PlanKind.Proj)
      return false;

    final ProjNode projNode = (ProjNode) sourcePlan.nodeAt(nodeId);
    if (proj.deduplicated() ^ PlanSupport.isDedup(sourcePlan, nodeId))
      return false;
    ...
    return model.assign(proj.attrs(), inValues) && model.assign(proj.schema(), outValues)
        && model.checkConstraints();
  }

  private boolean matchSetOp(Union union, int nodeId)
  {
    if (sourcePlan.kindOf(nodeId) != PlanKind.SetOp)
      return false;
  ...
  {
    if (sourcePlan.kindOf(nodeId) != PlanKind.Agg)
      return false;
    ...
    if (!isClosedAggregates(aggNode, aggRefs, groupRefs))
      return false;
    final List<Value> schema = valuesReg.valuesOf(nodeId);

    return model.assign(agg.schema(), schema) && agg.deduplicated() == aggNode.deduplicated()
        && model.assign(agg.aggFunc(), aggFuncs) && model.assign(agg.aggregateAttrs(), aggRefs)
  ...
  static List<Match> match(Match match, Op op, int nodeId)
  {
    ...
    if (op.kind().isJoin())
    {
      if (plan.kindOf(nodeId) != PlanKind.Join)
        return emptyList();
      ...
      if (shouldPermuteJoinTree() || matches.size() <= 1)
        return matches;
      else
        return new ArrayList<>(matches.subList(0, 1));
    }
    ...
    if (op.kind().isFilter())
    {
      ...
      if (!plan.kindOf(nodeId).isFilter())
        return emptyList();
