...
class BottomUpOptimizer implements Optimizer
{
  ...
  public Set<PlanContext> optimize(PlanContext plan)
  {
    ...
    setKeepOriginal(
        (OptimizerSupport.optimizerTweaks & OptimizerSupport.TWEAK_KEEP_ORIGINAL_PLAN) != 0);
  ...
  private Set<SubPlan> optimize0(SubPlan subPlan)
  {
    ...
    if (isTimedOut() || isFullyOptimized(subPlan))
      return memo.eqClassOf(subPlan);
    else
      return dispatch(subPlan);
  }
  ...
  private Set<SubPlan> optimizeChild0(SubPlan n)
  {
    ...
    if (numChildren >= 1)
      lhsOpts = optimize0(n.child(0));
    if (numChildren >= 2)
      rhsOpts = optimize0(n.child(1));
  ...
  private Set<SubPlan> transform(SubPlan subPlan)
  {
    ...
    if (kind.isFilter() && plan.kindOf(plan.parentOf(root)).isFilter())
      return emptySet();
    ...
    for (Substitution rule : rules)
    {
      ...
      for (Match match : fullMatches)
      {
        if (match.assembleModifiedPlan())
        {
          ...
          if (normalizedRoot == NO_SUCH_NODE)
            continue;
  ...
  protected Set<SubPlan> onFilter(SubPlan filter)
  {
    ...
    if (plan.kindOf(plan.parentOf(filter.nodeId())).isFilter())
      return optimizeChild0(filter);
    else
      return optimizeFull(filter);
  }
  ...
  protected Set<SubPlan> onJoin(SubPlan join)
  {
    ...
    if (plan.kindOf(plan.parentOf(join.nodeId())) == PlanKind.Join)
      return optimizeChild0(join);
    else
      return optimizeFull(join);
  }
  ...
  private Set<SubPlan> dispatch(SubPlan node)
  {
    switch (node.rootKind())
    {
      ...
      case Filter:
      case InSub:
      case Exists: return onFilter(node);
  ...
  private boolean isFullyOptimized(SubPlan subPlan)
  {
    ...
    {
      if (!memo.isRegistered(plan, plan.childOf(node, i)))
        return false;
    }
    return true;
  }
  ...
  private SubPlan replaceChild(SubPlan replaced, int childIdx, SubPlan replacement)
  {
    ...
    if (!replacedPlan.isPresent(result))
      return null;
  ...
  private int preprocess(PlanContext plan)
  {
    ...
    if ((OptimizerSupport.optimizerTweaks & OptimizerSupport.TWEAK_SORT_FILTERS_DURING_REWRITE)
        != 0)
  ...
  private int enforceInnerJoin(PlanContext plan, int planRoot)
  {
    ...
    if (inference.isModified() && tracing)
      traceStep(original, plan, PreprocessRule.EnforceInnerJoin.ruleId());
    return planRoot;
  }
  ...
  private int reduceSort(PlanContext plan, int planRoot)
  {
    ...
    if (reduceSort.isReduced() && tracing)
      traceStep(original, plan, PreprocessRule.ReduceSort.ruleId());
    return planRoot;
  }
  ...
  private int reduceDedup(PlanContext plan, int planRoot)
  {
    ...
    if (reduceDedup.isReduced() && tracing)
      traceStep(original, plan, PreprocessRule.ReduceDedup.ruleId());
    return planRoot;
  }
  ...
  private int convertExists(PlanContext plan, int planRoot)
  {
    ...
    if (converter.isConverted() && tracing)
      traceStep(original, plan, PreprocessRule.ConvertExists.ruleId());
    return planRoot;
  }
  ...
  private int flipRightJoin(PlanContext plan, int planRoot)
  {
    ...
    if (converter.isFlipped() && tracing)
      traceStep(original, plan, PreprocessRule.FlipRightJoin.ruleId());
    return planRoot;
  }

  private Set<PlanContext> collectRewritten(PlanContext origin, Set<SubPlan> subPlans)
  {
    final boolean shouldSortFilters =
        (OptimizerSupport.optimizerTweaks & OptimizerSupport.TWEAK_SORT_FILTERS_BEFORE_OUTPUT) != 0;
    ...
    for (SubPlan subPlan : subPlans)
    {
      ...
      final PlanContext plan = subPlan.plan();
      if (shouldSortFilters)
        OptimizerSupport.normalizeFilter(plan, plan.root());
