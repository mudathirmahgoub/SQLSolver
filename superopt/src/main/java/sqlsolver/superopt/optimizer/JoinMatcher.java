...
class JoinMatcher
{
  ...
  {
    if (rootJoineeIdx < 0 && !allowFlip())
      return null;
    if (!joinTree.isEligibleRoot(rootJoineeIdx))
      return null;
    ...
    if (baseMatch.matchRootNode() == joinTree.rootJoiner())
      derived.setMatchRootNode(joiner);

    if (derived.matchOne(op, joiner))
      return derived;
    else
      return null;
  }
