...
public abstract class OptimizerSupport
{
  ...
  {
    if ((rootId = normalizeJoin(plan, rootId)) == NO_SUCH_NODE)
      return NO_SUCH_NODE;
    if ((rootId = normalizeProj(plan, rootId)) == NO_SUCH_NODE)
      return NO_SUCH_NODE;
    if ((rootId = normalizeFilter(plan, rootId)) == NO_SUCH_NODE)
      return NO_SUCH_NODE;
    if (!compensateSubqueryExpr(plan, rootId))
      return NO_SUCH_NODE;
    return rootId;
  }
  ...
  {
    Printer.output.println("=== begin dump trace ===");
    final List<OptimizationStep> steps = optimizer.traceOf(result);
    if (!steps.isEmpty())
    {
      final PlanContext startPoint = steps.get(0).source();
      Printer.output.println(stringifyTree(startPoint, startPoint.root(), false, true));
      for (OptimizationStep step : steps)
      {
        final PlanContext target = step.target();
        final String ruleString;
        if (step.rule() != null)
          ruleString = step.rule().toString();
        else
          ruleString = PreprocessRule.getDescByRuleId(step.extra());

        Printer.output.println("~~ " + ruleString);
        Printer.output.println("==> " + stringifyTree(target, target.root(), true, true));
      }
    }
    Printer.output.println("=== end dump trace ===");
  }
