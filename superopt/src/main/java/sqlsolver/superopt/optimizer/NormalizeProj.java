...
class NormalizeProj
{
  ...
  int normalizeTree(int rootId)
  {
    ...
    {
      if (normalizeTree(plan.childOf(rootId, i)) == NO_SUCH_NODE)
        return NO_SUCH_NODE;
    }

    if (kind == Proj && shouldReduceProj(rootId))
      return reduceProj(rootId);
    if (shouldInsertProjBefore(rootId))
      return insertProjBefore(rootId);
    return rootId;
  }
  ...
  int insertProjBefore(int position)
  {
    ...
    if (!reBinder.rebindToRoot(projNode))
      return NO_SUCH_NODE;
  ...
  int reduceProj(int proj)
  {
    ...
    final ValueRefReBinder reBinder = new ValueRefReBinder(plan);
    if (!reBinder.rebindToRoot(replacement))
      return NO_SUCH_NODE;
  ...
  boolean shouldInsertProjBefore(int node)
  {
    ...
    if (nodeKind.isFilter())
      return parent == NO_SUCH_NODE || plan.kindOf(parent) == Join;
    if (nodeKind == Join)
      return parent == NO_SUCH_NODE;
    return false;
  }
  ...
  boolean shouldReduceProj(int node)
  {
    ...
    if (childKind.isFilter())
    {
      if (!parentKind.isFilter())
        return false;
      if (parentKind.isSubqueryFilter() && indexOfChild(plan, node) != 0)
        return false;
    }
    else if (childKind == Input)
    {
      if (!parentKind.isFilter() && parentKind != Join)
        return false;
      if (parentKind.isSubqueryFilter() && indexOfChild(plan, node) != 0)
        return false;
    }
    ...
    {
      if (PlanSupport.deRef(plan, outputs.get(i)) != inputs.get(i))
        return false;
    }
