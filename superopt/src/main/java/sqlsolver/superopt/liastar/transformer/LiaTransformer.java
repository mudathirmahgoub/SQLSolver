...
public class LiaTransformer
{
  ...
  private boolean contradicts(LiaStar g, List<String> v, List<String> w, LiaStar f)
  {
    ...
    final LiaStar g1 =
        g.deepcopy().transformPostOrder(fm -> renameVarsBesides(fm, v, renameMap1, usedVarNames));
    final LiaStar g2 =
        g.deepcopy().transformPostOrder(fm -> renameVarsBesides(fm, v, renameMap2, usedVarNames));
    final LiaStar g3 =
        g.deepcopy().transformPostOrder(fm -> renameVarsBesides(fm, v, renameMap3, usedVarNames));
  ...
  private LiaStar renameVarsBesides(
      LiaStar f, Collection<String> besides, Map<String, String> renameMap, Collection<String> used)
  {
    if (f instanceof LiaVarImpl var)
    {
      ...
      if (renamedVar != null)
        return LiaStar.mkVar(f.isInStar(), renamedVar);
      // find an unused name
      int count = 1;
      String newName;
      do
  ...
  {
    final LiaStar isZero = LiaStar.mkEq(false, LiaStar.mkVar(false, v), LiaStar.mkConst(false, 0));
  ...
  private LiaStar implyVarNonNegative(List<String> v, List<String> w, LiaStar f)
  {
    ...
    for (int i = 0; i < v.size(); i++)
    {
      ...
      {
        final LiaStar outVarNN =
            LiaStar.mkLe(false, LiaStar.mkConst(false, 0), LiaStar.mkVar(false, outVar));
