...
public class AugmentationFinder
{
  ...
  private List<Long> find(boolean withLimit, int lowerBound, int upperBound)
  {
    try (final Context ctx = new Context())
    {
      ...
      final BoolExpr limitFormula =
          defineVarsByNamesWithLimit(ctx, varDef, slsVector, withLimit, lowerBound, upperBound);
      ...
      final BoolExpr mainFormula =
          ctx.mkAnd(limitFormula, varConstraint, targetFormula, slsNegFormula);
      if (!withLimit)
      {
      ...
      final LiaStar vectorTotalLia =
          vectorLia.stream().reduce((a, b) -> LiaStar.mkPlus(false, a, b)).get();
