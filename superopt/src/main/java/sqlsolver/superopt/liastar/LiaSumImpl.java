...
public class LiaSumImpl extends LiaStar
{
  ...
  public EstimateResult estimate()
  {
    ...
    int n = (int) Math.ceil(6 * d
        * (Math.log(4 * d) / Math.log(2) + 2 * r.m * Math.log(2 + (rn + 1) * r.a) / Math.log(2)));
  ...
  public Expr expandStarWithK(Context ctx, Solver sol, String suffix)
  {
    ...
    double a = r.a;
    int k = (int) Math.ceil(
        4 * d * (Math.log(4 * d) / Math.log(2) + 2 * m * Math.log(2 + (n + 1) * a)) / Math.log(2));
    if (suffix.matches("(\\_1)*"))
    {
      // Printer.output.printf("expanding star for %s; suffix '%s'...\n", this.toString(), suffix);
      // Printer.output.printf("expanding star: n = %d, m = %d, a = %f; k = %d\n", n, m, a, k);
    }
  ...
  public boolean equals(Object that)
  {
    ...
    return outerVector.equals(tmp.outerVector) && innerVector.equals(tmp.innerVector)
   ...
   *     <li>If a literal f has common vars with a literal g in the closure, then f is in the
   * closure.</li>
   * </ul>
   ...
   */
  public static void classifyConjunctionLiteralsByParamClosure(
      LiaStar formula, List<LiaStar> noParamPart, List<LiaStar> paramPart, Set<String> params)
  {
  ...
  private List<LiaStar> destructToRemoveParam(LiaStar formula, Set<String> params)
  {
    ...
    {
      if (!canSeparate(f, params, importantVars))
        return null;
    }
    return result;
  }
   ...
   */
  public static boolean isSeparateFrom(
      Set<String> paramPartVars, Set<String> noParamPartVars, Set<String> importantVars)
  {
  ...
  private LiaStar reconstruct(List<LiaStar> destruction, Set<String> params)
  {
    ...
    if (destruction.size() == 1)
    {
      // v in {v'|f(v')}*
      return mkSum(
          innerStar, new ArrayList<>(outerVector), new ArrayList<>(innerVector), destruction.get(0))
          .separateParams(params);
    }
  ...
  private LiaStar separateParams(Set<String> params)
  {
    ...
    final LiaStar vectorIsZero = eq(innerStar,
        nameToLia(innerStar, new ArrayList<>(outerVector)),
        liaZero(innerStar, outerVector.size()));
    final LiaStar newStar =
        mkSum(innerStar, new ArrayList<>(outerVector), new ArrayList<>(innerVector), noParamFormula)
            .updateInnerVector();
  ...
  private LiaStar pushUpParameterForOneOutVar(int index, Set<String> newVars, LiaStar formula)
  {
    switch (formula.getType())
    {
      ...
      {
        final LiaStar eq1 =
            pushUpParameterForOneOutVar(index, newVars, ((LiaAndImpl) formula).operand1);
        final LiaStar eq2 =
            pushUpParameterForOneOutVar(index, newVars, ((LiaAndImpl) formula).operand2);
  ...
  private LiaStar[] pushUpParameterInIte(
      int varIndex, LiaStar condition, LiaStar trueValue, Set<String> newVars)
  {
    ...
    for (int i = 0; i < literals.size(); ++i)
    {
      ...
      if (lit instanceof LiaEqImpl)
      {
        ...
        if (equation.operand1 instanceof LiaVarImpl && equation.operand2 instanceof LiaIteImpl)
        {
          ...
          if (notOutInnverVar(var.varName) && iteOp2.equals(mkConst(innerStar, 0))
  ...
  LiaStar replaceNotZeroCondInItecond(LiaStar iteCond, Map<LiaVarImpl, LiaStar> paramNotZeroCond)
  {
  ...
  void replaceNotZeroCond(List<LiaStar> literals, Map<LiaVarImpl, LiaStar> paramNotZeroCond)
  {
      ...
      BiFunction<LiaStar, LiaStar, LiaStar> transformer, LiaStar parent)
  {
    ...
    return mkSum(
        innerStar, new ArrayList<>(outerVector), new ArrayList<>(innerVector), constraints0);
  }
