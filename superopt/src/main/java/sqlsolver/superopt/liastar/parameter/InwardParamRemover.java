...
public class InwardParamRemover
{
  ...
  {
    if (!formula.getType().isAtomicPredicate())
      return false;
  ...
  private LiaStar copyParamLiteralsInward(LiaStar formula)
  {
    ...
    if (cases.getUnitCount() == 0 && conclusions == null)
      return null;
    ...
    for (LiaStar cas : destruction)
    {
      ...
      if (fastReject(mkAnd(false, cas, formula)))
        continue;
      ...
      final LiaStar caseCondInwardFormula = formula.transformPostOrder(f -> {
        if (f instanceof LiaSumImpl s)
        {
          ...
          final LiaStar newConstraints =
              mkAnd(true, prefix.deepcopy(), s.getConstraints().deepcopy());
  ...
  private LiaStar implyByParams(LiaStar formula, Set<LiaVarImpl> params)
  {
    ...
    for (int i = 0, bound = paramList.size(); i < bound; i++)
    {
      ...
      {
        final LiaStar param1 =
            LiaStar.mkVar(innerStar, paramList.get(i).getName(), paramList.get(i).getValueType());
        final LiaStar param2 =
            LiaStar.mkVar(innerStar, paramList.get(j).getName(), paramList.get(j).getValueType());
