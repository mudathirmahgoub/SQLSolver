...
public class LiaNotImpl extends LiaStar
{
  ...
  public EstimateResult estimate()
  {
    if (operand instanceof LiaAndImpl)
    {
      ...
      return LiaStar.mkOr(innerStar, LiaStar.mkNot(innerStar, op1), LiaStar.mkNot(innerStar, op2))
          .estimate();
    }
    else if (operand instanceof LiaEqImpl)
    {
      ...
      return LiaStar
          .mkOr(innerStar, LiaStar.mkLt(innerStar, op1, op2), LiaStar.mkLt(innerStar, op2, op1))
          .estimate();
    }
    ...
    else if (operand instanceof LiaOrImpl)
    {
      ...
      return LiaStar.mkAnd(innerStar, LiaStar.mkNot(innerStar, op1), LiaStar.mkNot(innerStar, op2))
          .estimate();
    }
    else
    {
  ...
  {
    return ctx.mkNot((BoolExpr) operand.expandStarWithK(ctx, sol, suffix));
  }
