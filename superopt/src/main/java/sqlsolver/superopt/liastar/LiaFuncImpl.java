...
public class LiaFuncImpl extends LiaStar
{
  ...
  public Set<String> collectVarNames()
  {
    ...
    for (LiaStar var : vars) result.addAll(var.collectVarNames());
    return result;
  }
  ...
  {
    HashSet<String> result = new HashSet<>();
    for (LiaStar var : vars) result.addAll(var.collectAllVarNames());
    return result;
  }
  ...
  public Expr transToSMT(Context ctx, Map<String, Expr> varsName, Map<String, FuncDecl> funcsName)
  {
    ...
    if (allowsUndefined)
      ...
      return Z3Support.assembleFunction(
          funcName, args, PredefinedFunctions.ValueType.INT, ctx, funcsName);
  ...
  {
    HashSet<String> result = new HashSet<>();
    for (LiaStar var : vars) result.addAll(var.getVars());
    return result;
  }
  ...
  {
    List<LiaStar> newVars =
        new ArrayList<>(vars.stream().map(v -> v.transformPostOrder(transformer)).toList());
  ...
  {
    List<LiaStar> newVars =
        new ArrayList<>(vars.stream().map(v -> v.transformPostOrder(transformer, this)).toList());
  ...
  {
    List<LiaStar> newVars =
        new ArrayList<>(vars.stream().map(v -> v.transformPreOrder(transformer, this)).toList());
