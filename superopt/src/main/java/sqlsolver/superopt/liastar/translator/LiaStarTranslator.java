...
public class LiaStarTranslator
{
  ...
  public LiaStarTranslator(
      Map<UVar, List<Value>> varSchema, UTerm query1, UTerm query2, UVar outVar)
  {
  ...
  public LiaStar uexpPairToLiastar(BVM bvm)
  {
    ...
    {
      Printer.output.println("Lia* before simplification: ");
  ...
  {
    if (t.kind() != UKind.PRED)
      return false;
    UPred pred = (UPred) t;
    if (!isPredOfVarArg(pred))
      return false;
  ...
  private static LiaStar isNullCongruence(Map<UTerm, String> termVarMap, Set<UVarTerm> isNullTuples)
  {
    if (isNullTuples.isEmpty() || termVarMap.isEmpty())
      return null;
  ...
  private LiaStar uexpWithSumToLiastar(UTerm t1, UTerm t2, Map<USum, String> sumVarMap, BVM bvm)
  {
    ...
    result = LiaStar.mkAnd(false, result, sumsToLiaStar(false, bvm, 0, sums, sumVars, termMap));
  ...
  private LiaStar transUexpWithoutSum(
      boolean innerStar, UTerm exp, Map<UTerm, String> uTermToLiaVar, Set<UVarTerm> isnullTuples)
  {
  ...
  private LiaStar transUexpWithoutSum(boolean innerStar,
      UTerm exp,
      ...
      case MULTIPLY -> {
        ...
        for (UTerm t : subTerms)
        {
          ...
          else
          {
            ...
            {
              if (result instanceof LiaIteImpl)
                result = ((LiaIteImpl) result).MultIte(curLia);
              else if (curLia instanceof LiaIteImpl)
                result = ((LiaIteImpl) curLia).MultIte(result);
              else
                result = LiaStar.mkMul(innerStar, result, curLia);
            }
          }
        }
  ...
  case NEGATION,
      ...
      -> {
        ...
        final LiaStar cond = LiaStar.mkEq(innerStar,
  ...
  private LiaStar transFuncWithoutSum(
    ...
    for (UTerm arg : args)
        {
          LiaStar liaOp =
              transUexpWithoutSum(innerStar, arg, uTermToLiaVar, isnullTuples, stringVars);
          liaOps.add(liaOp);
        }
        ...
        {
          if (term instanceof UConst || term instanceof UTable || term instanceof ULiaVar)
            return false;
          if (term instanceof UString)
            return true;
        ...
        /**
         * Replace a bound var in each summation with a common tuple according to the chosen BVM (a
         * set of bound vars).
         */
        private void injectCommonTuple(List<UTerm> sums, Set<UVar> bvm)
        {
          ...
          for (Map.Entry<Schema, Set<UVar>> entry : schemaGroups.entrySet())
          {
            ...
            for (UVar bv : entry.getValue()) commonTuples.put(bv, commonTuple);
        ...
        private LiaStar sumsToLiaStar(boolean isInner,
            BVM bvm,
            int depth,
            List<UTerm> sumList,
            ...
            Map<UTerm, String> termVarMap)
        {
          ...
          if (!subSums.isEmpty())
          {
            constraints = LiaStar.mkAnd(true,
                constraints,
                sumsToLiaStar(true, bvm, depth + 1, subSums, subVars, termVarMap));
          }
