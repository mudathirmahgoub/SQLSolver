...
public class LiaStarTranslatorSupport
{
      ...
      Map<USum, String> sumMap, Set<USum> sums, UTerm uexp, NameSequence liaVarName)
  {
    switch (uexp.kind())
    {
      case SUMMATION:
        sums.add((USum) uexp);
        if (sumMap.containsKey(uexp))
          return ULiaVar.mk(sumMap.get(uexp));
      ...
      case NEGATION:
            ...
            case SQUASH:
              return USquash.mk(
                  replaceSummations(sumMap, sums, ((USquash) uexp).body(), liaVarName));
            default: return uexp;
        }
      }
        ...
        public
        static void copyUExprList(List<UTerm> src, List<UTerm> dst)
        {
          for (UTerm sum : src)
          {
            dst.add(sum.copy());
