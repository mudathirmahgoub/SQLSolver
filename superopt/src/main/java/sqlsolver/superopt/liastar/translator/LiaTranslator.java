...
public class LiaTranslator
{
  ...
  public static LiaStar translate(UTerm uexp,
      Map<UVar, List<Value>> varSchema,
      Map<UVar, String> varMap,
      Map<USum, String> sumVarMap)
  {
  ...
  {
    final UTerm expWithSumReplaced =
        LiaStarTranslatorSupport.replaceSummations(sumVarMap, new HashSet<>(), uexp, liaVarName);
        ...
        for (UTerm t : subt)
        {
          ...
          else
          {
            if (result instanceof LiaIteImpl)
              ...
              result = LiaStar.mkPlus(false, result, curLia);
          }
        }
      ...
      case MULTIPLY -> {
        ...
        for (UTerm t : subt)
        {
          ...
          if (curLia instanceof LiaConstImpl)
          {
            ...
            if (value == 0)
              return LiaStar.mkConst(false, 0);
            if (value == 1)
              continue;
          }
          if (result == null)
          ...
          else
          {
            if (result instanceof LiaIteImpl)
              ...
              result = LiaStar.mkMul(false, result, curLia);
          }
        }
  ...
  case NEGATION
    ...
    return LiaStar.mkConst(false, ((UConst) exp).value());
      ...
      -> {
    final UTable table = (UTable) exp; final String tblName = table.tableName().toString();
    ...
    if (pred.isPredKind(UPred.PredKind.FUNC) && isPredOfVarArg(pred)) {
      // Case 1. [p(a(t))]
      final List<UVar> pArgs = UExprSupport.getPredVarArgs(pred); assert pArgs.size() == 1;
      final UVar predVar = pArgs.get(0); // a(t)
      final String predName = pred.predName().toString();
      final LiaStar lVar =
          LiaStar.mkVar(false, varMap.computeIfAbsent(predVar, v -> newLiaVarName()));
      ...
      if (liaVar0 instanceof LiaConstImpl && liaVar1 instanceof LiaConstImpl) {
        ...
        boolean b =
            switch (pred.predKind()) {
              case EQ -> value0 == value1; case NEQ -> value0 != value1;
              case LE -> value0 <= value1;
              case LT -> value0<value1; case GE -> value0 >= value1; case GT -> value0> value1;
              default -> throw new IllegalArgumentException("unsupported predicate in Uexpr.");
            };
        return b ? LiaStar.mkConst(false, 1):
    LiaStar.mkConst(false, 0);
}
