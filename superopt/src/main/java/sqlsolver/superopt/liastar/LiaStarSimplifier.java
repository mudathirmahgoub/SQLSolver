...
public class LiaStarSimplifier
{
  ...
  private LiaStar collapseConsecutiveStars(LiaStar f)
  {
    if (f instanceof LiaSumImpl star)
    {
      ...
      return LiaStar.mkSum(star.innerStar, v0, innerStar.innerVector, innerStar.constraints);
    }
    return f;
  }
  ...
  private LiaSumImpl extractInnerStar(LiaStar constraints, List<String> v1)
  {
    if (constraints instanceof LiaAndImpl and)
    {
      ...
      for (String dimV1 : v1)
      {
        ...
        if (dimV2 == null)
          return null; // missing equation for dimV1
        expectedV2.add(dimV2);
      }

      // process the star: rearrange v2,v3
      if (star == null)
        return null; // missing a star
      ...
      return (LiaSumImpl) LiaStar.mkSum(star.innerStar, expectedV2, newV3V4, star.constraints);
    }
    return null;
  }
  ...
  {
    if (eq.operand1 instanceof LiaVarImpl left && eq.operand2 instanceof LiaVarImpl right)
    {
   ...
   */
  private <T, U> ArrayList<U> reorder(List<T> refSource, List<T> refTarget, List<U> source)
  {
  ...
  private List<LiaStar> removeRedundantInnerVar(
      String var, List<LiaStar> conds, List<String> redundantVars)
  {
  ...
  {
    if (f instanceof LiaNotImpl not && not.operand instanceof LiaEqImpl eq)
    {
      // match ite
      final boolean innerStar = f.isInStar();
      final LiaStar zero = LiaStar.mkConst(innerStar, 0);
      final LiaIteImpl ite;
      if (eq.operand1 instanceof LiaIteImpl ite0 && ite0.operand2.equals(zero)
          && eq.operand2.equals(zero))
      {
        // ~(ite=0)
        ite = ite0;
      }
      else if (eq.operand2 instanceof LiaIteImpl ite0 && ite0.operand2.equals(zero)
