...
public class LiaMulImpl extends LiaStar
{
  ...
  {
    return operand1.isPrettyPrintMultiLine() || operand2.isPrettyPrintMultiLine();
  }
  ...
  public LiaStar mergeMult(Map<LiaMulImpl, LiaVarImpl> multToVar)
  {
    ...
    final String type =
        IterableSupport.all(collectAllVars(), v -> Value.TYPE_NAT.equals(v.getValueType()))
        ? Value.TYPE_NAT
        : Value.TYPE_INT;
  ...
  boolean isSimp(LiaStar l)
  {
    ...
    return type.equals(LiaOpType.LMULT) || type.equals(LiaOpType.LVAR)
  ...
  LiaStar transMult(int nbits, ArrayList<String> varSet, int start)
  {
    ...
    for (int i = 0; i < nbits; i = i + 1)
    {
      ...
      {
        result =
            mkIte(true, mkEq(true, mkVar(true, bitName), mkConst(true, 0)), mkConst(true, 0), exp);
      }
      else
      {
        LiaStar tmp = exp;
        ...
        result = mkPlus(true,
            result,
            mkIte(true, mkEq(true, mkVar(true, bitName), mkConst(true, 0)), mkConst(true, 0), tmp));
      }
    }
    return result;
  }
  ...
  {
    return ctx.mkMul((ArithExpr) operand1.expandStarWithK(ctx, sol, suffix),
