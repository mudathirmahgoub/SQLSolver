...
public class Z3Support
{
  ...
  public static BoolExpr defineVarsByNamesWithLimit(Context ctx,
      Map<String, Expr> varDef,
   ...
   */
  public static BoolExpr defineVarsByVarsWithLimit(Context ctx,
      Map<String, Expr> varDef,
      Iterable<LiaVarImpl> vars,
      boolean withLimit,
      int lowerBound,
      int upperBound)
  {
   ...
   */
  public static Expr assemblePredefinedFunction(
      String funcName, List<Expr> args, Context ctx, Map<String, FuncDecl> funcs)
  {
    ...
    switch (funcName)
    {
      case PredefinedFunctions.NAME_SQRT:
      {
        ...
        FPExpr fpExpr =
            ctx.mkFPToFP(ctx.mkFPRoundNearestTiesToEven(), realExpr, ctx.mkFPSortDouble());
      ...
      default:
      {
        ...
        if (func == null)
        {
          ...
          final PredefinedFunctions.FunctionFamily ff =
              PredefinedFunctions.getFunctionFamily(funcName, arity);
          if (ff == null)
          {
   ...
   */
  public static Expr assembleFunction(String funcName,
      List<Expr> args,
  ...
  public static boolean isValidLia(LiaStar lia, Set<String> universalBVNames)
  {
    ...
    // We want to check validity of "forall universalBVs. some universalBVs >= 0 -> exists existBVs.
    // some existBVs >= 0 /\ lia", which can be reduced to falsehood of "exists universalBVs. some
    // universalBVs >= 0 /\ forall existBVs. some existBVs >= 0 -> not lia", or unsatisfiability of
    ...
    try (final Context ctx = new Context())
    {
      ...
      {
        body = ctx.mkForall(existBVNames.stream().map(varDef::get).toList().toArray(new Expr[0]),
  ...
  public static Model findModel(LiaStar lia, Set<String> existBVNames, Context ctx)
  {
    ...
    // Check satisfiability of "some existBVs >= 0 /\ (forall universalBVs. some universalBVs >= 0
    // -> lia)" Non-negativity of vars (some universalBVs and existBVs >= 0) is considered. Whether
    // a var is non-negative is determined by its type. (natural vars are non-negative)
    final Map<String, Expr> varDef = new HashMap<>();
    final BoolExpr nnUn = defineVarsByVars(ctx, varDef, universalBVs);
    final BoolExpr nnEx = defineVarsByVars(ctx, varDef, existBVs);
    ...
    {
      body = ctx.mkForall(universalBVNames.stream().map(varDef::get).toList().toArray(new Expr[0]),
