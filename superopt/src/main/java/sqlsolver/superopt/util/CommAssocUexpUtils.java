...
public class CommAssocUexpUtils
{
  ...
  private static boolean matchGroups(
      List<UTerm> lefts, List<UTerm> rights, SetMatching<String> matching)
  {
    if (lefts.size() == 1)
    {
  ...
  public static boolean groupSimilarVariables(
      List<UTerm> factors, List<UTerm> thatFactors, SetMatching<String> matching)
  {
    int size = factors.size();
    if (size != thatFactors.size())
      return false;
    ...
    for (int i = 0; i < size; i++)
    {
      ...
      if (i > 0 && lastHash != lHash)
      {
        ...
        if (!matchGroups(lefts, rights, matching))
          return false;
        // a new group
        lefts = new ArrayList<>();
