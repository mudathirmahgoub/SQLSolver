...
public class Fingerprint
{
  ...
  private static char getOpIdentifier(OpKind kind, boolean dedup)
  {
    switch (kind)
    {
      case PROJ: return dedup ? 'q' : 'p';
      case SIMPLE_FILTER: return 'f';
      case IN_SUB_FILTER: return 's';
      case INNER_JOIN: return 'j';
      case LEFT_JOIN: return 'l';
      case AGG: return 'a';
      case UNION: return dedup ? 'v' : 'u';
      default: return '?';
    }
  }
  ...
  private static class OpFingerprintBuilder
  {
    ...
    private Fingerprint build(Op op, int limit)
    {
      ...
      final boolean dedup = op.kind() == OpKind.PROJ && ((Proj) op).deduplicated()
  ...
  private static class PlanFingerprintBuilder
  {
    ...
    private char identifierOf(int node)
    {
      ...
      if (kind == PlanKind.Proj)
        return getOpIdentifier(OpKind.PROJ, isDedup(plan, node));
      else if (kind == PlanKind.SetOp)
        return getOpIdentifier(OpKind.UNION, isDedup(plan, node));
      else if (kind == PlanKind.Agg)
        return getOpIdentifier(OpKind.AGG, false);
      else if (kind == PlanKind.Filter)
        return getOpIdentifier(OpKind.SIMPLE_FILTER, false);
      else if (kind == PlanKind.InSub)
        return getOpIdentifier(OpKind.IN_SUB_FILTER, false);
    ...
    private int[] countFilters(int node)
    {
      ...
      {
        ++total;
        if (plan.kindOf(node).isSubqueryFilter())
          ++subquery;
    ...
    private int[] countJoins(int node)
    {
      ...
      {
        ++total;
        if (joinKindOf(plan, node) == JoinKind.LEFT_JOIN)
          ++leftJoin;
        node = plan.childOf(node, 0);
      }
