...
public interface Complexity extends Comparable<Complexity>
{
  ...
  default int compareTo(Complexity other, boolean preferInnerJoin)
  {
    ...
    for (int i = 0, bound = opCount0.length; i < bound; i++)
    {
      ...
      if (i < OpKind.values().length && OpKind.values()[i].isJoin())
        continue;

      if (result < 0 && opCount0[i] > opCount1[i])
        return 0;
      if (result > 0 && opCount0[i] < opCount1[i])
        return 0;
      if (opCount0[i] > opCount1[i])
        result = 1;
      else if (opCount0[i] < opCount1[i])
        result = -1;
    }
    ...
    final int numInnerJoin0 =
        opCount0[OpKind.INNER_JOIN.ordinal()] + opCount0[OpKind.CROSS_JOIN.ordinal()];
    final int numOuterJoin0 = opCount0[OpKind.LEFT_JOIN.ordinal()]
        + opCount0[OpKind.RIGHT_JOIN.ordinal()] + opCount0[OpKind.FULL_JOIN.ordinal()];
    final int numInnerJoin1 =
        opCount1[OpKind.INNER_JOIN.ordinal()] + opCount1[OpKind.CROSS_JOIN.ordinal()];
    final int numOuterJoin1 = opCount1[OpKind.LEFT_JOIN.ordinal()]
        + opCount1[OpKind.RIGHT_JOIN.ordinal()] + opCount1[OpKind.FULL_JOIN.ordinal()];
    final int numJoin0 = numInnerJoin0 + numOuterJoin0;
    final int numJoin1 = numInnerJoin1 + numOuterJoin1;

    if (numJoin0 < numJoin1)
      return -1;
    if (numJoin0 > numJoin1)
      return 1;
