...
public class PlanComplexity implements Complexity
{
  ...
  private void countOps(PlanContext plan, int nodeId)
  {
    ...
    //
    // } else {
    //   final int joinOpKindNum = filter(Arrays.stream(OpKind.values()).toList(),
    //   OpKind::isJoin).size(); if (nodeKind.ordinal() > PlanKind.Join.ordinal())
    ...
    if (nodeKind == PlanKind.Proj && isDedup(plan, nodeId))
