...
public class SetSolver
{
  ...
  static
  {
    ...
    TRANSLATOR_CONFIGS.add(
        new SetTranslator.Config(SetTranslator.Config.VAR_MODE_COLUMN_AS_VAR, false));
    TRANSLATOR_CONFIGS.add(
        new SetTranslator.Config(SetTranslator.Config.VAR_MODE_COLUMN_AS_VAR, true));
  }
  ...
  public VerificationResult proveEq()
  {
    ...
    for (SetTranslator.Config config : TRANSLATOR_CONFIGS)
    {
      ...
      {
        Printer.output.println("Set solver result (config #" + (++count) + "): " + result);
      }
  ...
  private VerificationResult proveEq(SetTranslator.Config config)
  {
    ...
    try (final Context z3 = new Context())
    {
      try
      {
        ...
        if (LogicSupport.dumpLiaFormulas)
        {
          Printer.output.println("Term 1:");
          Printer.output.println(term1.toPrettyString());
          Printer.output.println("Term 2:");
          Printer.output.println(term2.toPrettyString());
          Printer.output.println("FOL formula 1:");
          Printer.output.println(arithExp1);
          Printer.output.println("FOL formula 2:");
          Printer.output.println(arithExp2);
        }
        // solve
