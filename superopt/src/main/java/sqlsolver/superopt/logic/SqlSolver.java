...
import sqlsolver.superopt.liastar.Cvc5LiaStarSolver;
...
import sqlsolver.superopt.uexpr.UAdd;
...
import sqlsolver.superopt.uexpr.UKind;
import sqlsolver.superopt.uexpr.UMul;
import sqlsolver.superopt.uexpr.UMulImpl;
import sqlsolver.superopt.uexpr.UName;
import sqlsolver.superopt.uexpr.UNeg;
import sqlsolver.superopt.uexpr.UPred;
import sqlsolver.superopt.uexpr.USquash;
import sqlsolver.superopt.uexpr.USum;
import sqlsolver.superopt.uexpr.UTerm;
import sqlsolver.superopt.uexpr.UVar;
import sqlsolver.superopt.uexpr.UVarTerm;
...
public class SqlSolver
{
  ...
  public VerificationResult proveEq()
  {
    ...
    {
      Printer.output.println("==> Rewritten UExpressions sent to Lia solver: ");
      Printer.output.println("[[q0]](" + outVar1 + ") := ");
      query1.prettyPrint();
      Printer.output.println();
      Printer.output.println("[[q1]](" + outVar2 + ") := ");
      query2.prettyPrint();
      Printer.output.println();
    }
    ...
    if (LogicSupport.dumpLiaFormulas)
    {
      Printer.output.println("==> Try set solver:");
    }
    final VerificationResult setResult =
        new SetSolver(query1, query2, varSchema, tableSchema).proveEq();
    if (LogicSupport.dumpLiaFormulas)
    {
      Printer.output.println("Set solver result: " + setResult);
    }
    if (setResult == VerificationResult.EQ)
      return VerificationResult.EQ;
    ...
    while (currentBVM != null)
    {
      ...
      if (LogicSupport.dumpLiaFormulas)
      {
        Printer.output.println("==> Using bound var matching " + (++count) + ": ");
      ...
      if (LogicSupport.dumpLiaFormulas)
      {
        Printer.output.println("==> Lia* formula: ");
        Printer.output.println(fstar);
      }
      // solve LIA*
      for (Properties config : LIA_SOLVER_CONFIGS)
      {
        ...
        switch (result)
        {
          case UNSAT:
            ...
            return VerificationResult.EQ;
          default: allSAT = allSAT && result == LiaSolverStatus.UNKNOWN;
        }
      }
      // move onto next BVM
  ...
  public static void analyseNullPredicate(UTerm expr, Set<UVar> nullTuples, Set<UVar> notnullTuples)
  {
    UKind kind = expr.kind();
    switch (kind)
    {
    ...
    while (isModified == true)
    {
      ...
      for (UVar nullTuple : nullTuples)
      {
        for (Set<UVar> eqTuples : eqPairs)
        {
          ...
          boolean useNullTuple = (nullTuple.kind() == UVar.VarKind.BASE)
  ...
  public static UVar selectConcreteTuple(List<Set<UVar>> eqRels, UVar boundedVar, Set<UVar> outVars)
  {
