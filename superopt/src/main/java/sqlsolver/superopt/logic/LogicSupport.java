...
public abstract class LogicSupport
{
  ...
  public static boolean dumpFormulas = true;
  public static boolean dumpLiaFormulas = true;
  ...
  {
    LogicSupport.dumpFormulas = true;
  }
  ...
  public static VerificationResult proveEqByLIAStar(
      UExprConcreteTranslationResult uExprs, Schema schema)
  {
    try
    {
  ...
  public static VerificationResult proveEqByLIAStar(
      Substitution rule, SubstitutionTranslatorResult extraInfo, boolean isConcretePlan)
  {
    try
    {
      int flag = UExprSupport.UEXPR_FLAG_INTEGRITY_CONSTRAINT_REWRITE;
      if (isConcretePlan)
        flag |= UExprSupport.UEXPR_FLAG_VERIFY_CONCRETE_PLAN;
      ...
      Substitution rule, SubstitutionTranslatorResult extraInfo, boolean isConcretePlan)
  {
    ...
    try
    {
      ...
      if (all(srcFuncSymbols,
              f -> ((Agg) rule._0().symbols().ownerOf(f)).aggFuncKind() != AggFuncKind.UNKNOWN)
          && all(tgtFuncSymbols,
              f -> ((Agg) rule._1().symbols().ownerOf(f)).aggFuncKind() != AggFuncKind.UNKNOWN))
        return proveEqByLIAStar(rule, extraInfo, isConcretePlan);
      ...
      {
        if (visitedFuncSymbols.contains(fSym))
          continue;
        ...
        if (any(eqFuncSyms, s -> ((Agg) s.ctx().ownerOf(s)).deduplicated()))
          ...
          feasibleFuncMap.put(eqFuncSyms, AggFuncKind.commonAggFuncKinds);
  ...
  private static VerificationResult enumAggFunc(Substitution rule,
  ...
  private static VerificationResult proveEqByLIAStarSelectedIC(
      RelNode p0, RelNode p1, Schema schema, int extraFlags)
  {
    int selectedIC = 0;
    do
    {
      QueryUExprICRewriter.selectIC(selectedIC);
      final UExprConcreteTranslationResult uExprsWithICRewrite = UExprSupport.translateQueryToUExpr(
          p0, p1, schema, UExprSupport.UEXPR_FLAG_INTEGRITY_CONSTRAINT_REWRITE | extraFlags);
  ...
  private static VerificationResult proveEqByLIAStarConcreteNoSortWithIC(
      RelNode p0, RelNode p1, Schema schema, int extraFlags)
  {
    final UExprConcreteTranslationResult uExprsWithIC = UExprSupport.translateQueryToUExpr(
        p0, p1, schema, UExprSupport.UEXPR_FLAG_INTEGRITY_CONSTRAINT_REWRITE | extraFlags);
    if (uExprsWithIC != null)
    {
      ...
      final VerificationResult noSelectedICResult =
          LogicSupport.proveEqByLIAStar(uExprsWithIC, schema);
      if (noSelectedICResult == VerificationResult.EQ)
        return VerificationResult.EQ;
      if (proveEqByLIAStarSelectedIC(p0, p1, schema, extraFlags) == VerificationResult.EQ)
        return VerificationResult.EQ;
    }
    return (uExprsWithIC == null) ? VerificationResult.UNKNOWN : VerificationResult.NEQ;
  }

  private static VerificationResult proveEqByLIAStarConcreteNoSort(
      RelNode p0, RelNode p1, Schema schema)
  {
    VerificationResult resICUnexplainedPred = proveEqByLIAStarConcreteNoSortWithIC(
        p0, p1, schema, UExprSupport.UEXPR_FLAG_NO_EXPLAIN_PREDICATES);
    if (resICUnexplainedPred == VerificationResult.EQ)
      return VerificationResult.EQ;
    VerificationResult resIC = proveEqByLIAStarConcreteNoSortWithIC(p0, p1, schema, 0);
    if (resIC == VerificationResult.EQ)
      return VerificationResult.EQ;
    final UExprConcreteTranslationResult uExprs =
        UExprSupport.translateQueryToUExpr(p0, p1, schema, 0);
    if (uExprs != null)
    {
      if (!isEqualTwoValueList(uExprs.srcTupleVarSchemaOf(uExprs.sourceOutVar()),
              uExprs.tgtTupleVarSchemaOf(uExprs.targetOutVar())))
      {
        return VerificationResult.NEQ;
      }
      final VerificationResult res0 = LogicSupport.proveEqByLIAStar(uExprs, schema);
      if (res0 == VerificationResult.EQ)
        return VerificationResult.EQ;
    }
    return (resICUnexplainedPred == VerificationResult.UNKNOWN
               || resIC == VerificationResult.UNKNOWN)
  ...
  static boolean containsLatentSummation(UTerm term)
  {
    ...
    if (kind == UKind.SUMMATION)
      return true;
    if (kind == UKind.SQUASH || kind == UKind.NEGATION || kind.isTermAtomic())
      return false;
    if (kind == UKind.ADD || kind == UKind.MULTIPLY)
      for (UTerm subTerm : term.subTerms())
      {
        if (containsLatentSummation(subTerm))
          return true;
      }
    return false;
  }
