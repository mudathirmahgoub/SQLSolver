...
class LogicProver
{
  ...
  {
    if (LogicSupport.isFastRejected(uExprs))
  ...
  private void trConstraints()
  {
    ...
    for (var tableSym : srcSide.symbolsOf(sqlsolver.superopt.fragment.Symbol.Kind.TABLE))
    {
      ...
      if (touched.add(tableName))
        trTableBasic(tableName);
    }
    touched.clear();
    ...
    for (var attrsSym : srcSide.symbolsOf(sqlsolver.superopt.fragment.Symbol.Kind.ATTRS))
    {
      ...
      if (touched.add(attrsName))
        trAttrsBasic(attrsName);
    }
    touched.clear();
  ...
  {
    if (c.symbols()[1].kind() == sqlsolver.superopt.fragment.Symbol.Kind.TABLE)
      return;
    ...
    final BoolExpr assertion = mkForall(vars,
        z3.mkEq(outerProj.apply(outerSchema, innerProj.apply(innerSchema, tuple)),
  ...
  private VerificationResult proveEq0(UTerm masterTerm, UTerm slaveTerm)
  {
    ...
    /*
     ...
     Sum{y}(Z(y)) != 1 can be broken into (q2 \/ q3 \/ q4), where
       ...
       We prove (X = Y /\ X != 0 /\ q2), (X = Y /\ X != 0 /\ q3) and (X = Y /\ X != 0 /\ q4) are all
     unsat.
    ...
    */
    ...
    if (answer != Status.UNSATISFIABLE)
  ...
  private ArithExpr trAsBag(UTerm uExpr)
  {
    ...
    else if (kind == UKind.MULTIPLY)
    {
      ...
      {
        if (term.kind() == UKind.TABLE)
          nums.add((ArithExpr) trAtom(term, false));
        else if (term.kind() == UKind.ADD)
          nums.add(trAsBag(term));
        else if (term.kind() == UKind.MULTIPLY)
          nums.add(trAsBag(term));
        else
          bools.add(trAsSet(term));
      }
      final ArithExpr[] factors = nums.toArray(ArithExpr[] ::new);
      if (bools.isEmpty())
        return mkMul(factors);
      else
      {
    ...
    else if (kind.isUnary())
    {
      ...
      return (ArithExpr) (kind == UKind.SQUASH ? z3.mkITE(e, one(), zero())
                                               : z3.mkITE(e, zero(), one()));
    }
    else
    {
  ...
  private Expr trVar(UVar var)
  {
    ...
    if (kind == UVar.VarKind.BASE)
      return z3.mkConst(name, tupleSort());
  ...
  private BoolExpr trPredAtom(UPred predTerm)
  {
    ...
    switch (predKind)
    {
      case FUNC:
      {
        ...
        if (UName.FUNC_IS_NULL_NAME.equals(funcName))
          return mkIsNull(arg);
        else
          return (BoolExpr) predFunc(funcName).apply(arg);
      }
      case GT:
      case EQ:
      case GE:
      ...
      case NEQ:
      {
        ...
        if (!UExprSupport.isPredOfVarArg(predTerm))
          return null;
      ...
      default:
        ...
        private Status check(Solver solver, BoolExpr... exprs)
        {
          ...
          {
            Printer.output.println("==== Begin of Snippet-" + (++callCount) + " ====");
            Printer.output.println(solver);
            Printer.output.println("(check-sat)");
            Printer.output.println("==== End of Snippet-" + callCount + " ====");
            Printer.output.println("==> Result: " + res);
            return res;
          }
          solver.pop();
          return res;
