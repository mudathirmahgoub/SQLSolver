...
class FragmentUtils
{
  ...
  static int structuralHash(Op tree)
  {
    ...
    for (Op operator : tree.predecessors())
    {
      ...
      if (operator == null || operator instanceof Input)
        h = h * 31;
      else
        h = h * 31 + structuralHash(operator);
    }
    return h;
  }
  ...
  static boolean structuralEq(Op tree0, Op tree1)
  {
    ...
    for (int i = 0, bound = prevs0.length; i < bound; i++)
      if (!structuralEq(prevs0[i], prevs1[i]))
        return false;
  ...
  static int structuralCompare(Op tree0, Op tree1)
  {
    ...
    if (type0.ordinal() < type1.ordinal())
      return -1;
    if (type0.ordinal() > type1.ordinal())
      return 1;
    if (!isDedup(tree0) && isDedup(tree1))
      return -1;
    if (isDedup(tree0) && !isDedup(tree1))
      return 1;
  ...
  static StringBuilder structuralToString(Op tree, SymbolNaming naming, StringBuilder builder)
  {
    ...
    if (tree.kind() == OpKind.PROJ && ((Proj) tree).deduplicated())
      builder.append('*');
    if (tree.kind().isSetOp() && ((SetOp) tree).deduplicated())
      builder.append('*');
    if (tree.kind() == OpKind.AGG && ((Agg) tree).deduplicated())
      builder.append('*');
    ...
    if (naming != null)
      switch (tree.kind())
      {
        ...
        case INNER_JOIN, LEFT_JOIN, RIGHT_JOIN, FULL_JOIN:
          ...
          break;
        case CROSS_JOIN: break;
        case UNION, INTERSECT, EXCEPT: break;
        case AGG:
  ...
  static void bindNames(Op op, String[] names, SymbolNaming naming)
  {
    switch (op.kind())
    {
      case INPUT: naming.setName(((Input) op).table(), names[1]); break;
      case INNER_JOIN, LEFT_JOIN, RIGHT_JOIN, FULL_JOIN:
      ...
      case SIMPLE_FILTER:
        ...
        break;
      case IN_SUB_FILTER: naming.setName(((InSubFilter) op).attrs(), names[1]); break;
      case EXISTS_FILTER: break;
      case PROJ:
