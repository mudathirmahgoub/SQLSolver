  ...
  private void initAttrsSub() {
        ...
        for (Symbol lhsSource : viableSources.get(lhs))
          if (lhsSource.kind() == Symbol.Kind.TABLE)
            for (Symbol rhsSource : viableSources.get(rhs))
              ...
              {
                constraints.add(
                    Constraint.mk(Constraint.Kind.Reference, lhsSource, lhs, rhsSource, rhs));
                constraints.add(
                    Constraint.mk(Constraint.Kind.Reference, rhsSource, rhs, lhsSource, lhs));
              }
      }
    }
    ...
    int indexOfInstantiation(Symbol from, Symbol to)
    {
      ...
      for (int i = begin; i < end; ++i)
      {
        ...
        if (instantiation.symbols()[0] == to && instantiation.symbols()[1] == from)
          return i;
      }
      assert false;
      return -1;
