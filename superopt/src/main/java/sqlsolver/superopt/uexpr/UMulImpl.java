...
final public class UMulImpl implements UMul
{
  ...
  {
    int sum = 0;
    for (UTerm e : others) sum += e.subTerms().size();
    final List<UTerm> factors = new ArrayList<>(e0.subTerms().size() + sum + 1);
    addFactor(factors, e0);
    for (UTerm factor : others) addFactor(factors, factor);
    return new UMulImpl(factors);
  }
  ...
  {
    if (factor.kind() == UKind.MULTIPLY)
      factors.addAll(factor.subTerms());
    else
      factors.add(factor);
  }
  ...
  public UTerm replaceVar(UVar baseVar, UVar repVar, boolean freshVar)
  {
    final List<UTerm> replaced =
        transformTerms(factors, t -> t.replaceVar(baseVar, repVar, freshVar));
  ...
  public boolean replaceVarInplace(UVar baseVar, UVar repVar, boolean freshVar)
  {
    ...
    {
      if (factor.replaceVarInplace(baseVar, repVar, freshVar))
        modified = true;
    }
    return modified;
  }
  ...
  public boolean replaceVarInplaceWOPredicate(UVar baseVar, UVar repVar)
  {
    ...
    for (UTerm factor : factors)
    {
      if (factor.replaceVarInplaceWOPredicate(baseVar, repVar))
        modified = true;
    }
    return modified;
  }
  ...
  public UTerm replaceAtomicTermExcept(UTerm baseTerm, UTerm repTerm, UTerm exceptTerm)
  {
    ...
    if (this.equals(exceptTerm))
      return this;
    final List<UTerm> replaced =
        transformTerms(factors, t -> t.replaceAtomicTermExcept(baseTerm, repTerm, exceptTerm));
    return new UMulImpl(replaced);
  }
  ...
  public UTerm replaceAtomicTerm(UTerm baseTerm, UTerm repTerm)
  {
    assert baseTerm.kind().isTermAtomic();
    final List<UTerm> replaced =
        transformTerms(factors, t -> t.replaceAtomicTerm(baseTerm, repTerm));
    return new UMulImpl(replaced);
  }
  ...
  public boolean equals(Object obj)
  {
    ...
    for (UTerm thisTerm : thisSubTerms)
    {
      ...
      {
        if (Collections.frequency(thisSubTerms, thisTerm)
            != Collections.frequency(thatSubTerms, thisTerm))
        {
          return false;
        }
      }
  ...
  public boolean weakEquals(Object obj)
  {
    ...
    for (UTerm thisTerm : thisSubTerms)
    {
      if (thisTerm.kind() == UKind.PRED && ((UPred) thisTerm).predKind() == UPred.PredKind.EQ
          && thatEQ.find(((UPred) thisTerm).args().get(0)) != null
          && thatEQ.find(((UPred) thisTerm).args().get(0))
              == thatEQ.find(((UPred) thisTerm).args().get(1)))
      {
        continue;
      }
      ...
      else if (!idemTerm(thisTerm)
          && Collections.frequency(thisSubTerms, thisTerm)
              != Collections.frequency(thatSubTerms, thisTerm))
      {
        eq = false;
        break;
    ...
    for (UTerm thatTerm : thatSubTerms)
    {
      if (thatTerm.kind() == UKind.PRED && ((UPred) thatTerm).predKind() == UPred.PredKind.EQ
          && thisEQ.find(((UPred) thatTerm).args().get(0)) != null
          && thisEQ.find(((UPred) thatTerm).args().get(0))
              == thisEQ.find(((UPred) thatTerm).args().get(1)))
      {
        continue;
      }
