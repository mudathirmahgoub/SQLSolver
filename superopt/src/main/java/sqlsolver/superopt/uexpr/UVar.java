...
public interface UVar extends Copyable<UVar>
{
  ...
  {
    if (var.is(BASE))
      return new LinkedHashSet<>(Collections.singleton(var));
  ...
  static UVar getSingleBaseVar(UVar var)
  {
    ...
    if (var.is(BASE))
      ...
      return getSingleBaseVar(var.args()[0]);
  }
  ...
  static UVar removeNestedProjVar(UVar var)
  {
    ...
    if (Arrays.equals(newVars, var.args()))
      return var.copy();
    else
      return new UVarImpl(var.kind(), var.name(), newVars);
  }
