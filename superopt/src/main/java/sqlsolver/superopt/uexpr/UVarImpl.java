...
record UVarImpl(VarKind kind, UName name, UVar[] arguments) implements UVar
{
  ...
  {
    if (!var.is(VarKind.BASE))
      return false;
    if (this.is(VarKind.BASE))
      return this.equals(var);
    else
      return any(asList(arguments), it -> it.isUsing(var));
  }

  @Override
  public boolean isUsingProjVar(UVar var)
  {
    if (!var.is(VarKind.PROJ))
      return false;
    if (this.is(VarKind.PROJ))
      return this.equals(var);
    else if (this.is(VarKind.CONCAT))
      return any(asList(arguments), it -> it.isUsingProjVar(var));
    else
      return false;
  }
  ...
  public UVar replaceVar(UVar baseVar, UVar repVar)
  {
    ...
    if (this.is(VarKind.BASE))
      return this.equals(baseVar) ? repVar.copy() : this.copy();
    if (this.is(VarKind.PROJ) && this.equals(baseVar))
      return repVar.copy();
    if (this.is(VarKind.CONCAT) && this.equals(baseVar))
      return repVar.copy();
  ...
  public boolean equals(Object obj)
  {
    ...
    {
      if (!this.name.equals(that.name()) || this.args().length != that.args().length)
        return false;
  ...
  {
    int size = arguments.length;
    if (kind != that.kind() || !is(VarKind.BASE) && !Objects.equals(name, that.name())
