...
final class UPredImpl implements UPred
{
  ...
  {
    final List<UTerm> replaced =
        transformTerms(arguments, t -> t.replaceVar(baseVar, repVar, freshVar));
  ...
  public boolean replaceVarInplace(UVar baseVar, UVar repVar, boolean freshVar)
  {
    ...
    {
      if (arg.replaceVarInplace(baseVar, repVar, freshVar))
        modified = true;
    }
    return modified;
  }
  ...
  public boolean replaceVarInplaceWOPredicate(UVar baseVar, UVar repVar)
  {
    ...
    for (UTerm arg : arguments)
    {
      if (arg.replaceVarInplaceWOPredicate(baseVar, repVar))
        modified = true;
    }
    return modified;
  }
  ...
  public UTerm replaceAtomicTermExcept(UTerm baseTerm, UTerm repTerm, UTerm exceptTerm)
  {
    ...
    if (this.equals(exceptTerm))
      return this;
    if (this.equals(baseTerm))
      return repTerm.copy();
    final List<UTerm> replaced =
        transformTerms(arguments, t -> t.replaceAtomicTermExcept(baseTerm, repTerm, exceptTerm));
    return UPred.mk(predKind, predName, replaced, nullSafe);
  }
  ...
  public UTerm replaceAtomicTerm(UTerm baseTerm, UTerm repTerm)
  {
    ...
    if (this.equals(baseTerm))
      return repTerm.copy();
    final List<UTerm> replaced =
        transformTerms(arguments, t -> t.replaceAtomicTerm(baseTerm, repTerm));
    return UPred.mk(predKind, predName, replaced, nullSafe);
  }
  ...
  public boolean isPrettyPrintMultiLine()
  {
    ...
    {
      assert arguments.size() == 2;
      return arguments.get(0).isPrettyPrintMultiLine() || arguments.get(1).isPrettyPrintMultiLine();
    }
  }
  ...
  public int hashForSort(Map<String, Integer> varHash)
  {
    ...
    {
      assert arguments.size() == 1;
      return Objects.hash(arguments.get(0).hashForSort(varHash), predKind);
    }
    else
    {
      assert arguments.size() == 2;
      return Objects.hash(arguments.get(0).hashForSort(varHash),
          arguments.get(1).hashForSort(varHash),
          predKind,
          predName);
    }
  }
  ...
  public Set<String> getFVs()
  {
    ...
    if (!isUnaryPred())
      fvs.addAll(arguments.get(1).getFVs());
    return fvs;
  }
  ...
  public boolean groupSimilarVariables(UTerm that, SetMatching<String> matching)
  {
    ...
    {
      if (arguments.size() != pred.arguments.size() || predKind != pred.predKind
      ...
      else
      {
        ...
        if ((pred.isPredKind(PredKind.EQ) || pred.isPredKind(PredKind.NEQ)) && hash1 == hash2)
        {
          // symmetric predicate
  ...
  public boolean equals(Object obj)
  {
    ...
    if (predKind != that.predKind() || !predName.equals(that.predName()))
      return false;
  ...
  public int hashCode()
  {
    return predKind.hashCode() * 31 * 31 + predName.hashCode() * 31
        + new HashSet<>(arguments).hashCode();
  }
    ...
    switch (predKind)
    {
      ...
      case FUNC:
      {
        if (predName.equals(UName.NAME_IS_NULL))
        {
          ...
          if (expr == null)
            return -1;
          //          Printer.output.println("constraint: " + notNulls);
              ...
              if (arg.name().toString().equals(column.tableName() + "." + column.name()))
              {
                if (arg.args().length == 1)
                {
                  ...
                  else
                  {
                    ...
                    Pattern pattern1 = Pattern.compile("\\[" + first_matcher + "\s"
                        + "="
                        + "\s" + second_matcher + "\\]");
                    Pattern pattern2 = Pattern.compile("\\[" + second_matcher + "\s"
                        + "="
                        + "\s" + first_matcher + "\\]");
                      ...
                      while (matcher.find())
                      {
                        if (!Objects.equals(arg.args()[0].toString(), matcher.group())
                            && expr.toString().contains(
                                column.tableName() + "(" + matcher.group() + ")"))
                          return 0;
                      }
                    }
                    while (matcher2.find())
