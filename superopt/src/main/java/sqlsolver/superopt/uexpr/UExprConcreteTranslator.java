...
public class UExprConcreteTranslator
{
      ...
      Map<UVar, List<Value>> scalarFVSchema)
  {
    static Context mk()
    {
      return new Context(new HashSet<>(), new HashMap<>(), new HashMap<>(), new HashMap<>());
    }
     ...
     */
    public ScalarTerm addScalarTerm(ScalarTerm st,
        Supplier<UVar> varSupplier,
        Supplier<List<Value>> varSchemaSupplier,
        Map<UVar, List<Value>> varSchemas)
    {
  ...
  UExprConcreteTranslationResult translate()
  {
    ...
    final QueryTranslator translator0 =
        new QueryTranslator(p0, false, NameSequence.mkIndexed(VAR_NAME_PREFIX, 0));
    final QueryTranslator translator1 =
        new QueryTranslator(p1, true, NameSequence.mkIndexed(VAR_NAME_PREFIX, 0));
  ...
  public class QueryTranslator
  {
    ...
    private final Set<ScalarTerm>
        scalarTerms; // terms representing scalar queries / source of SINGLE_VALUE
    ...
    private static boolean hasNullInArithmeticExpression(RexNode node)
    {
      ...
      Function<SqlKind, Boolean> isUnaryArithmetic = (SqlKind kind)
          -> kind == SqlKind.NOT || kind == SqlKind.PLUS_PREFIX || kind == SqlKind.MINUS_PREFIX
          || kind == SqlKind.IS_TRUE || kind == SqlKind.IS_FALSE || kind == SqlKind.IS_NOT_TRUE
          || kind == SqlKind.IS_NOT_FALSE || kind == SqlKind.IS_UNKNOWN;

      Function<SqlKind, Boolean> isBinaryArithmetic = (SqlKind kind)
          -> kind == SqlKind.EQUALS || kind == SqlKind.NOT_EQUALS || kind == SqlKind.LESS_THAN
          || kind == SqlKind.GREATER_THAN || kind == SqlKind.GREATER_THAN_OR_EQUAL
          || kind == SqlKind.LESS_THAN_OR_EQUAL || kind == SqlKind.MINUS || kind == SqlKind.PLUS
          || kind == SqlKind.MOD || kind == SqlKind.DIVIDE || kind == SqlKind.TIMES;
    ...
    public void putTupleVarSchema(UVar var, List<Value> varSchema)
    {
      ...
      if (!isTargetSide)
        result.setSrcTupleVarSchema(var, varSchema);
      else
        result.setTgtTupleVarSchema(var, varSchema);
    }
    ...
    public void deleteTupleVarSchemaByIndexs(UVar var, List<Integer> indexs)
    {
      ...
      if (!isTargetSide)
        result.setSrcTupleVarSchema(var, newVarSchema);
      else
        result.setTgtTupleVarSchema(var, newVarSchema);
    }
    ...
    public UVar mkProjVar(Value value, UVar var)
    {
      ...
      if (var.is(UVar.VarKind.BASE))
        return UVar.mkProj(projFullName, var);
      else if (var.is(UVar.VarKind.PROJ))
        return mkProjVar(value, var.args()[0]);
      // concat var
      for (UVar argVar : var.args())
      {
        ...
        if (getTupleVarSchema(argVar).contains(valueCol))
        {
          ...
          final UName projTrueName = UName.mk(getIndexStringByInfo(
              valueCol.qualification(), valueCol.name(), getTupleVarSchema(argVar)));
    ...
    private boolean usingUString(UTerm expr) {
      ...
      private Set<TupleInstance> searchTupleInstances(UTerm expr, UVar baseVar)
      {
        ...
        if (expr.kind() != UKind.MULTIPLY && expr.kind() != UKind.PRED)
          return tupleInstances;

        assert baseVar.is(UVar.VarKind.BASE);
        final List<Value> tupleSchema = getTupleVarSchema(baseVar);
        final List<UTerm> predicates = expr.kind() == UKind.MULTIPLY
            ? expr.subTermsOfKind(UKind.PRED)
            : new ArrayList<>(List.of(expr));
      ...
      private UTerm propagateNullVar(UTerm expr, UVarTerm nullVarTerm)
      {
        ...
        if (varIsNullPred(pred) && getIsNullPredVar(pred).equals(nullVarTerm.var()))
          return UConst.one();
        if (pred.isPredKind(UPred.PredKind.EQ))
        {
          final UTerm arg0 = pred.args().get(0), arg1 = pred.args().get(1);
          if (!arg0.equals(nullVarTerm) && !arg1.equals(nullVarTerm))
            return expr;
          final UTerm targetVarTerm = arg0.equals(nullVarTerm) ? arg0 : arg1;
          final UTerm otherTerm = targetVarTerm == arg0 ? arg1 : arg0;
          if (otherTerm.kind() == UKind.CONST)
            return UConst.zero();
          else if (otherTerm.kind() == UKind.VAR)
            return mkIsNullPred((UVarTerm) otherTerm);
        }
      ...
      private void extractColRef(RexNode node, List<RexNode> results)
      {
        ...
        {
          if (rexCall.getOperator().getKind().name().equalsIgnoreCase("CASE"))
            return;
      ...
      private UTerm mkProjEqCond(
          List<Value> outputs, List<RexNode> projList, UVar outVar, UVar inVar)
      {
        ...
        for (int i = 0, bound = outputs.size(); i < bound; ++i)
        {
          ...
          if (projTargetTerm == null)
            return null;
          // handle scalar query, e.g. SELECT (SELECT EMP0.DEPTNO FROM EMP AS EMP0 WHERE EMP0.EMPNO
          // < 20) AS D FROM EMP AS EMP
          if (projTargetTerm.getSubQueryOutVar() != null)
          {
            final UVar subQueryProjVar =
                mkProjVar(getValueByIndex(0), projTargetTerm.getSubQueryOutVar());
            projTargetTerm.replaceVar(subQueryProjVar, outProjVar);
            assert (projTargetTerm.toPredUTerm().kind() == UKind.SUMMATION);
            // here we assert that scalar query is a simple summation like \sum{t1} (R(t1) * p(t1) *
            // [t = a(t1)]) fullSum is \sum{t1} (R(t1) * p(t1) * [t = a(t1)]), while smallSum is
            // \sum{t1} (R(t1) * p(t1))
            final USum fullSum = (USum) projTargetTerm.toPredUTerm();
            final USum smallSum = USum.mk(fullSum.boundedVars(),
                remakeTerm(fullSum.body(),
                    filter(fullSum.body().subTerms(), t -> !t.isUsingProjVar(outProjVar))));
            final UTerm term1 = UMul.mk(
                USquash.mk(fullSum), UPred.mkBinary(UPred.PredKind.EQ, smallSum, UConst.one()));
          ...
          final UTerm freeProjTerm =
              ComposedUTerm.doComparatorOp(outProjVarTerm, projTargetTerm, SqlKind.EQUALS, true);
      ...
      private UTerm[] getScalarQueryTerm(UTerm child, UVar scalarVisibleVar)
      {
        ...
        final USum bigSum = USum.mk(new HashSet<>(Set.of(scalarVisibleVar)),
            UMul.mk(child, UPred.mkBinary(UPred.PredKind.EQ, projNewVar, projScalarVar, true)));
        final UTerm[] result = {
            UAdd.mk(UMul.mk(UPred.mkBinary(
                                UPred.PredKind.EQ, replaceAllBoundedVars(littleSum), UConst.one()),
                        USquash.mk(bigSum)),
                UMul.mk(mkIsNullPred(projNewVar), UNeg.mk(replaceAllBoundedVars(littleSum)))),
            UVarTerm.mk(newVar)};
        return result;
      }
       ...
       */
      private UTerm mkAggOutVarEqCond(List<Value> outputs,
          ...
          boolean isNullResult)
      {
        ...
        for (int i = 0, bound = outputs.size(); i < bound; ++i)
        {
          ...
          List<Value> aggValueRefs =
              getValueListByIndexes(aggExpr.getArgList(), getTupleVarSchema(inVar));
          boolean isCountStar = aggExpr.getArgList().size() == 0;
          if (isCountStar)
            aggValueRefs =
                getValueListBySize(getTupleVarSchema(inVar).size()); // Special case for `count(*)`
          ...
          case MAX, MIN -> {
            ...
            final UPred.PredKind predKind = aggFunc == SqlKind.MAX ? UPred.PredKind.GT :
              UPred.PredKind.LT;
              final UTerm notSumPred = UPred.mkBinary(predKind, maxminProjVar, outProjVar, true);
              final UTerm notSum = USum.mk(UVar.getBaseVars(inVar),
                  UMul.mk(groupByTerm, notSumPred, mkNotNullPred(maxminProjVar)));
              final UTerm squashSumPred =
                  UPred.mkBinary(UPred.PredKind.EQ, maxminProjVar, outProjVar, true);
              final UTerm squashSum = UMul.mk(
                  USum.mk(UVar.getBaseVars(inVar), UMul.mk(groupByTerm.copy(), squashSumPred)),
                  mkNotNullPred(outProjVar));
              aggTerm = UMul.mk(UNeg.mk(notSum), USquash.mk(squashSum));
              if (isNullResult)
                aggTerm = mkIsNullPred(UVarTerm.mk(outProjVar));
          }
          case VAR_POP -> {
    ...
    private UTerm mkInSubEqCond(List<ComposedUTerm> lhsExprTerms, UVar rhsVar) {
      ...
      return eqs.size() == 1 ? eqs.get(0) :
        ...
        private boolean isOrIn(RexCall call)
        {
          ...
          if (any(call.getOperands(),
                  o
                  -> !(o instanceof RexCall rexCall) || rexCall.getOperands().size() != 2
                      || rexCall.getOperator().getKind() != SqlKind.EQUALS))
            return false;
          ...
          for (final RexNode operand : call.getOperands())
          {
            ...
            if (firstArgument == null)
              firstArgument = op0;
            if (!Objects.equals(firstArgument, op0))
              return false;
            ...
            case INPUT_REF
              ...
              switch (type.getSqlTypeName()) {
                ...
                -> {
                  ...
                  return Boolean.TRUE.equals(value) ? UConst.one():
              UConst.zero();
          }
            case NULL -> {
              return UConst.zero();
        ...
        case IS_NULL, IS_NOT_NULL -> {
          ...
          if (call.getOperands().get(0).getKind() != SqlKind.CASE) {
            ...
            for (final RexNode inputRef : inputRefs)
              ...
              final UTerm result = ComposedUTerm.doComparatorOp(
                  lhs, ComposedUTerm.mk(UConst.nullVal()), SqlKind.EQUALS, true);
              switch (exprKind)
              {
            case IS_NULL -> {
        ...
        case AND, OR -> {
          ...
          if (exprKind == SqlKind.OR
            ...
            for (final RexNode operand : call.getOperands())
              ...
              all(secondArguments,
                  literal -> arguments.add(mkValue(literal, baseVar).toPredUTerm()));
              String funcName = PredefinedFunctions.instantiateFamilyFunc(
                  PredefinedFunctions.NAME_IN_LIST, arguments.size());
            ...
            case OR -> {
              ...
              default
                  -> throw new IllegalArgumentException(
                      "[Exception] Unsupported binary operator: " + exprKind);
        }
      }
        case EQUALS, NOT_EQUALS, LESS_THAN, GREATER_THAN,
          ...
          if (call.getOperands().get(0).getKind() == SqlKind.MINUS)
          {
            ...
            if (lhsLhs0 == null || lhsRhs0 == null || rhs0 == null)
              return null;
            lhs = lhsLhs0;
          ...
          else if (call.getOperands().get(1).getKind() == SqlKind.MINUS)
          {
            ...
            if (rhsLhs0 == null || rhsRhs0 == null || lhs0 == null)
              return null;
              ...
              || call.getOperands().get(1).getKind() == SqlKind.SCALAR_QUERY)
          {
            ...
            {
              final UTerm[] results =
                  getScalarQueryTerm(lhs.toPredUTerm(), lhs.getSubQueryOutVar());
              final UTerm scalarTerm = results[0];
              final UVar newVar = ((UVarTerm) results[1]).var();
              lhs = ComposedUTerm.mk(scalarTerm);
              return USquash.mk(USum.mk(new HashSet<>(Set.of(newVar)),
                  UMul.mk(
                      lhs.toPredUTerm(), ComposedUTerm.doComparatorOp(lhs, rhs, exprKind, false))));
            }
            if (call.getOperands().get(1).getKind() == SqlKind.SCALAR_QUERY
                && call.getOperands().get(0).getKind() != SqlKind.SCALAR_QUERY)
            {
              final UTerm[] results =
                  getScalarQueryTerm(rhs.toPredUTerm(), rhs.getSubQueryOutVar());
              final UTerm scalarTerm = results[0];
              final UVar newVar = ((UVarTerm) results[1]).var();
              rhs = ComposedUTerm.mk(scalarTerm);
              return USquash.mk(USum.mk(new HashSet<>(Set.of(newVar)),
                  UMul.mk(
                      rhs.toPredUTerm(), ComposedUTerm.doComparatorOp(lhs, rhs, exprKind, false))));
            }
            // for both scalar subquery case
            final UTerm[] leftResults =
                getScalarQueryTerm(lhs.toPredUTerm(), lhs.getSubQueryOutVar());
            final UTerm[] rightResults =
                getScalarQueryTerm(rhs.toPredUTerm(), rhs.getSubQueryOutVar());
          ...
          if (!inputRefTerms.isEmpty())
          {
            return UMul.mk(ComposedUTerm.doComparatorOp(lhs, rhs, exprKind, false),
                mkNotNullPred(inputRefTerms));
          }
    ...
    case LIKE
        -> {
      // LIKE operator case
      final RexCall call = (RexCall) exprCtx; assert call.getOperands().size() == 2;
      ...
      if (rhsComposedUTerm.toPredUTerm() instanceof UString string) {
        String regex = "^[0-9a-zA-Z]+$"; Pattern pattern = Pattern.compile(regex);
        Matcher matcher = pattern.matcher(string.value());
        if (matcher.matches()) {
          return UPred.mkBinary(UPred.PredKind.EQ,
              lhsComposedUTerm.toPredUTerm(),
              rhsComposedUTerm.toPredUTerm(),
              false);
        } else {
          final UName funcName = UName.mk(PredefinedFunctions.NAME_LIKE);
          final UFunc func = UFunc.mk(UFunc.FuncKind.NON_INT,
              funcName,
              new ArrayList<>(
                  List.of(lhsComposedUTerm.toPredUTerm(), rhsComposedUTerm.toPredUTerm())));
      ...
      for (int x = 0; x < whenConds.size(); x++) {
        ...
        for (int y = 0; y < x; y++) preCondList.add(UNeg.mk(whenConds.get(y).copy()));
        preCondList.add(whenConds.get(x).copy());
        final UTerm preCond = preCondList.size() == 1 ? preCondList.get(0):
      UMul.mk(preCondList);
  ...
  final UTerm elsePreCond =
      elsePreCondList.size() == 1 ? elsePreCondList.get(0) : UMul.mk(elsePreCondList);
      ...
      switch (nodeKind) {
        ...
        case AND, OR -> {
          ...
          for (final RexNode operand : call.getOperands())
            ...
            case EQUALS, NOT_EQUALS, LESS_THAN, GREATER_THAN, GREATER_THAN_OR_EQUAL,
                LESS_THAN_OR_EQUAL
                -> {
                    // Binary comparison case
                    final RexCall call = (RexCall) node; assert call.getOperands().size() == 2;
                    final ComposedUTerm lhs = mkValue(call.getOperands().get(0), baseVar);
                    final ComposedUTerm rhs = mkValue(call.getOperands().get(1), baseVar);
                    if (lhs == null || rhs == null) return null;
                    return ComposedUTerm.mk(
                        ComposedUTerm.doComparatorOp(lhs, rhs, nodeKind, false));} case MINUS,
                PLUS, MOD, DIVIDE,
                TIMES
                -> {
                    ...
                    final ComposedUTerm rhs = mkValue(call.getOperands().get(1), baseVar);
                    if (lhs == null || rhs == null) return null;
                    return ComposedUTerm.doArithmeticOp(lhs, rhs, nodeKind);} case NOT,
                PLUS_PREFIX, MINUS_PREFIX, IS_TRUE, IS_FALSE, IS_NOT_TRUE, IS_NOT_FALSE, IS_NULL,
                IS_NOT_NULL,
                IS_UNKNOWN
                -> {
                    // Unary case
                    final RexCall call = (RexCall) node; assert call.getOperands().size() == 1;
                    final ComposedUTerm lhs = mkValue(call.getOperands().get(0), baseVar);
                    if (lhs == null) return null;
                    return ComposedUTerm.doUnaryOp(lhs, nodeKind);} case CASE
                -> {
                    // CASE operator case
                    final List<UTerm> whenConds = new ArrayList<>();
                    ...
                    // construct whenConds and thenExprs
                    int i = 0;
                    for (; i < call.getOperands().size() - 1; i++){if (i % 2 == 0){
                        // when conditions
                        final UTerm whenCond = mkPredicate0(call.getOperands().get(i), baseVar);
                        if (whenCond == null) return null;
                        whenConds.add(whenCond);} else {
                        // then exprs
        ...
        case OTHER_FUNCTION, OTHER, TRIM, ROW -> {
          ...
          return ComposedUTerm.mk(
              ComposedUTerm.mkFuncCall(UFunc.FuncKind.NON_INT, funcName, arguments));
          }
          }
          ...
          private UTerm trValues(RelNode node)
          {
            ...
            zip(projValueList,
                values.getRowType().getFieldList(),
                (value,
                    typeField) -> value.setType(typeField.getType().getSqlTypeName().getName()));
            ...
            for (ImmutableList<RexLiteral> tuple : literals)
            {
              ...
              for (int i = 0, bound = tuple.size(); i < bound; ++i)
              {
                ...
                if (value.getValue() == null)
                  mulFactors.add(mkIsNullPred(UVarTerm.mk(projVar)));
                else
                {
                  switch (value.getTypeName())
                  {
                    case CHAR
                      ...
                      valueTerm = UConst.mk(value.getValueAs(BigDecimal.class).intValue());
                        ...
                        -> {
                      valueTerm = value.getValueAs(Boolean.class) ? UConst.one():
                      UConst.zero();
                  }
                  default ->
                  {
                ...
                mulFactors.add(
                    UPred.mkBinary(UPred.PredKind.EQ, UVarTerm.mk(projVar), valueTerm, true));
              }
            }
            addFactors.add(UMul.mk(mulFactors));
          }

          if (addFactors.size() == 0)
            return UConst.zero();
          else
            return addFactors.size() == 1 ? addFactors.get(0) : UAdd.mk(addFactors);
          }
          ...
          private UTerm trProj(RelNode relNode)
          {
            ...
            final List<Value> outputValues =
                getValueListByIndexExpr(proj.getProjects(), getTupleVarSchema(visibleVar));
            ...
            final UTerm eqCond =
                mkProjEqCond(projValueList, proj.getProjects(), outVar, visibleVar);
            if (eqCond == null)
              return null;

            // Case 1. Common summation introduced by Proj, since input expr is not from VALUES
            // table when visible var is not baseVar, use this case
            ...
            // Case 2. Input expr is from VALUES table: build a template to expand summation with
            // finite tuples
            ...
            final Set<TupleInstance> tupleInstances =
                searchTupleInstances(normalizedPredecessor, visibleVar);
            ...
            for (TupleInstance tupleInstance : tupleInstances)
            {
              ...
              for (var pair : zip(getTupleVarSchema(visibleVar), tupleInstance.values()))
              {
                ...
                if (val != null)
                  instantiation = instantiation.replaceAtomicTerm(projVarTerm, UConst.mk(val));
                else
                  instantiation =
                      propagateNullVar(instantiation, projVarTerm); // Null value of tuple
              }
            ...
            if (instantiatedTerms.size() == 0)
            {
              ...
              res = instantiatedTerms.size() == 1 ? instantiatedTerms.get(0)
                                                  : UAdd.mk(instantiatedTerms);
          ...
          private UTerm trJoin(RelNode relNode)
          {
            ...
            if (joinCondTerm == null)
              return null;

            final UTerm innerJoinBody = UMul.mk(lhs, rhs, joinCondTerm);
            if (joinKind == JoinRelType.INNER)
              return innerJoinBody;

            // Left Join
            if (joinKind == JoinRelType.LEFT)
            {
              UTerm newSum =
                  USum.mk(UVar.getBaseVars(rhsVisibleVar), UMul.mk(rhs, joinCondTerm).copy());
              ...
              if (checkRelNodeScalar(relNode.getInput(1)))
              {
                ...
                // the term is like [1 = \sum{t1}E] * || \sum{t1}(E * [t = a(t1)] || + IsNull(t) *
                // not(\sum{t1}E) the targetSum here is getting the \sum{t1}E in [1 = \sum{t1}E]
                final UPred targetPred =
                    (UPred) filter(rhs.subTerms().get(0).subTerms().get(0).subTerms(),
                        t
                        -> t instanceof UPred pred && pred.isPredKind(UPred.PredKind.EQ)
                            && any(pred.args(), a -> a.equals(UConst.one())))
                        .get(0);
                final USum targetSum =
                    (USum) filter(targetPred.args(), a -> a.kind() == UKind.SUMMATION).get(0);
                final UMul lJoinBody = UMul.mk(lhs.copy(),
                    mkIsNullPredForAllAttrs(rhsVisibleVar),
                    UNeg.mk(newSum),
                    UPred.mkBinary(UPred.PredKind.LE, targetSum, UConst.one()));
                return UAdd.mk(innerJoinBody, lJoinBody);
              }
              final UMul lJoinBody =
                  UMul.mk(lhs.copy(), mkIsNullPredForAllAttrs(rhsVisibleVar), UNeg.mk(newSum));
              return UAdd.mk(innerJoinBody, lJoinBody);
            }
            // Right Join
            ...
            {
              UTerm newSum =
                  USum.mk(UVar.getBaseVars(lhsVisibleVar), UMul.mk(lhs, joinCondTerm).copy());
              newSum = replaceAllBoundedVars(newSum);
              ...
              {
                final UPred targetPred =
                    (UPred) filter(lhs.subTerms().get(0).subTerms().get(0).subTerms(),
                        t
                        -> t instanceof UPred pred && pred.isPredKind(UPred.PredKind.EQ)
                            && any(pred.args(), a -> a.equals(UConst.one())))
                        .get(0);
              ...
              final UMul rJoinBody =
                  UMul.mk(rhs.copy(), mkIsNullPredForAllAttrs(lhsVisibleVar), UNeg.mk(newSum));
              return UAdd.mk(innerJoinBody, rJoinBody);
            }
            // Full Join
            if (joinKind == JoinRelType.FULL)
            {
              ...
              final UTerm lJoinBody =
                  UMul.mk(lhs.copy(), mkIsNullPredForAllAttrs(rhsVisibleVar), UNeg.mk(newSumLJoin));

              UTerm newSumRJoin =
                  USum.mk(UVar.getBaseVars(lhsVisibleVar), UMul.mk(lhs, joinCondTerm).copy());
              newSumRJoin = replaceAllBoundedVars(newSumRJoin);
              final UTerm rJoinBody =
                  UMul.mk(rhs.copy(), mkIsNullPredForAllAttrs(lhsVisibleVar), UNeg.mk(newSumRJoin));
          ...
          private UTerm trSetOp(RelNode relNode)
          {
            ...
            assert lhsVisibleVar.kind() == rhsVisibleVar.kind()
                && lhsVisibleVar.args().length
                    == rhsVisibleVar.args().length
                : "different visible var types of SetOp's children";

            final List<Value> lhsSchema =
                getValueListBySize(getTupleVarSchema(lhsVisibleVar).size());
            final List<Value> rhsSchema =
                getValueListBySize(getTupleVarSchema(rhsVisibleVar).size());
            assert lhsSchema.size()
                == rhsSchema.size() : "Schemas of SetOp's children are not aligned";
            ...
            {
              if (!intersect.all)
                return USquash.mk(UMul.mk(lhs, rhs));
              else
                return UAdd.mk(
                    UMul.mk(lhs, USquash.mk(rhs)), UMul.mk(USquash.mk(lhs.copy()), rhs.copy()));
            }
           ...
           * SELECT S.a1, S.a3 FROM (SELECT a AS a1, b AS a2, AGG(expr) AS a3 FROM T GROUP BY a, b)
           * AS S</code>
           */
          private UTerm trAgg(RelNode relNode, RelNode father)
          {
            ...
            final List<Value> outputValues = getValueListByAgg(
                agg.getGroupSet().asList(), agg.getAggCallList(), getTupleVarSchema(visibleVar));
            final List<Value> projValueList = getValueListBySize(outputValues.size());
            ...
            final List<Value> aggregateOutputs =
                projValueList.subList(agg.getGroupSet().asList().size(), projValueList.size());
            final List<RexNode> commonProjExprs =
                getRexInputRefByIndexs(agg.getGroupSet().asList());
            final List<Value> commonProjOutputs =
                projValueList.subList(0, agg.getGroupSet().asList().size());
            ...
            final UTerm commonProjBody = commonProjExprs.isEmpty()
                ? predecessor
                : UMul.mk(predecessor,
                      mkProjEqCond(commonProjOutputs, commonProjExprs, outBaseVar, visibleVar));
            if (!commonProjExprs.isEmpty())
            {
              final UTerm commonProjTerm =
                  USquash.mk(USum.mk(UVar.getBaseVars(visibleVar), commonProjBody));
            ...
            if (!aggregateExprs.isEmpty())
            {
              ...
              final UTerm aggregateTerm = mkAggOutVarEqCond(aggregateOutputs,
                  aggregateExprs,
                  outBaseVar,
                  visibleVar,
                  commonProjBody.copy(),
          ...
          private UTerm normalizeWithIntegrityConstraints(
              UTerm expr, Map<Integer, VarSchema> constToTuple)
          {
          ...
          private void checkType(UTerm expr)
          {
            if (expr instanceof UPred pred && pred.isPredKind(UPred.PredKind.EQ))
            {
              ...
              if (vt1 != null && isIntegerExpr(rhs) && !isVarFromTable(vt1.var().args()[0]))
              {
                updateValueType(vt1.var(), Value.TYPE_INT);
              }
              else if (vt2 != null && isIntegerExpr(lhs) && !isVarFromTable(vt2.var().args()[0]))
              {
          ...
          private UVarTerm isSingleVarTermSet(Set<UVarTerm> set)
          {
            ...
            if (vt.var().kind() == UVar.VarKind.PROJ)
              return vt;
            return null;
          }
  ...
  static class ComposedUTerm {
      ...
      for (var pair0 : pred0.preCondAndValues)
            {
              for (var pair1 : pred1.preCondAndValues)
              {
                ...
                if (preCond0.equals(UConst.ONE))
                  combinedPreCond = preCond1.copy();
                else if (preCond1.equals(UConst.ONE))
                  combinedPreCond = preCond0.copy();
                else
                  combinedPreCond = UMul.mk(preCond0.copy(), preCond1.copy());
    ...
    static Pair<UTerm, UTerm> convertDivisionToProduct(UTerm value0, UTerm value1) {
      ...
      final UTerm left = divisor1 == null ? dividend0 :
              UMul.mk(dividend0, divisor1);
              final UTerm right = divisor0 == null ? dividend1 : UMul.mk(dividend1, divisor0);
              return Pair.of(left, right);
                }

                static UTerm doComparatorOp(ComposedUTerm pred0,
                    ComposedUTerm pred1,
                    SqlKind opKind,
                    boolean isMkProjEqCond)
                {
                  ...
                  for (var pair0 : pred0.preCondAndValues)
                  {
                    for (var pair1 : pred1.preCondAndValues)
                    {
                      ...
                      // handle division
                      if (SqlKind.EQUALS.equals(opKind) && !value0.equals(UConst.NULL)
                      ...
                      if (isMkProjEqCond)
                      {
                        ...
                        if (value0.equals(UConst.NULL))
                          compareTerm = mkIsNullPred(value1.copy());
                        else if (value1.equals(UConst.NULL))
                          compareTerm = mkIsNullPred(value0.copy());
                        else
                          compareTerm =
                              UPred.mkBinary(UPred.PredKind.EQ, value0.copy(), value1.copy(), true);
                      }
                      else
                      {
                        // But `mkInSubEqCond` uses code here, since attrs of In sub-query should be
                        // not NULL
                        if (value0.equals(UConst.NULL) || value1.equals(UConst.NULL))
                          compareTerm = UConst.zero();
                        else
                        {
                          final UPred.PredKind uPredOp =
                              QueryTranslator.castBinaryOp2UPredOp(opKind);
                          final UTerm comp =
                              UPred.mkBinary(uPredOp, value0.copy(), value1.copy(), false);
                          if (uPredOp == UPred.PredKind.EQ && value0.kind() == UKind.VAR
                              && value1.kind() == UKind.VAR)
                            compareTerm = UMul.mk(comp, mkNotNullPred(value0.copy()));
                          else
                            compareTerm = comp;
                        }
                      }
                      subTerms.add(combinedPreCond.equals(UConst.ONE)
                              ? compareTerm
                              : UMul.mk(combinedPreCond, compareTerm));
                    }
                  }
                  return UAdd.mk(subTerms);
                }

                static ComposedUTerm doLogicOp(
                    ComposedUTerm pred0, ComposedUTerm pred1, SqlKind opKind)
                {
              ...
              return UFunc.mk(funcKind, funcName, funcArguments);
                    ...
                    static class VALUESTable
                    {
                      ...
                      public boolean equals(Object o)
                      {
                        ...
                        return Objects.equals(tupleMultiSet, valuesTable.tupleMultiSet)
                            && Objects.equals(schema, valuesTable.schema);
                      }
                      ...
                      static VALUESTable parse(String str, List<String> defaultSchema)
                      {
                          ...
                          for (String vs : valueStrs)
                          {
                            ...
                            if (!Character.isDigit(vs.charAt(0)))
                            {
                              if ("NULL".equals(vs))
                                ...
                                values.add(vs.substring(1, vs.length() - 1).hashCode());
                            }
                            else
                        ...
                        if (tuples.size() > 0)
                        {
                          ...
                          for (int i = 0; i < schemaLength; ++i) schema.add("expr$" + i);
                        }
                        return new VALUESTable(tuples, coalesce(defaultSchema, schema));
                      }
                    }
                    ...
                    class VALUESTableParser
                    {
                      ...
                      private String registerVALUESTable(String sql)
                      {
                        ...
                        while (sql.contains("(VALUES"))
                        {
                          ...
                          final String VALUESInfo =
                              sql.substring(start, idx0 + 1); // `(VALUES  (30, 3))`
                          // For case of `(VALUES) as t(col, ..)` -> r0 as t, and pick schema info
                          // in t(col, ..)
                          idx0++;
                          List<String> schema = null;
                          if (idx0 < sql.length()
                              && sql.substring(idx0).trim().toUpperCase().startsWith("AS"))
                          {
                            ...
                            while (idx1 < sql.length() && sql.charAt(idx1) == ' ') idx1++;
                            if (idx1 < sql.length() && sql.charAt(idx1) == '(')
                            {
                              ...
                              schema = Arrays
                                           .stream(schemaStr.substring(1, schemaStr.length() - 1)
                                                       .split(", "))
                                           .toList();
