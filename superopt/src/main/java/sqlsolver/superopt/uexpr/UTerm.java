...
public interface UTerm extends Copyable<UTerm>
{
  ...
  static boolean returnsNatural(UTerm term)
  {
    ...
    if (kind == UKind.SQUASH || kind == UKind.NEGATION || kind == UKind.PRED || kind == UKind.TABLE)
      return true;
   ...
   * This function assumes that comm and assoc items in mult/add/eq/neq have been sorted in their
   * structure hash order (i.e. <code>hashForSort</code>), and bound variables do not share the same
   * name.
