...
class UExprTranslator
{
  ...
  private final boolean enableDependentSubquery, enableSchemaFeasibilityCheck,
      enableIntegrityConstraintRewrite, enableConcretePlanFeature;
  ...
  class TemplateTranslator
  {
    ...
    private UVar mkProj(Symbol attrSym, AttrsDesc desc, UVar base)
    {
      ...
      if (isTargetSide)
      {
        ...
        if (source == null)
          attrSym = rule.constraints().instantiationOf(attrSym);
      }
      assert attrSym != null;
      ...
      // Judge whether attrSym equals groupByAttrs or aggOutAttrs, and perform projection on concat
      // var
      if (source.ctx().ownerOf(source).kind() == AGG)
      {
        ...
        final UVar ret =
            UVar.mkProj(desc.name(), comp.is(UVar.VarKind.PROJ) ? comp.args()[0] : comp);
      ...
      if (source == null)
        ...
        comp = base.is(UVar.VarKind.CONCAT) ? base.args()[index] : base;
    ...
    private SchemaDesc mkSchema(Symbol /* Table or Schema or Attrs */ sym, int componentNum)
    {
      ...
      if (isTargetSide)
        ret = result.schemaOf(rule.constraints().instantiationOf(sym));
      else
      {
    ...
    private UTerm trSetOp(SetOp setOp) {
      ...
      final UVar secondaryVar = (primaryVar == lhsVisibleVar) ? rhsVisibleVar :
          lhsVisibleVar;
          if (primaryVar.is(UVar.VarKind.BASE))
          {
            if (primaryVar == lhsVisibleVar)
              rhs.replaceVarInplace(secondaryVar, primaryVar, false);
            else
              lhs.replaceVarInplace(secondaryVar, primaryVar, false);
          ...
          { // PROJ or CONCAT var
            ...
            if (!isTargetSide)
            {
              ...
              outVar = pivotVars.get(result.schemaOf(primaryVar));
          ...
          switch (setOp.kind())
          {
            case UNION: // `lhs + rhs`, `||lhs + rhs||` if deduplicated
              ...
              if (setOp.deduplicated())
                union = USquash.mk(union);
              return union;
      ...
      {
        if (var0.kind() != var1.kind() || var0.args().length != var1.args().length)
          return false;

        if (var0.is(UVar.VarKind.BASE))
          return true;
        else if (var0.is(UVar.VarKind.PROJ) || var0.is(UVar.VarKind.CONCAT))
        {
          for (int i = 0, bound = var0.args().length; i < bound; ++i)
          {
            if (!varKindAligned(var0.args()[i], var1.args()[i]))
              return false;
          }
          return true;
        }
      ...
      private UTerm trAgg(Agg agg)
      {
        ...
        // Q(concat(t_grp, t_agg)) ∶= ||\sum{t1} (E(t1) * [t_grp = a0(t1)]|| * AggUExpr(t_agg, a1,
        // count, E) * [p(a2(t))] E(t1) = predecessor(t1) * [a0(t)  = a0(t1)]
        final UTerm predecessor = tr(agg.predecessors()[0]);
        if (predecessor == null)
          return null;
        if (!checkSchemaFeasible(agg))
          return null;
        ...
        final UVar groupByProjVar =
            mkProj(agg.groupByAttrs(), groupByAttrDesc, viableVar); // a0(t1)
        ...
        if (!isTargetSide)
        {
          ...
          groupByOutVar = pivotVars.get(subSchemaDesc0);
        ...
        if (!isTargetSide)
        {
          ...
          aggOutVar = pivotVars.get(subSchemaDesc1);
        ...
        final UTerm commonExpr =
            USquash.mk(USum.mk(UVar.getBaseVars(freeVar), predecessorWithPred));
          ...
          case SUM, AVERAGE,
              ...
              -> {
                if (agg.aggFuncKind() == COUNT && agg.deduplicated()) {
                  ...
                  final UTerm innerSumEq =
                      UPred.mkBinary(UPred.PredKind.EQ, newOuterVar, aggProjVar);
                  final UTerm innerSumNotNull = mkNotNullPred(aggProjVar);
                  final UTerm innerSumBody =
                      UMul.mk(predecessorWithPred2, innerSumEq, innerSumNotNull);
                  UTerm innerSum = USum.mk(UVar.getBaseVars(freeVar), innerSumBody);
                  innerSum = replaceAllBoundedVars(innerSum);
                  final UTerm outerSum =
                      USum.mk(new HashSet<>(Set.of(newOuterVar)), USquash.mk(innerSum));
                  ...
                  final UTerm sumBody = UMul.mk(
                      predecessorWithPred2.copy(), notNullCond.copy(), UVarTerm.mk(aggProjVar));
                  UTerm sumSum = USum.mk(UVar.getBaseVars(freeVar), sumBody);
                  sumSum = replaceAllBoundedVars(sumSum);
                  switch (agg.aggFuncKind()) {
                    case SUM -> aggExpr = UPred.mkBinary(UPred.PredKind.EQ, projOutVarTerm, sumSum);
                    case AVERAGE
                    -> aggExpr = UPred.mkBinary(
                           UPred.PredKind.EQ, UMul.mk(projOutVarTerm, countSum), sumSum);
                    case COUNT
              ...
              -> {
                ...
                final UTerm squashSumPred =
                    UPred.mkBinary(UPred.PredKind.EQ, aggProjVar, aggOutVar);
                UTerm squashSum = USum.mk(
                    UVar.getBaseVars(freeVar), UMul.mk(predecessorWithPred2, squashSumPred));
                ...
                final UPred.PredKind predKind = agg.aggFuncKind() == MAX ? UPred.PredKind.GT:
            UPred.PredKind.LT;
            final UTerm notSumPred = UPred.mkBinary(predKind, aggProjVar, aggOutVar);
            UTerm notSum =
                USum.mk(UVar.getBaseVars(freeVar), UMul.mk(predecessorWithPred2_, notSumPred));
      ...
      if (enableConcretePlanFeature)
      {
        ...
        if (expr.equals(
                Expression.EXPRESSION_TRUE)) // Some concrete query doesn't have `HAVING` clause
        ...
        // Agg result in `HAVING` pred should not be null, no need to add not([IsNull]) after the
        // predicate
      }
      else
      {
    ...
    private UTerm trProj(Proj proj)
    {
      ...
      if (!isClosed)
        ...
        outVar = mkFreshVar(outSchema);
      else
        outVar = pivotVars.get(outSchema);
      ...
      if (isClosed)
      {
        ...
        if (proj.deduplicated() && !isImplicitDeduplicated(proj) && !isEffectiveDeduplicated(proj))
        {
          return USquash.mk(s);
        }
        else
    ...
    private boolean needNewFreeVar(Proj proj)
    {
      return isOutermostProj(proj) // 1. directly affects the output
          ...
          || (proj.deduplicated() && !isImplicitDeduplicated(proj)
    ...
    private boolean isOutermostProj(Proj proj)
    {
      ...
      while (succ != null)
      {
        ...
        if (succKind == PROJ)
          return false;
        if (succKind.isSubquery() && succ.predecessors()[1] == op)
          return false;
        op = succ;
        succ = succ.successor();
      }
    ...
    private boolean isImplicitDeduplicated(Proj proj)
    {
      ...
      {
        final OpKind succKind = succ.kind();
        if (succKind == PROJ && ((Proj) succ).deduplicated())
          return true;
        if (succKind.isSubquery() && succ.predecessors()[1] == op)
          return true;
        op = succ;
    ...
    private boolean isUniqueCoreAt(Symbol attrs, Op surface)
    {
      ...
      if (kind.isFilter())
        return isUniqueCoreAt(attrs, surface.predecessors()[0]);
      ...
      if (kind.isSetOp())
        return ((Union) surface).deduplicated();
    ...
    private boolean checkSchemaFeasible(Agg tgtAgg)
    {
      ...
      return (
          groupByS0 == groupByS1 || knownEqSchemas.get().contains(Pair.of(groupByS0, groupByS1)));
  ...
  class ICRewriter
  {
    ...
    private UTerm applyNotNull(UTerm expr)
    {
      ...
      if (expr.kind() != MULTIPLY)
        return expr;
      // In `[a0(t0) = a1(t1)] * not([IsNull(a1(t1))])`, `[IsNull(a1(t1))]` can also be rewritten by
      // NotNull(r0, a0) Create an equivalence class for vars in this UMul
    ...
    private UTerm checkNotNullPreds(
        UTerm mulContext, UTerm expr, NaturalCongruence<UVar> varEqClass)
    {
      expr = transformSubTerms(expr, e -> checkNotNullPreds(mulContext, e, varEqClass));
      assert mulContext.kind() == MULTIPLY;

      if (!varIsNullPred(expr))
        return expr;
      ...
      for (UVar eqVar : varEqClass.eqClassOf(predVar))
      {
        ...
        if (!any(mulContext.subTermsOfKind(TABLE), t -> t.isUsing(predBaseVar)))
          continue;
     ...
     * \sum_{t1}(R0(t0) * R1(t1) * [a0(t0) = a1(t1)] * not([IsNull(a0(t0))])) -> R0(t0) *
     * not([IsNull(a0(t0))]))
     */
    ...
    private UTerm applyReference0(UTerm expr, boolean inSetSem)
    {
      ...
      for (Constraint ref : rule.constraints().ofKind(Reference))
      {
        ...
        if (!setSem && !isUniqueKey(rSym1, aSym1))
          continue;
        ...
        for (UTerm subTerm : body.subTermsOfKind(TABLE))
        {
          ...
          if (!summation.isUsingBoundedVar(tblBaseVar))
            continue;
          if (uTableMatchRelSym((UTable) subTerm, rSym1))
            uTables.add((UTable) subTerm);
        }
        if (uTables.isEmpty())
          continue;
        ...
        for (UTable uTable : uTables)
        {
          ...
          for (UTerm subTerm : body.subTermsOfKind(PRED))
          {
            ...
            final UVar projVar0 =
                uVarMatchAttrsSym(pArgs.get(1), aSym1, false) ? pArgs.get(0) : pArgs.get(1);
            final UVar projVar1 =
                uVarMatchAttrsSym(pArgs.get(1), aSym1, false) ? pArgs.get(1) : pArgs.get(0);
            if (!projVar1.isUsing(tgtVar))
              continue;
            ...
            if (!any(varEqClass.eqClassOf(projVar0), this::uVarNotNullByConstraint)
                && !any(varEqClass.eqClassOf(projVar0), v -> uVarNotNullByTerm(v, body)))
              continue;
          ...
          final UVar eqArg1 =
              eqPredArgs.get(1).isUsing(tgtVar) ? eqPredArgs.get(1) : eqPredArgs.get(0); // `a1(t1)`
          final UVar eqArg0 =
              eqPredArgs.get(1).isUsing(tgtVar) ? eqPredArgs.get(0) : eqPredArgs.get(1); // `a0(t0)`
          boolean extraUseTgtVar = false;
          for (UTerm subTerm : body.subTerms())
          {
            if (subTerm == uTable || subTerm == eqPred)
              continue;
          ...
          if (extraUseTgtVar)
            continue;
          // If no extra using of `t1`, then perform rewrite: \sum_{t1}(R1(t1) * [a0(t0) = a1(t1)])
          // -> 1
          isModified = true;
        ...
        for (UTable uTable : uTables)
        {
          ...
          for (UTerm subTerm : body.subTermsOfKind(PRED))
          {
            ...
            if (!uVarMatchAttrsSym(pArgs.get(0), aSym, true)
                && !uVarMatchAttrsSym(pArgs.get(1), aSym, true))
              continue;
            final UVar matchVar = uVarMatchAttrsSym(pArgs.get(0), aSym, true)
                ? pArgs.get(0)
                : pArgs.get(1); // `a(ti)`
            final UVar constVar =
                uVarMatchAttrsSym(pArgs.get(0), aSym, true) ? pArgs.get(1) : pArgs.get(0); // `e`
            if (!matchVar.isUnaryVar())
              continue;
          ...
          final UVar constVar =
              predArgs.get(0).isUsing(targetVar) ? predArgs.get(1) : predArgs.get(0);
          ...
          while (!adjacentSquashVar.isEmpty())
          {
            ...
            while (iter.hasNext())
            {
              ...
              if (subTerm.kind() == PRED)
              {
                ...
                final UVar relatedVar =
                    predVar0.isUsing(squashVar) ? predVar0 : predVar1; // var using targetVar
                final UVar adjacentVar =
                    (relatedVar == predVar0) ? predVar1 : predVar0; // adjacent var
      ...
      for (Constraint unique : rule.constraints().ofKind(Unique))
      {
        ...
        for (int i = 0, bound = eqVars.size(); i < bound; ++i)
        {
          ...
          for (int j = i + 1, bound0 = eqVars.size(); j < bound0; ++j)
          {
            ...
            if (var0.equals(var1) || !varEqClass.isCongruent(var0, var1))
              continue;
            if (!uVarMatchAttrsSym(var0, aSym, true) || !uVarMatchAttrsSym(var1, aSym, true))
              continue;
    ...
    private boolean uVarMatchAttrsSym0(UVar uVar, Symbol aSym, boolean allowSuper)
    {
      ...
      // Same tables have different schema id, so search for every eq tables of `srcTables`
      // (`eqSrcTables`)
      ...
      if (allowSuper)
        return (uVar.is(UVar.VarKind.BASE)
            || (uVar.is(UVar.VarKind.PROJ)
                && any(superAttrs, a -> uVar.name().equals(initiatedNames.get(a)))));
      else
        return uVar.is(UVar.VarKind.PROJ) && uVar.name().equals(initiatedNames.get(aSym));
    }
