...
public class UNormalizationEnhance extends UNormalization
{
  ...
  UTerm removeRedundantBoundedVar(UTerm expr)
  {
    ...
    while (iter.hasNext())
    {
      ...
      if (subTerm.kind() == UKind.PRED)
      {
        ...
        if (!predVar0.is(UVar.VarKind.BASE) || !predVar1.is(UVar.VarKind.BASE))
          continue;
        if (!summation.isUsingBoundedVar(predVar0) && !summation.isUsingBoundedVar(predVar1))
          continue;
        // By default, rmVar is `t1` and repVar is `t0`; But if `t1` isn't bounded var, then
        // exchange.
  ...
  UTerm removeNestedProjOnProjVar(UTerm expr)
  {
    ...
    {
      isModified = true;
      return expr.kind() == UKind.TABLE ? UTable.mk(((UTable) expr).tableName(), newVar)
                                        : UVarTerm.mk(newVar);
    }
  }
  ...
  UTerm removeRedundantProjOnBaseVar(UTerm expr)
  {
    ...
    for (UVar var0 : schemaEqVarClass.keys())
    {
      ...
      {
        if (!var1.is(UVar.VarKind.PROJ) || !var1.args()[0].is(UVar.VarKind.BASE))
          continue; // `var1` -> `a0(x1)`
        final UVar redundantVarModel =
            UVar.mkProj(var1.name(), var0); // `redundantVarModel` -> `a0(x2)`
        if (expr.replaceVarInplace(redundantVarModel, var0, false))
          isModified = true; // perform `a0(x2)` -> `x2`
      }
    }
    return expr;
  }
    ...
    for (UTerm curTerm : mul.subTerms())
    {
      ...
      if (pred.isPredKind(UPred.PredKind.EQ) && UExprSupport.isPredOfVarArg(pred))
      {
        ...
        if (predVar0.is(UVar.VarKind.BASE))
        {
          ...
          {
            if (subTerm.equals(curTerm))
              continue;
            if (subTerm.replaceVarInplace(predVar0, predVar1, false))
              isModified = true;
          }
        }
        else if (predVar1.is(UVar.VarKind.BASE))
        {
          for (UTerm subTerm : mul.subTerms())
          {
            if (subTerm.equals(curTerm))
              continue;
            if (subTerm.replaceVarInplace(predVar1, predVar0, false))
              isModified = true;
          }
        }
      }
  ...
  private UTerm removeUselessBoundedVar(UTerm expr)
  {
    ...
    while (iter.hasNext())
    {
      ...
      if (pred.isPredKind(UPred.PredKind.EQ))
      {
        ...
        if (predArg0.kind() == UKind.VAR && summation.isUsingBoundedVar(((UVarTerm) predArg0).var())
