...
public class ScalarNormalizer extends UNormalization
{
  ...
  {
    // [\sum{x}f(x) <= 1]
    if (term instanceof UPred pred && pred.predKind() == UPred.PredKind.LE
        && pred.args().get(1).equals(UConst.one()) && pred.args().get(0) instanceof USum sum
    ...
    // not(\sum{x}f(x))
    if (term instanceof UNeg neg && neg.body() instanceof USum sum && sum.boundedVars().size() == 1)
    {
      final UVar bv = sum.boundedVars().stream().toList().get(0).copy();
      return ScalarTerm.mk(bv, translator.getTupleVarSchema(bv), sum.body().copy());
    }
