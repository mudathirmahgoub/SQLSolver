...
public class QueryUExprNormalizer extends UNormalization
{
  ...
  public QueryUExprNormalizer(
      UTerm expr, Schema schema, UExprConcreteTranslator.QueryTranslator translator)
  {
  ...
  private static boolean haveTargetKind(UTerm term, UKind kind)
  {
    ...
    for (UTerm subTerm : term.subTerms())
      if (haveTargetKind(subTerm, kind))
        return true;
    return false;
  }
  ...
  {
    final List<Value> schema =
        CalciteSupport.getValueListBySize(translator.getTupleVarSchema(tuple).size());
    if (schema.size() != 1)
      return false;
    final UVar v = vt.var();
    if (v.equals(tuple))
    ...
    return v.kind() == UVar.VarKind.PROJ && v.name().toString().equals(column.name())
        && v.isUsing(tuple);
  }
  ...
  private UTerm isComparisonWith(UTerm term, UVar column, UVar x1, UPred.PredKind op)
  {
    ...
    {
      if (pred.args().get(1) instanceof UVarTerm vt && vt.var().equals(column))
        if (pred.args().get(0).isUsing(x1))
          // [f(x1) op column]
          return pred.args().get(0);
      if ((op == UPred.PredKind.EQ || op == UPred.PredKind.NEQ)
          && pred.args().get(0) instanceof UVarTerm vt && vt.var().equals(column))
  ...
  /**
   * Check whether targetTerm and its children have a critical not null-safe pred that is using
   * projVar.
   */
  private boolean isCriticalNotNullSafePredUsingProjVar(UTerm targetTerm, UVar projVar)
  {
    if (targetTerm.kind() == UKind.PRED)
    {
      ...
      if (!targetPred.nullSafe() && targetPred.isUsingProjVar(projVar))
        return true;
    }

    // only SQUASH, MULTIPLY, SUMMATION maintain critical value.
    if (targetTerm.kind() == UKind.SQUASH || targetTerm.kind() == UKind.MULTIPLY
        || targetTerm.kind() == UKind.SUMMATION)
    {
      for (final UTerm subTerm : targetTerm.subTerms())
      {
        if (isCriticalNotNullSafePredUsingProjVar(subTerm, projVar))
          return true;
      }
    }
  ...
  private boolean isPredNotNullArguments(UPred pred)
  {
    ...
    {
      if (any(pred.args(), a -> a.kind() == UKind.SUMMATION))
        return true;
    }
  ...
  private List<UTerm> extractFromMaxMinSummation(USum sum, UVar column, UPred.PredKind op) {
     ...
     * This function is used by matchMaxMinNotSquash.
     */
    private List<UTerm> extractFromMaxMinSummation(
        USum sum, UVar column, UVar x1, UPred.PredKind op)
    {
      final List<UTerm> result = new ArrayList<>();
      if (!(sum.body() instanceof UMul mul))
        return result;
      ...
      y1.remove(x1);
      if (any(matchX1, t -> t == null || t.isUsingProjVar(column) || any(y1, t::isUsing)))
        return result;
      if (any(e1, t -> t.isUsingProjVar(column) || t.isUsing(x1)))
        return result;
    ...
    {
      List<UTerm> negMatches =
          extractFromMaxMinSummation((USum) neg.body(), column, UPred.PredKind.GT);
      ...
      {
        final List<UTerm> squashMatches =
            extractFromMaxMinSummation(squashSum, column, squashSumVar, UPred.PredKind.EQ);
        if (squashMatches.isEmpty())
          continue;
        // check whether tuple schemas match
        if (!CalciteSupport.isExactlyEqualTwoValueList(translator.getTupleVarSchema(negSumVar),
                translator.getTupleVarSchema(squashSumVar)))
          return null;
        ...
        // r match
        if (!replacer.replace(negMatches.get(0), negSumVar, newX1)
                 .equals(replacer.replace(squashMatches.get(0), squashSumVar, newX1)))
          return null;
        newTable = replacer.replace(negMatches.get(0), negSumVar, newX1);
        // p match
        if (negMatches.get(1) != null && squashMatches.get(1) != null)
        {
          if (!replacer.replace(negMatches.get(1), negSumVar, newX1)
                   .equals(replacer.replace(squashMatches.get(1), squashSumVar, newX1)))
            return null;
          newPred = replacer.replace(negMatches.get(1), negSumVar, newX1);
        }
        // f match
        if (!replacer.replace(negMatches.get(2), negSumVar, newX1)
                 .equals(replacer.replace(squashMatches.get(2), squashSumVar, newX1)))
          return null;
        ...
        final USum resultSum = USum.mk(
            newBoundedVars, newPred != null ? UMul.mk(newTable, newPred) : UMul.mk(newTable));
    ...
    private void simplifyMaxMinInSumByOneColumn(Set<UVar> bvs, List<UTerm> factors, UVar column)
    {
      ...
      final List<UTerm> negations =
          filter(factors, t -> t instanceof UNeg neg && neg.body().kind() == UKind.SUMMATION);
      final List<UTerm> squashes = filter(
          factors, t -> t instanceof USquash squash && squash.body().kind() == UKind.SUMMATION);
      final List<UTerm> unrelatedTerms =
          filter(factors, t -> !negations.contains(t) && !squashes.contains(t));
      // unrelated terms should not use $i(x).
      if (any(unrelatedTerms, t -> t.isUsingProjVar(column)))
        return;
    ...
    private boolean containSubTerms(USum bigSum, USum smallSum)
    {
      ...
      if (bigBoundVars.size() < smallBoundVars.size())
        return false;
      if (bigSum.equals(smallSum))
        return true;
     ...
     */
    private boolean checkSubSum(int cur,
        HashSet<UVar> bigBoundVars,
        UTerm bigBody,
        ArrayList<UVar> smallBoundVars,
        UTerm smallBody)
    {
        ...
        for (UTerm smallTerm : smallBody.subTerms())
        {
          ...
          if (smallTerm.kind() == UKind.PRED)
          {
            ...
            if (pred.isPredKind(UPred.PredKind.EQ))
            {
              ...
              if (congruence.isCongruent(op0, op1))
                continue;
            }
          }
          if (isNotNullPred(smallTerm))
          {
            ...
            if (any(eqTerms, t -> bigBody.subTerms().contains(UNeg.mk(mkIsNullPred(t)))))
              continue;
          }
          return false;
        }
      ...
      {
        if (!isExactlyEqualTwoValueList(
                translator.getTupleVarSchema(v), translator.getTupleVarSchema(newVar)))
          continue;
        ...
        final boolean result =
            checkSubSum(cur + 1, tmpVars, tmpBigBody, smallBoundVars, tmpSmallBody);
        if (result)
          return true;
      }
      return false;
    ...
    private UTerm removeNegatedSumByMaxMin(UTerm term, boolean isUnderSet)
    {
      ...
      // current
      if (!isUnderSetFinal || !(term instanceof USum sumX) || !(sumX.body() instanceof UMul bodyX))
        return term;
      ...
      {
        static final Set<UPred.PredKind> CMP_OPS =
            Set.of(UPred.PredKind.GT, UPred.PredKind.GE, UPred.PredKind.LT, UPred.PredKind.LE);
        static final Set<UPred.PredKind> CMP_OPS_STRICT =
            Set.of(UPred.PredKind.GT, UPred.PredKind.LT);
        static Cmp mk(UPred pred, UVar x)
        {
          // [$i(x) >/>=/</<= c]
          return mk(pred, false, (l, r) -> {
            final String iX = getColName(l, x);
            if (iX != null && !r.isUsing(x))
              return iX;
            return null;
          });
        }
        static Cmp mk(UPred pred, UVar y, UVar x)
        {
          ...
          return mk(pred, true, (l, r) -> {
            ...
            if (iY != null && iY.equals(iX))
              return iY;
            return null;
          });
        }
        static Cmp mk(
            UPred pred, boolean strictPredKind, BiFunction<UTerm, UTerm, String> matchesLeftRight)
        {
          // [matchesLeft >/< matchesRight]
          final UPred.PredKind predKind = pred.predKind();
          if (!CMP_OPS.contains(predKind))
            return null;
          if (strictPredKind && !CMP_OPS_STRICT.contains(predKind))
            return null;
          final UTerm arg0 = pred.args().get(0);
          final UTerm arg1 = pred.args().get(1);
          String colName = matchesLeftRight.apply(arg0, arg1);
          if (colName != null)
            return new Cmp(colName, toGTLT(predKind));
          colName = matchesLeftRight.apply(arg1, arg0);
          if (colName != null)
            return new Cmp(colName, toGTLTInverse(predKind));
          return null;
        }
    ...
    outer:
            for (UVar x : sumX.boundedVars())
            {
              ...
              for (UTerm factor : filter(bodyX.subTerms(), t -> t.isUsing(x)))
              {
                    ...
                    || factor instanceof UConst cst && cst.value() >= 0)
                {
                  // not(sum{y})
                  // or part of f(x)
                  if (factor instanceof UNeg neg && neg.body() instanceof USum sumY
                      && sumY.boundedVars().size() == 1 && sumY.body() instanceof UMul bodyY)
                  {
                    if (y != null)
                      continue outer;
              ...
              if (!contains(fgY, fXRenamed, eqClasses))
                continue;
              // modify here
              // remove not(sum{y})
              isModified = true;
          ...
          private boolean contains(
              List<UTerm> terms1, List<UTerm> terms2, NaturalCongruence<UTerm> congruence)
          {
                    ...
                    && pred2.predKind() == pred1.predKind())
                {
                  ...
                  if (predKind == UPred.PredKind.EQ || predKind == UPred.PredKind.NEQ)
                  {
                    ...
                    if (congruence.isCongruent(arg10, arg20) && congruence.isCongruent(arg11, arg21)
                        || congruence.isCongruent(arg11, arg20)
                            && congruence.isCongruent(arg10, arg21))
          ...
          private UTerm removeSumByContainment(UTerm term, Set<UTerm> ctx)
          {
              ...
              term = transformSubTerms(term, t -> {
                ...
                newCtx.addAll(filter(factors, factor -> {
                  ...
                  if (!(factor instanceof UNeg neg))
                    return false;
                  return neg.body() instanceof USum sum && sum.body() instanceof UMul
            ...
            if (!(term instanceof USum sumY) || !(sumY.body() instanceof UMul bodyY))
              return term;
            ...
            for (UVar y : sumY.boundedVars())
            {
              ...
              for (UTerm candidate : ctx)
              {
                ...
                final Bag<UTerm> factorsX =
                    new Bag<>(map(bodyX.subTerms(), t -> t.replaceVar(x, y, false)));
          ...
          private UTerm reduceRedundantTables(UTerm term)
          {
            ...
            {
              final List<UTerm> tableSubTerms =
                  filter(term.subTerms(), t -> t.kind() == UKind.TABLE);
              final List<UTerm> newTableSubTerms = new ArrayList<>();
              final List<UTerm> notTableSubTerms =
                  filter(term.subTerms(), t -> t.kind() != UKind.TABLE);
              for (final UTerm tableSubTerm : tableSubTerms)
              {
                if (any(newTableSubTerms, t -> t.equals(tableSubTerm)))
                  continue;
           ...
           */
          private UTerm replaceInequalityWithZero(
              UTerm context, NaturalCongruence<UTerm> congruence)
          {
            if (context instanceof UPred pred
                && (pred.isPredKind(UPred.PredKind.GT) || pred.isPredKind(UPred.PredKind.LT)))
            {
  ...
  private UTerm removeNullPredInMulRecursiveTerms(UTerm expr) {
    ...
    for (final UTerm subTerm : subTerms)
        ...
        return UMul.mk(subTerms);
            ...
            /**
             * For a summation \sum{..., x, ...}, if x is determined, then x can be removed and its
             * column can be replaced. determined: For every column i of x, if it exists a term like
             * [$i(x) = f()], where f is not related to x, then x can be removed and $i(x) can be
             * replaced by f(). e.g. \sum{x, y} {[$0(x) = 1] * [$1(x) = $0(y)]} Only consider the
             * summation and its sub-term must be multiplication.
             */
            private UTerm removeDeterminedBoundedVarByConst(UTerm expr)
            {
              ...
              if (expr.kind() != UKind.SUMMATION || expr.subTerms().get(0).kind() != UKind.MULTIPLY)
                return expr;

              final USum summation = (USum) expr;
              UMul multiply = (UMul) expr.subTerms().get(0);
              final NaturalCongruence<UTerm> congruence =
                  getEqIsNullCongruenceInTermsOfMul(multiply, pred -> true);
              final List<UTerm> tableTerms =
                  filter(multiply.subTerms(), t -> t.kind() == UKind.TABLE);

              for (final UVar boundedVar : summation.boundedVars())
              {
                final List<Value> varIndexSchema =
                    getValueListBySize(translator.getTupleVarSchema(boundedVar).size());
                ...
                for (final Value varIndex : varIndexSchema)
                {
                  ...
                  // heuristically pick one term that is equivalent to projVar and is not using
                  // boundedVar
                ...
                if (any(tableTerms, t -> t.isUsing(boundedVar)))
                  continue;

                // if a var can be removed, then the replaceMap size is equal to its schema size.
                if (varIndexSchema.size() != replaceMap.size())
                  continue;
                ...
                for (final Value varIndex : varIndexSchema)
                {
                  final UVar projVar = translator.mkProjVar(varIndex, boundedVar);
                  tempMultiply =
                      tempMultiply.replaceAtomicTerm(UVarTerm.mk(projVar), replaceMap.get(projVar));
            ...
            /**
             * For a summation \sum{..., x, ...}, if x is determined by another var y, then x can be
             * replaced by y. determined by another var: For every column i of x, if it exists a
             * term like [$i(x) = $i(y)], then x can be replaced by y. e.g. \sum{x, y} {[$0(x) =
             * $0(y)] * [$1(x) = $1(y)] * T(x)} -> \sum{y} {[$0(y) = $0(y)] * [$1(y) = $1(y)] *
             * T(y)} Only consider the summation and its sub-term must be multiplication.
             */
            private UTerm removeDeterminedBoundedVarByTuple(UTerm expr)
            {
              ...
              for (final UVar boundedVar : summation.boundedVars())
              {
                final List<Value> varIndexSchema =
                    getValueListBySize(translator.getTupleVarSchema(boundedVar).size());
                final List<UVar> projVars =
                    map(varIndexSchema, v -> translator.mkProjVar(v, boundedVar));
                final List<Set<UTerm>> tupleEqualSet =
                    map(projVars, v -> congruence.eqClassOf(UVarTerm.mk(v)));
                // tupleEqualSet size should be same to varIndexSchema.
                if (tupleEqualSet.size() != varIndexSchema.size())
                  continue;
                ...
                for (final UTerm firstEqualTerm : firstEqualSet)
                {
                  ...
                  if (firstEqualTerm.kind() != UKind.VAR)
                    continue;
                  final UVar firstEqualVar = ((UVarTerm) firstEqualTerm).var();
                  if (firstEqualVar.kind() != UVar.VarKind.PROJ || !firstEqualVar.isUnaryVar())
                    continue;
                  final UVar firstBaseVar = firstEqualVar.args()[0];
                  if (firstBaseVar.equals(boundedVar))
                    continue;
                  if (!isEqualTwoValueList(translator.getTupleVarSchema(firstBaseVar),
            ...
            private UTerm removeDeterminedBoundedColumnByConst(UTerm expr)
            {
              ...
              for (final UVar boundedVar : summation.boundedVars())
              {
                  ...
                  for (final UTerm eqTerm : eqTerms)
                  {
                    if (!eqTerm.isUsingProjVar(projVar))
                    {
                      ...
                      if (!replaceMap.containsKey(projVar))
                        replaceMap.put(projVar, new ArrayList<>());
                ...
                // if a var's columns can be removed, then the replaceMap size should be greater
                // than 0
                if (replaceMap.isEmpty())
                  continue;

                final UVar newBoundedVar = mkFreshBaseVar();
                translator.putTupleVarSchema(
                    newBoundedVar, translator.getTupleVarSchema(boundedVar));
                final Set<UVar> newBoundedVars = summation.boundedVars();
                newBoundedVars.remove(boundedVar);
                ...
                for (final Value varIndex : varIndexSchema)
                {
                  ...
                  if (replaceMap.containsKey(projVar))
                  {
                    ...
                    {
                      if (replaceTerm.isUsingProjVar(projVar))
                        continue;
                      if (replaceTerm.equals(UConst.nullVal()))
                      {
                        final UTerm newMultiply = transformNullTerm(
                            multiply.replaceAtomicTerm(UVarTerm.mk(projVar), replaceTerm));
                        ...
                        {
                          if (key.equals(projVar))
                            continue;
                          replaceMap.put(key,
                              map(replaceMap.get(key),
                                  t
                                  -> transformNullTerm(
                      ...
                      for (final UVar key : replaceMap.keySet())
                      {
                        ...
                        replaceMap.put(key,
                            map(replaceMap.get(key),
                                t -> t.replaceAtomicTerm(UVarTerm.mk(projVar), replaceTerm)));
                      }
                      deleteIndexs.add(index);
                      deleteCount++;
                      find = true;
                  ...
                  final UVar projReplaceVar =
                      translator.mkProjVar(getValueByIndex(index - deleteCount), boundedVar);
                  multiply =
                      multiply.replaceAtomicTerm(UVarTerm.mk(projVar), UVarTerm.mk(projReplaceVar));
                  for (final UVar key : replaceMap.keySet())
                  {
                    if (key.equals(projVar))
                      continue;
                    replaceMap.put(key,
                        map(replaceMap.get(key),
                            t
                            -> t.replaceAtomicTerm(
                                UVarTerm.mk(projVar), UVarTerm.mk(projReplaceVar))));
                  }
                }
            ...
            private UTerm removeUnaryBoundedVarBySameSchema(UTerm expr)
            {
              ...
              switch (body.kind())
              {
                ...
                case ADD:
                {
                  if (any(body.subTerms(), t -> !isTermNaturalNumber(t)))
                    return expr;
            ...
            /**
             * For ||∑{..., v0, v1,...} (f(v0) * u(v1) * z(v0, v1)||, if v0 and v1 has the same
             * schema, and if we replace v1 with v0 in u(v1) * z(v0, v1), the u(v0) * z(v0, v0)
             * becomes f(v0)'s subset, then delete v1's related terms.
             */
            private UTerm removeUnaryBoundedVarBySameSchemaInSum(USum summation)
            {
              ...
              for (final UVar boundedVar : summation.boundedVars())
              {
                final List<UVar> otherBoundedVars =
                    filter(summation.boundedVars(), v -> !v.equals(boundedVar));

                for (final UVar otherBoundedVar : otherBoundedVars)
                {
                  if (!isExactlyEqualTwoValueList(translator.getTupleVarSchema(otherBoundedVar),
                          translator.getTupleVarSchema(boundedVar)))
                    continue;
                  ...
                  {
                    if (subTerm.isUsing(otherBoundedVar))
                      u.add(subTerm.copy());
                    else
                      f.add(subTerm.copy());
                  }
                  ...
                  u.replaceAll(termU
                      -> new QueryUExprNormalizer(
                          termU.replaceVar(otherBoundedVar, boundedVar, true),
                          this.schema,
                          this.translator)
                             .normalizeTerm());
                  // check whether u and z have term that f doesn't have.
                  final NaturalCongruence<UTerm> congruence =
                      getEqCongruenceInTermsOfMul(UMul.mk(f), pred -> true);
                  boolean canModify = true;
                  for (final UTerm target : u)
                  {
                    if (target.equals(UConst.one()))
                      continue;
                    // case like [a = b], use congruence to determine.
                    if (target.kind() == UKind.PRED
                        && ((UPred) target).isPredKind(UPred.PredKind.EQ))
                    {
                      final UTerm eqTerm = ((UPred) target).args().get(0).isUsing(boundedVar)
                          ? ((UPred) target).args().get(1)
                          : ((UPred) target).args().get(0);
                      if (any(congruence.eqClassOf(eqTerm),
                              t -> !t.equals(eqTerm) && t.isUsing(boundedVar)))
                        continue;
                    }
                    ...
                    if (isNotNullPred(target))
                    {
                      ...
                      if (any(congruence.eqClassOf(eqTerm),
                              t -> !t.equals(eqTerm) && f.contains(mkNotNullPred(t))))
                        continue;
                    }
              ...
              for (UTerm subTerm : body.subTerms())
              {
                if (subTerm instanceof USquash)
                {
                  ...
                  if (squashBody instanceof final USum squashSum)
                  {
                    ...
                    final List<UTerm> relatedTerms =
                        filter(squashSum.body().subTerms(), t -> !unrelatedTerms.contains(t));
                    final ArrayList<UTerm> subTerms = new ArrayList<>(body.subTerms());
                    subTerms.remove(subTerm);
                    final USum tmp =
                        USum.mk(new HashSet<>(expr.boundedVars()), UMul.mk(subTerms).copy());
                    ...
                    if (containSubTerms(tmp, smallSum))
                    {
                      ...
                      {
                        return USum.mk(tmp.boundedVars(),
                            UMul.mk(remakeTerm(squashSum.body(), unrelatedTerms), tmp.body()));
                      }
                      return tmp;
                    }
                  }
            ...
            private UTerm removeUselessSetSemanticSumBySetSolver(UTerm term)
            {
              ...
              if (!term.kind().isUnary() || term.subTerms().get(0).kind() != UKind.SUMMATION)
                return term;
              ...
              for (final UTerm subTerm : subTerms)
              {
                ...
                {
                  final USum newSum = USum.mk(sum.boundedVars(),
                      remakeTerm(sum.body(), filter(subTerms, t -> !t.equals(subTerm))));
                  VerificationResult equalOrNot =
                      new SetSolver(remakeTerm(term, new ArrayList<>(List.of(sum))),
            ...
            /**
             * for || \sum{..., t1, t2 ...} (f(t1, t2)) ||, if we replace t2 with t1 and get
             * equivalent term, then replace it.
             * => || \sum{..., t1, ...} (f(t1)) ||
             * NOTE: the schema of t1 and t2 must be the same.
             */
            private UTerm removeUselessBoundedVarInUnarySumBySetSolver(UTerm term)
            {
              ...
              for (int i = 0; i < boundedVars.size(); i++)
              {
                for (int j = 0; j < boundedVars.size(); j++)
                {
                  ...
                  if (!isExactlyEqualTwoValueList(translator.getTupleVarSchema(srcVar),
                          translator.getTupleVarSchema(tgtVar)))
                    continue;
                  ...
                  if (equalOrNot == VerificationResult.EQ)
                  {
                    isModified = true;
                    return remakeTerm(
                        term, new ArrayList<>(List.of(reduceRedundantTables(sumCompared))));
                  }
             ...
             * For ||∑{..., v0, v1,...} (f(v0) * u(v1) * z(v0, v1)||, if v0 and v1 has the same
             * schema, and if we replace v1 with v0 in u(v1) * z(v0, v1), the u(v0) * z(v0, v0)
             * becomes f(v0)'s subset, then replace v1 with v0.
             */
            private UTerm simplifyMulToZeroByContradictCongruence(UTerm expr)
            {
              ...
              final NaturalCongruence<UTerm> congruence =
                  getEqCongruenceInTermsOfMul(multiply, pred -> true);

              for (final UTerm key : congruence.keys())
              {
                final Set<UTerm> eqTerms = congruence.eqClassOf(key);
                final List<UTerm> eqConsts =
                    filter(eqTerms, t -> t.kind() == UKind.CONST || t.kind() == UKind.STRING);
            ...
            private UTerm eliminateNegationTerm(UTerm expr)
            {
              ...
              final List<UTerm> notSubTerms =
                  filter(expr.subTerms(), t -> t.kind() == UKind.NEGATION);
              final List<UTerm> otherSubTerms =
                  filter(expr.subTerms(), t -> t.kind() != UKind.NEGATION);
            ...
            loop:
              for (final UTerm otherSubTerm : otherSubTerms)
              {
                for (final UTerm notSubTerm : notSubTerms)
                {
                  ...
                  final UTerm newTerm =
                      replaceTermRecursive(otherSubTerm, negation.body(), UConst.zero());
            ...
            /**
             * For ∑{v} (f(v)), if f(v) has another summation, use f(v)'s congruence to transform
             * another summation to be unrelated with this summation as much as possible.
             */
            private UTerm transformUnrelatedSummation(UTerm expr)
            {
              ...
              newBody = applyTransformationUnrelatedSummationRecursively(
                  summation.body(), congruences, summation.boundedVars());
              newBody = applyTransformationUnrelatedSummationTerm(newBody, summation.boundedVars());
              newBody =
                  applyTransformationUnrelatedSummationInMul(newBody, summation.boundedVars());
             ...
             */
            private UTerm applyTransformationUnrelatedSummationRecursively(
                UTerm context, List<NaturalCongruence<UTerm>> congruences, Set<UVar> boundedVars)
            {
      ...
      case SUMMATION -> {
        ...
        for (final NaturalCongruence<UTerm> congruence : congruences)
        {
          ...
          /**
           * Recursively transform summation's term based on congruences, trying to make the term
           * unrelated to the summation's boundedVars. e.g. \sum{x} ([x = c] * \sum{y} ([x = y] * [y
           * > C])) -> \sum{x} ([x = c] * \sum{y} ([x = y] * [x > c]))
           */
          private UTerm applyTransformationUnrelatedSummationTerm(
              UTerm context, Set<UVar> boundedVars)
          {
            if (context.kind() != UKind.MULTIPLY)
              return context;
            final NaturalCongruence<UTerm> congruence = getEqCongruenceInTermsOfMul(
                context, pred -> all(pred.args(), v -> v.kind().isTermAtomic()));
              ...
              for (final Value column : schema)
              {
                ...
                for (final UTerm subTerm : multiply.subTerms())
                {
                  ...
                  if (subTerm instanceof UPred pred && pred.isPredKind(UPred.PredKind.EQ))
                    continue;
                  if (!subTerm.isUsingProjVar(projVar))
                    continue;
                  // try to modify
                  UTerm choose = null;
                  for (final UTerm replaceTerm : congruence.eqClassOf(UVarTerm.mk(projVar)))
                  {
                    if (any(boundedVars, replaceTerm::isUsing))
                      continue;
                    choose = replaceTerm;
                    break;
                  }
           ...
           */
          private UTerm applyTransformationUnrelatedSummationInMul(
              UTerm context, Set<UVar> boundedVars)
          {
            if (context.kind() != UKind.MULTIPLY)
              return context;
            ...
            final NaturalCongruence<UTerm> congruence =
                getEqCongruenceInTermsOfMul(context, pred -> true);
            ...
            for (final UVar boundedVar : boundedVars)
            {
              ...
              for (final UTerm subTerm : subTerms)
              {
                if (subTerm instanceof UPred pred && pred.isPredKind(UPred.PredKind.EQ))
                {
                  ...
                  {
                    constEqTerms.addAll(filter(congruence.eqClassOf(firstArgument),
                        t -> all(boundedVars, v -> !t.isUsing(v))));
                  }
                  ...
                  {
                    constEqTerms.addAll(filter(congruence.eqClassOf(secondArgument),
                        t -> all(boundedVars, v -> !t.isUsing(v))));
                  }
                  ...
                  newSubTerms.add(
                      UPred.mkBinary(UPred.PredKind.EQ, firstArgument, constEqTerms.get(0)));
                  newSubTerms.add(
                      UPred.mkBinary(UPred.PredKind.EQ, secondArgument, constEqTerms.get(0)));
                }
                else
                {
                  newSubTerms.add(subTerm);
              ...
              // because every modification will bring the bigger size of newSubTerms, we can judge
              // by size.
              if (newSubTerms.size() == subTerms.size())
                continue;
          ...
          private UTerm extractUnrelatedSummation(UTerm expr)
          {
            ...
            if (expr.kind() != UKind.SUMMATION || ((USum) expr).body().kind() != UKind.MULTIPLY)
              return expr;
          ...
          private UTerm splitUnrelatedSummation(UTerm expr)
          {
            ...
            for (final UTerm subTerm : multiply.subTerms())
            {
              if (subTerm instanceof USquash squash && squash.body().kind() == UKind.MULTIPLY)
              {
                ...
                for (final UVar boundedVar : boundedVars)
                {
                  final List<UTerm> boundedVarRelated =
                      filter(originSquashTerms, t -> t.isUsing(boundedVar));
            ...
            for (final UVar boundedVar : boundedVars)
            {
              ...
              if (any(newSumTerms, t -> ((USum) t).boundedVars().contains(boundedVar)))
                continue;
              ...
              while (continueLooping)
              {
                ...
                {
                  if (newBoundedVars.contains(otherBoundedVar))
                    continue;
            ...
            for (final UTerm subTerm : multiply.subTerms())
            {
              if (all(boundedVars, v -> !subTerm.isUsing(v)))
                unrelatedTerms.add(subTerm);
            }
          ...
          private UTerm extractUnrelatedSummationTerms(UTerm expr)
          {
            ...
            final List<UTerm> unrelatedTerms =
                filter(multiply.subTerms(), t -> all(summation.boundedVars(), v -> !t.isUsing(v)));

            if (!unrelatedTerms.isEmpty())
            {
              isModified = true;
              final List<UTerm> relatedTerms =
                  filter(multiply.subTerms(), t -> !unrelatedTerms.contains(t));
