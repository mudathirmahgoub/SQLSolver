...
public record USumImpl(Set<UVar> boundedVars, UTerm body) implements USum
{
  ...
  public UTerm replaceVar(UVar baseVar, UVar repVar, boolean freshVar)
  {
    ...
    {
      // Replace a var with an existing known var, `repVar` is either an outer var or using existing
      // bounded vars So do not consider adding `repVar` to boundedVars
      final UTerm newBody = body.replaceVar(baseVar, repVar, freshVar);
      final Set<UVar> newBoundedVars = new HashSet<>(boundedVars);
      return USum.mk(newBoundedVars, newBody);
    }
  ...
  public boolean replaceVarInplace(UVar baseVar, UVar repVar, boolean freshVar)
  {
    ...
    {
      // Replace a var with an existing known var, then do not consider adding `repVar` to
      // boundedVars
  ...
  public boolean groupSimilarVariables(UTerm that, SetMatching<String> matching)
  {
    ...
    {
      if (boundedVars.size() != sum.boundedVars().size())
        return false;
  ...
  boolean checkSameSum(
      int cur, List<UVar> boundedVars, UTerm body, HashSet<UVar> thatBoundVars, UTerm thatBody)
  {
    if (cur == boundedVars.size())
  ...
  private static SetMatching<UVar> strings2BoundVars(
      SetMatching<String> matching, Set<String> leftBoundVarNames, Set<String> rightBoundVarNames)
  {
  ...
  private static boolean tryMatchInPair(int depth,
      SetMatching<UVar> matching,
      UTerm t1,
      UTerm t2,
      int depthInPair,
      List<UVar> s1,
      Set<UVar> s2)
  {
    if (depthInPair == s1.size())
      return tryMatch(depth + 1, matching, t1, t2);
  ...
  private static boolean tryMatch(int depth, SetMatching<UVar> matching, UTerm t1, UTerm t2)
  {
    ...
    return tryMatchInPair(depth, matching, t1, t2, 0, new ArrayList<>(pair[0]), pair[1]);
  }
  ...
  private static boolean fastEquals(USum sum1, USum sum2)
  {
    ...
    if (!sum1.body().groupSimilarVariables(sum2.body(), matching))
      return false;
    // enumerate matching
    SetMatching<UVar> varMatching =
        strings2BoundVars(matching, sum1.getBoundVarNames(), sum2.getBoundVarNames());
    if (varMatching == null)
      return false;
  ...
  public boolean equals(Object obj)
  {
    if (this == obj)
      return true;
    if (!(obj instanceof final USum that))
      return false;
    HashSet<UVar> thatBoundVars = new HashSet<>(that.boundedVars());

    if (boundedVars.size() != thatBoundVars.size())
      return false;

    if (boundedVars.size() > 4)
      return fastEquals(this, that);

    UTerm thatBody = that.body().copy();
    if (boundedVars.equals(thatBoundVars) && body.equals(thatBody))
      return true;
