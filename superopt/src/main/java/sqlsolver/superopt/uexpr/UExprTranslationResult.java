...
public class UExprTranslationResult
{
  ...
  public void setVarSchema(UVar var, SchemaDesc... schema)
  {
    ...
    if (schema.length == 1)
      varToSchema.put(var, schema[0]);
    else
      varToSchema.put(var, new SchemaDesc(schema));
  }
  ...
  {
    if (schema.length == 1)
      symToSchema.put(sym, schema[0]);
    else
      symToSchema.put(sym, new SchemaDesc(schema));
  }
  ...
  {
    if (srcOutVar == null || tgtOutVar == null)
      return;
    if (srcOutVar.kind() != tgtOutVar.kind() || srcOutVar.args().length != tgtOutVar.args().length)
      return;
    if (srcOutVar.equals(tgtOutVar))
      return;
    ...
    for (int i = 0, bound = srcOutVar.args().length; i < bound; ++i)
    {
      ...
      if (srcSchemaDesc == null || tgtSchemaDesc == null)
        return;

      // Sym tends to be schema sym, or may be other kind
      final List<Symbol> srcSchemaSyms =
          rule.constraints().sourceSymbols().symbolsOf(Symbol.Kind.SCHEMA);
      final List<Symbol> tgtSchemaSyms =
          rule.constraints().targetSymbols().symbolsOf(Symbol.Kind.SCHEMA);
      ...
      if (srcSchemaSym == null || tgtSchemaSym == null)
        return;
      ...
      if (!rule.constraints().isEq(srcSchemaSym, instantiationSchema))
        return;
    }
