...
public class SubstitutionTranslator
{
  ...
  public SubstitutionTranslatorResult translate()
  {
    ...
    if (srcFragment == null || tgtFragment == null)
      return result;

    if (mkConstraints())
      result.setRule(Substitution.mk(srcFragment, tgtFragment, constraints));
  ...
  {
    if (!mkSymbolEq())
      return fail("Fail to generate eq-type constraints");
    if (!mkAttrsSub())
      return fail("Fail to generate AttrsSub constraints");
    if (!mkIntegrityConstraints())
      return fail("Fail to generate integrity constraints");
    if (!mkInstantiation())
      return fail("Fail to generate instantiation constraints");
    ...
    for (int i = 0; i < bound; ++i)
    {
      for (int j = i + 1; j < bound; ++j)
      {
        if (tableCanEq(tableSyms.get(i), tableSyms.get(j)))
          constraints.add(
              Constraint.mk(Constraint.Kind.TableEq, tableSyms.get(i), tableSyms.get(j)));
      }
    }
  }
    ...
    for (int i = 0; i < bound; ++i)
    {
      for (int j = i + 1; j < bound; ++j)
      {
        if (attrsCanEq(attrsSyms.get(i), attrsSyms.get(j)))
          constraints.add(
              Constraint.mk(Constraint.Kind.AttrsEq, attrsSyms.get(i), attrsSyms.get(j)));
      }
    }
  }
    ...
    for (int i = 0; i < bound; ++i)
    {
      for (int j = i + 1; j < bound; ++j)
      {
        if (predCanEq(predSyms.get(i), predSyms.get(j)))
          constraints.add(
              Constraint.mk(Constraint.Kind.PredicateEq, predSyms.get(i), predSyms.get(j)));
      }
    }
  }
    ...
    for (int i = 0; i < bound; ++i)
    {
      for (int j = i + 1; j < bound; ++j)
      {
        if (schemaCanEq(schemaSyms.get(i), schemaSyms.get(j)))
          constraints.add(
              Constraint.mk(Constraint.Kind.SchemaEq, schemaSyms.get(i), schemaSyms.get(j)));
      }
    }
  }
  ...
  private boolean mkAttrsSub()
  {
    ...
    for (Symbol attrs : srcFragment.symbols().symbolsOf(ATTRS))
    {
      ...
      for (Symbol source : viableSources.get(attrs))
      {
        ...
        if (subAttrs == null || srcAttrs == null)
          return false;
  ...
  private static List<Symbol> analyzeSource(Op op, ListMultimap<Symbol, Symbol> viableSources)
  {
    ...
    final List<Symbol> lhs =
        kind.numPredecessors() > 0 ? analyzeSource(predecessor[0], viableSources) : null;
    final List<Symbol> rhs =
        kind.numPredecessors() > 1 ? analyzeSource(predecessor[1], viableSources) : null;

    switch (kind)
    {
      case INPUT: return Collections.singletonList(((Input) op).table());
      ...
      case CROSS_JOIN: return ListSupport.join(lhs, rhs);
      case SIMPLE_FILTER, IN_SUB_FILTER: viableSources.putAll(((AttrsFilter) op).attrs(), lhs);
      case EXISTS_FILTER: return lhs;
      case PROJ:
        final Proj proj = (Proj) op;
        viableSources.putAll(proj.attrs(), lhs);
        return Collections.singletonList(proj.schema());
      case UNION, INTERSECT, EXCEPT: return ListSupport.join(lhs, rhs);
      case AGG:
      ...
      default: throw new IllegalArgumentException("unsupported op kind " + kind);
    }
  }
  ...
  {
    final List<Constraint> attrsSubConstraints = filter(
        constraints, c -> c.kind() == Constraint.Kind.AttrsSub && c.symbols()[1].kind() == TABLE);
    for (Constraint constraint : attrsSubConstraints)
    {
      ...
      if (columns == null || columns.isEmpty())
        continue; // some attrs has no backed column

      if (none(columns, column -> isParticipateIn(src, column, ConstraintKind.UNIQUE)))
        continue;
  ...
  private void mkNotNull()
  {
    ...
    for (Constraint constraint : attrsSubConstraints)
    {
      ...
      if (none(columns, column -> isParticipateIn(src, column, ConstraintKind.NOT_NULL)))
        continue;
  ...
  private void mkReference()
  {
    ...
    for (Constraint constraint0 : attrsSubConstraints)
    {
      ...
      {
        if (constraint0.symbols()[0] == constraint1.symbols()[0])
          continue;
        ...
        if (referringAttrs.isEmpty() || referringColumns == null || referringColumns.isEmpty())
          continue;
        ...
        if (referredAttrs.isEmpty() || referredCols == null || referredCols.isEmpty())
          continue;

        final var fks = findIC(src.schema(), referringColumns, FOREIGN);
        if (!referringColumns.equals(referredCols)
            && linearFind(fks, it -> it.refColumns().equals(referredCols)) == null)
          continue;
        ...
        if (!isRootRef(src, rootRef))
          rootRef = deRef(src, rootRef);
        assert rootRef != null;
  ...
  private boolean isParticipateIn(
      PlanContext plan, Column column, ConstraintKind integrityConstraint)
  {
  ...
  {
    if (!mkTableInstantiation())
      return false;
    if (!mkSchemaInstantiation())
      return false;
    if (!mkAttrsInstantiation())
      return false; // Put it after SchemaInstantiation for checking validity
    if (!mkPredInstantiation())
      return false;
    if (!mkFuncInstantiation())
      return false;
    //
  ...
  private boolean mkTableInstantiation()
  {
    ...
    for (Symbol tgtSym : tgtTableSyms)
    {
      ...
      {
        if (instantiatedSrcTables.contains(srcSym))
          continue;
  ...
  private boolean mkSchemaInstantiation()
  {
    ...
    for (Symbol tgtSym : tgtSchemaSyms)
    {
      ...
      if (preferredInstantiation != null
          && !instantiatedSrcSchemas.contains(preferredInstantiation))
      {
      ...
      {
        if (instantiatedSrcSchemas.contains(srcSym))
          continue;
      ...
      // If no instantiation found, instantiate Proj_r's schema's with the same instantiation of
      // Proj_l
      final Op owner = tgtSym.ctx().ownerOf(tgtSym), parent = owner.successor();
      if (parent != null && parent.kind() == OpKind.UNION)
      {
        final Op another =
            owner == parent.predecessors()[0] ? parent.predecessors()[1] : parent.predecessors()[0];
        if (owner.kind() == another.kind())
        {
          ...
          {
            constraints.add(
                Constraint.mk(Constraint.Kind.SchemaEq, tgtSym, currAnotherInstantiation));
            continue;
          }
        }
      }
  ...
  private boolean mkAttrsInstantiation()
  {
    ...
    for (Symbol tgtSym : tgtAttrsSyms)
    {
      ...
      {
        if (!validateAttrsInstantiation(srcSym, tgtSym))
          continue;
      ...
      if (hasInstantiation || result.getSpecialProjAttrs(tgtSym) != null)
        continue;
      // Special case for Union: Union(Proj_l, Proj_r).
      // If no attrs instantiation found, instantiate Proj_r's attrs with the same instantiation of
      // Proj_l's attrs
      final Op owner = tgtSym.ctx().ownerOf(tgtSym), parent = owner.successor();
      if (parent != null && parent.kind() == OpKind.UNION)
      {
        final Op another =
            owner == parent.predecessors()[0] ? parent.predecessors()[1] : parent.predecessors()[0];
        if (owner.kind() == another.kind()
            && (owner.kind() == OpKind.PROJ || owner.kind() == OpKind.AGG))
        {
          final Symbol anotherAttrs;
          if (another.kind() == OpKind.PROJ)
            anotherAttrs = ((Proj) another).attrs();
          else
          {
            if (tgtSym == ((Agg) owner).groupByAttrs())
              anotherAttrs = ((Agg) another).groupByAttrs();
            else if (tgtSym == ((Agg) owner).aggregateAttrs())
              anotherAttrs = ((Agg) another).aggregateAttrs();
            else
              anotherAttrs = ((Agg) another).aggregateOutputAttrs();
          }
          if (attrsCanEq(tgtSym, anotherAttrs) && currentInstantiationOf(anotherAttrs) != null)
          {
            constraints.add(Constraint.mk(
                Constraint.Kind.AttrsEq, tgtSym, currentInstantiationOf(anotherAttrs)));
            continue;
          }
        }
      }
  ...
  private boolean mkPredInstantiation()
  {
    ...
    for (Symbol tgtSym : tgtPredSyms)
    {
      ...
      if (!hasInstantiation && !isSimpleIntArithmeticExpr(ofPred(tgtSym, true)))
        return false;
    }
    return true;
  }
  ...
  private static boolean isAttrsEq(
      PlanContext plan0, List<Value> attrs0, PlanContext plan1, List<Value> attrs1)
  {
    if (attrs0.size() != attrs1.size())
      return false;
    // `plan0` may be same or different with `plan1`
    for (int i = 0, bound = attrs0.size(); i < bound; i++)
    {
      if (!valueEq(plan0, attrs0.get(i), plan1, attrs1.get(i)))
        return false;
    }
    return true;
  }

  private static boolean isAttrsContained(
      PlanContext plan0, List<Value> attrs0, PlanContext plan1, List<Value> attrs1)
  {
  ...
  private static boolean isAttrsIntersect(
      PlanContext plan0, List<Value> attrs0, PlanContext plan1, List<Value> attrs1)
  {
  ...
  private Symbol preferredSchemaInstantiation(Symbol to)
  {
    ...
    for (Symbol from : srcFragment.symbols().symbolsOf(SCHEMA))
    {
      ...
      if (!schemaCanEq(from, to))
        continue;
      if (toOwner.kind() != fromOwner.kind())
        continue;
      if (toOwner.kind() == OpKind.PROJ)
      {
        // Precondition 3. corresponding `attrs` on Proj should be eq
        final Symbol toOwnerAttrs = ((Proj) toOwner).attrs(),
                     fromOwnerAttrs = ((Proj) fromOwner).attrs();
        if (!attrsCanEq(fromOwnerAttrs, toOwnerAttrs))
          continue;

        final Symbol toSource = directTableSource(to), fromSource = directTableSource(from);
        if (toSource != null && fromSource != null
            && currentInstantiationOf(toSource) == fromSource)
          return from;
      }
    }
    return null;
  ...
  private Symbol directTableSource(Symbol sym)
  {
    ...
    while (true)
    {
      ...
      if (sourceOwner.kind() != OpKind.PROJ)
        return null;
  ...
  private boolean validateAttrsInstantiation(Symbol from, Symbol to)
  {
    ...
    for (Symbol sourceOfTo : viableSources.get(to))
    {
      ...
      if (sourceChain.contains(sourceInstantiation))
        return true;
    }
  ...
  {
    assert sym.kind() == Symbol.Kind.ATTRS || sym.kind() == Symbol.Kind.SCHEMA
        || sym.kind() == Symbol.Kind.TABLE;
  ...
  private Symbol currentSourceOf(Symbol sym)
  {
    ...
    final List<Constraint> sourceConstraints =
        filter(constraints, c -> c.kind() == Constraint.Kind.AttrsSub);
  ...
  private class FragmentConstructor
  {
    ...
    private Op buildTree(int nodeId)
    {
      ...
      if (plan.kindOf(nodeId).numChildren() > 0 && child0 == null)
        return null;
      if (plan.kindOf(nodeId).numChildren() > 1 && child1 == null)
        return null;
    ...
    private boolean trAgg(int nodeId, Op op) {
      ...
      final Agg agg = (Agg) op;
      final Expression havingPredExpr0 = havingPredExpr == null ? Expression.EXPRESSION_TRUE :
          havingPredExpr;
      ...
      private void collectAggregates(PlanContext ctx,
          AggNode aggNode,
      ...
      private boolean isClosedAggregates(
          AggNode aggNode, List<Value> aggRefs, List<Value> groupRefs)
      {
        final ValuesRegistry valuesReg = plan.valuesReg();
        for (Expression attrExpr : aggNode.attrExprs())
        {
          ...
          if (!aggRefs.containsAll(refs) && !groupRefs.containsAll(refs))
            return false;
        }
        return true;
      }
      ...
      private void assign(Symbol sym, Object assignment)
      {
        ...
        if (isTargetSide)
          tgtAssignments.get().put(sym, assignment);
        else
          srcAssignments.get().put(sym, assignment);
      }
