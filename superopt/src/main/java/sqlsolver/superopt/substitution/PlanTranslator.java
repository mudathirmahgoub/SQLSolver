...
class PlanTranslator
{
  ...
  private AttrsDesc attrsDescOf(Symbol attrsSym)
  {
    ...
    if (desc != null)
      return desc;
    if (attrsSym.ctx() != srcSyms)
      attrsSym = constraints.instantiationOf(attrsSym);
  ...
  {
    for (Symbol table : srcSyms.symbolsOf(Symbol.Kind.TABLE))
      sourceDescs.put(table, assignSourceDesc(table));
    for (Symbol attrs : srcSyms.symbolsOf(Symbol.Kind.ATTRS))
      attrsDescs.put(attrs, assignAttrsDesc(attrs));
    for (Symbol pred : srcSyms.symbolsOf(Symbol.Kind.PRED))
      predDescs.put(pred, assignPredDesc(pred));
  ...
  private Schema mkSchema()
  {
    ...
    for (Constraint notNull : constraints.ofKind(NotNull))
    {
      ...
      if (!initiatedConstraints.add(attr.colName))
        continue;
    ...
    for (Constraint uniqueKey : constraints.ofKind(Unique))
    {
      ...
      if (!initiatedConstraints.add(attrs.colName))
        continue;
    ...
    for (Constraint foreignKey : constraints.ofKind(Reference))
    {
      ...
      if (!initiatedConstraints.add(attrs.colName + refAttrs.colName))
        continue;
  ...
  private class PlanConstructor
  {
    ...
    private int trTree(Op op)
    {
      switch (op.kind())
      {
        ...
        case INNER_JOIN, LEFT_JOIN: return trJoin((Join) op);
    ...
    private int trAgg(Agg agg)
    {
      ...
      if (aggRefAst == null || groupRefAst == null)
        return NO_SUCH_NODE;
      ...
      final String aggFuncName =
          (agg.aggFuncKind() == AggFuncKind.UNKNOWN) ? "count" : agg.aggFuncKind().text(); // TODO
