...
public class NormalizationTest
{
  ...
  void testRemoveBoolConstant()
  {
    ...
    {
      final SqlNode sql =
          TestHelper.parseSql("select a from t where 1+1 = 2 and (c or 1 between 0 and 2)");
      Clean.clean(sql);
      assertEquals("SELECT `a` FROM `t` WHERE `c`", sql.toString());
    }
    {
      final SqlNode sql = TestHelper.parseSql(
  ...
  {
    final SqlNode sql =
        TestHelper.parseSql(("select a from t where a like concat('%', concat('1', '%'))"));
    Clean.clean(sql);
  ...
  {
    final SqlNode sql = TestHelper.parseSql("select * from a join b on a.i = b.x and a.j=3 join c "
                                            + "on b.y=c.v and b.z=c.w and c.u<10 where a.j=b.y");
    NormalizeJoinCond.normalize(sql);
    assertEquals(
        "SELECT * FROM `a` INNER JOIN `b` ON `a`.`i` = `b`.`x` AND `a`.`j` = `b`.`y` INNER JOIN "
        + "`c` ON `b`.`y` = `c`.`v` AND `b`.`z` = `c`.`w` WHERE `a`.`j` = 3 AND `c`.`u` < 10",
        sql.toString());
  }
  ...
  {
    final SqlNode sql = TestHelper.parseSql("select sub.j from a inner join (select 1 as j) as sub "
                                            + "on a.i = sub.j where sub.j = 1 order by sub.j");
  ...
  {
    final SqlNode sql = TestHelper.parseSql("Select a.i, a.j From a Group By a.i, a.j, 3, 2 + 3 "
                                            + "HAVING a.i > 10 AND (count(a.i) > 1 OR a.j < 5)");
    NormalizeGrouping.normalize(sql);
    final SqlContext ctx = sql.context();
    ctx.deleteDetached(ctx.root());
    ctx.compact();
    assertEquals("SELECT DISTINCT `a`.`i`, `a`.`j` FROM `a` WHERE `a`.`i` > 10 HAVING "
                 + "COUNT(`a`.`i`) > 1 OR `a`.`j` < 5",
        sql.toString());
  }
