...
public class Rewriter
{
  ...
  public SqlNode getSqlNode(String sql)
  ...
  {
    return sqlNode.getKind() == SqlKind.COUNT || sqlNode.getKind() == SqlKind.MAX
        || sqlNode.getKind() == SqlKind.MIN || sqlNode.getKind() == SqlKind.SUM
        || sqlNode.getKind() == SqlKind.AVG;
  }

  public static boolean isArithmetical(SqlNode sqlNode)
  {
    return sqlNode.getKind() == SqlKind.PLUS || sqlNode.getKind() == SqlKind.MINUS
        || sqlNode.getKind() == SqlKind.TIMES || sqlNode.getKind() == SqlKind.DIVIDE;
  }
  ...
  private SqlNode processSelectHaveFrom(SqlNode node, Map<String, String> aliasMap) throws Exception {
    ...
    for(final SqlNode selectNode : selectList)
        {
          if (selectNode.getKind() == SqlKind.AS)
          {
            ...
            AliasSelectList.add(asOperator.createCall(
                newPos, firstOperand, new SqlIdentifier(secondOperand.toString(), newPos)));
          }
          else
          {
    ...
    private SqlNode processSelectHave(SqlNode node) throws Exception
    {
      ...
      if (node.getKind() == SqlKind.SELECT)
      {
        ...
        final SqlSelect newSqlSelect = new SqlSelect(newPos,
            null,
            originSelectList,
            asOperator.createCall(newPos, newFromSqlSelect, newAs),
            newWhere,
            null,
            null,
            null,
        ...
        if (selectNode.getKind() == SqlKind.AS)
        {
          ...
          if (isArithmetical(firstOperand))
          {
            ...
            SqlNode arithNew = new SqlBasicCall(arithmeticalCall.getOperator(),
                new SqlNode[] {aliasFirstOperand, aliasSecondOperand},
                newPos);
    ...
    private SqlNode processAggregate(SqlNode node) throws Exception
    {
      ...
      if (node.getKind() == SqlKind.SELECT)
      {
        ...
        final SqlSelect newSqlSelect = new SqlSelect(newPos,
            null,
            columnList,
            asOperator.createCall(newPos, newSelectFrom, newAs),
            null,
            null,
            null,
            null,
    ...
    private SqlNode processWithAs(SqlNode node) throws Exception
    {
      ...
      final SqlNode targetNode = new SqlBasicCall(
          new SqlAsOperator(), new SqlNode[] {asNode.query, asNode.name}, SqlParserPos.ZERO);
