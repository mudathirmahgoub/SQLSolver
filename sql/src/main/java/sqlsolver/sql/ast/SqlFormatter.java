...
public class SqlFormatter implements SqlVisitor
{
  ...
  public boolean enterColumnDef(SqlNode colDef)
  {
    ...
    if (colDef.isFlag(ColDef_Cons, ConstraintKind.UNIQUE))
      append(" UNIQUE");
    if (colDef.isFlag(ColDef_Cons, ConstraintKind.PRIMARY))
      append(" PRIMARY KEY");
    if (colDef.isFlag(ColDef_Cons, ConstraintKind.NOT_NULL))
      append(" NOT NULL");
    if (colDef.isFlag(ColDef_AutoInc))
      append(" AUTO_INCREMENT");

    final var references = colDef.$(ColDef_Ref);
    if (references != null)
      safeVisit(references);
    ...
    if (schemaName == null && "overlay".equalsIgnoreCase(funcName)) {
      ...
      try (final var ignored = withParen(true)) {
        if (args.size() < 3) append(UNKNOWN_PLACEHOLDER);
          ...
          final SqlNode arg1 = args.get(1);
          final SqlNode arg2 = args.get(2);
          final SqlNode arg3 = args.size() > 3 ? args.get(3) :
          null;
          safeVisit(arg0);
          append(" PLACING ");
          safeVisit(arg1);
...
if (schemaName == null && args.isEmpty() && PostgreSQL.equals(funcCall.dbType()))
{
  ...
  if (upperCase.startsWith("CURRENT") || "SESSION_USER".equals(upperCase)
      || "USER".equals(upperCase) || "LOCALTIME".equals(upperCase)
...
public boolean enterCollation(SqlNode collation)
{
  ...
  {
    append('\'')
        .append(collation.$(ExprFields.Collate_Collation).$(ExprFields.Symbol_Text))
        .append('\'');
  }
  return false;
}
...
{
  if (tuple.isFlag(ExprFields.Tuple_AsRow))
    append("ROW");
...
public boolean enterMatch(SqlNode match)
{
  ...
  if (option != null)
    append(' ').append(option.optionText());
  append(')');
...
public boolean enterAggregate(SqlNode aggregate)
{
  ...
  if (aggregate.isFlag(ExprFields.Aggregate_Distinct))
    append("DISTINCT ");
  ...
  if (sep != null)
    append(" SEPARATOR '").append(sep).append('\'');
...
public boolean enterWindowFrame(SqlNode windowFrame)
{
  ...
  if (exclusion != null)
    append(" EXCLUDE ").append(exclusion.text());
...
{
  if (frameBound.$(FrameBound_Direction) == null)
    append("CURRENT ROW");
  else
  {
...
public boolean enterWindowSpec(SqlNode windowSpec)
{
  ...
  if (name != null)
    appendName(windowSpec, name, false);
...
public boolean enterBinary(SqlNode binary)
{
  ...
  if (subqueryOption != null)
    append(subqueryOption).append(' ');

  final SqlNode right = binary.$(ExprFields.Binary_Right);

  final boolean needParen = op == BinaryOpKind.MEMBER_OF || ExprKind.Array.isInstance(right)
    ...
    if (names != null && !names.isEmpty())
    {
      for (String name : names)
      {
        if (name.equalsIgnoreCase("primary"))
          ...
          appendName(indexHint, name, false);
        append(", ");
      }
...
{
  if (derivedTableSource.isFlag(Derived_Lateral))
    append("LATERAL ");
  try (final var ignored = withParen(true))
  {
    increaseIndent();
...
{
  private static final SqlFormatter.DumbAutoCloseable INSTANCE =
      new SqlFormatter.DumbAutoCloseable();
