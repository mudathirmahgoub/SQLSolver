...
public interface TableSourceFields
{
  ...
  FieldKey<List<String>> Simple_Partition =
      TableSourceKind.SimpleSource.field("Partitions", List.class);
  ...
  FieldKey<List<String>> Derived_InternalRefs =
      TableSourceKind.DerivedSource.field("InternalRefs", List.class);
  ...
  static String tableSourceNameOf(SqlNode node)
  {
    ...
    else if (TableSourceKind.DerivedSource.isInstance(node))
      return node.$(Derived_Alias);
  ...
  {
    return !TableSourceKind.SimpleSource.isInstance(node)
        ? null
        : node.$(Simple_Table).$(SqlNodeFields.TableName_Table);
  }
}
