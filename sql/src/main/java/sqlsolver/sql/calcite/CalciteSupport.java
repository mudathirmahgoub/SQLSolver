...
public abstract class CalciteSupport
{
   ...
   */
  public static final JavaTypeFactory JAVA_TYPE_FACTORY =
   ...
   */
  public static final NameSequence TEMP_TABLE_NAME_SEQUENCE =
      NameSequence.mkIndexed(TEMP_TABLE_PREFIX, 0);
   ...
   */
  public static final NameSequence TEMP_COL_NAME_SEQUENCE =
      NameSequence.mkIndexed(TEMP_COL_PREFIX, 0);
  ...
  public static boolean supportsAggFunc(SqlKind kind)
  {
    return kind == COUNT || kind == SUM || kind == AVG || kind == MAX || kind == MIN
        || kind == STDDEV_SAMP || kind == STDDEV_POP || kind == VAR_SAMP || kind == VAR_POP
        || kind == SINGLE_VALUE;
  }
  ...
  public static List<Value> getValueListByIndexExpr(List<RexNode> expr, List<Value> originList)
  {
    ...
    for (RexNode index : expr)
    {
      ...
      if (index instanceof RexInputRef inputRef
          && originList.get(inputRef.getIndex()).qualification() != null)
      {
      ...
      else
      {
        ...
        int id = ++nextId;
        final Value value = Value.mk(id,
            null,
            indexToColumnName(curIndex),
            index.getType().getSqlTypeName().getName(),
            isNotNullExpr(index, originList));
        result.add(value);
      }
      curIndex++;
    }
  ...
  public static boolean isNotNullExpr(RexNode expr, List<Value> refList)
  {
    switch (expr.getKind())
    {
      ...
      case CASE -> {
        ...
        for (RexNode op : ops)
        {
          if (opIndex % 2 == 1 && !isNotNullExpr(op, refList))
            return false;
          opIndex++;
        }
...
public static List<Value> getValueListByIndexes(List<Integer> indexes, List<Value> schema)
{
  ...
  for (Integer index : indexes)
  {
    ...
    final Value value =
        new ValueImpl(id, null, indexToColumnName(index), refValue.type(), refValue.isNotNull());
    result.add(value);
  }
  return result;
}
 ...
 */
public static List<Value> getValueListByAgg(
    List<Integer> groupIndex, List<AggregateCall> aggCallList, List<Value> originList)
{
...
public static String getIndexStringByInfo(String qualification, String name, List<Value> valueList)
{
  for (int i = 0; i < valueList.size(); i++)
  {
    ...
    if (Objects.equals(value.qualification(), qualification) && Objects.equals(value.name(), name))
    {
...
public static boolean isExactlyEqualTwoValueList(List<Value> valueList0, List<Value> valueList1)
{
  ...
  for (int i = 0; i < valueList0.size(); i++)
  {
    ...
    if (!Objects.equals(value0.qualification(), value1.qualification())
        ...
        || !Objects.equals(value0.type(), value1.type()))
      return false;
  }
  return true;
}
...
public static List<Value> mergeTwoValueLists(List<Value> valueList0, List<Value> valueList1)
{
  ...
  for (int i = 0; i < size; i++)
  {
    ...
    else
    {
      ...
      // Printer.output.println(type0 + ", " + type1);
      return null;
    }
  }
  return newValueList;
...
public static Planner getPlanner(CalciteSchema schema)
{
  ...
  Frameworks.ConfigBuilder builder =
      Frameworks.newConfigBuilder()
          ...
          .parserConfig(SqlParser.config().withCaseSensitive(false).withParserFactory(
              SqlBabelParserImpl.FACTORY))
          .operatorTable(SqlOperatorTables.chain(
              SqlLibraryOperatorTableFactory.INSTANCE.getOperatorTable(
                  SqlLibrary.BIG_QUERY, SqlLibrary.SPARK, SqlLibrary.MYSQL, SqlLibrary.STANDARD),
              SqlOperatorTables.of(USER_DEFINED_FUNCTIONS)))
  ...
  Frameworks.ConfigBuilder builder =
      Frameworks.newConfigBuilder()
          .parserConfig(SqlParser.config().withCaseSensitive(false).withParserFactory(
              SqlBabelParserImpl.FACTORY))
          .operatorTable(SqlOperatorTables.chain(
              getCalciteOperators(), SqlOperatorTables.of(USER_DEFINED_FUNCTIONS)))
          .sqlValidatorConfig(SqlValidator.Config.DEFAULT.withTypeCoercionEnabled(true));
  FrameworkConfig config = builder.build();
  return Frameworks.getPlanner(config);
}
...
public static boolean isAggOperator(SqlOperator operator)
{
  ...
  if (operatorName.equals("COUNT") || operatorName.equals("SUM") || operatorName.equals("AVG")
      || operatorName.equals("MAX") || operatorName.equals("MIN") || operatorName.equals("VAR_POP")
      || operatorName.equals("VAR_SAMP") || operatorName.equals("STDDEV_POP")
      || operatorName.equals("STDDEV_SAMP"))
    return true;
  return false;
}
...
{
  final Set<SqlKind> booleans = EnumSet.of(IN,
      NOT_IN,
      EQUALS,
      NOT_EQUALS,
      LESS_THAN,
...
public static void addUserDefinedFunctions(Collection<String> sqls)
{
  ...
  final Collection<String> calciteOpNames =
      new HashSet<>(ListSupport.map(getCalciteOperators().getOperatorList(), Objects::toString));
  for (String sql : sqls)
  {
    try
    {
...
public static boolean hasNodeOfKind(RelNode plan, Class kind)
{
  ...
  {
    if (hasNodeOfKind(children, kind))
      return true;
  }
  return false;
}
...
public static boolean checkRelNodeScalar(RelNode node)
{
  if (node instanceof Aggregate aggregate)
  {
    return aggregate.getAggCallList().size() == 1
        && aggregate.getAggCallList().get(0).getAggregation().getName().equalsIgnoreCase(
            "SINGLE_VALUE");
  }
  return false;
}
...
public static List<RexNode> getRexInputRefByIndexs(List<Integer> indexs)
{
  ...
  for (Integer index : indexs)
  {
    ...
    final RexInputRef rexInputRef =
        new RexInputRef(index, new JavaTypeFactoryImpl().createUnknownType());
...
public static boolean isUnaryRexNode(RexNode node)
{
  ...
  return kind == NOT || kind == PLUS_PREFIX || kind == MINUS_PREFIX || kind == IS_TRUE
      || kind == IS_FALSE || kind == IS_NOT_TRUE || kind == IS_NOT_FALSE || kind == IS_NULL
      || kind == IS_NOT_NULL || kind == IS_UNKNOWN;
}
...
{
  final SqlKind kind = node.getKind();
  return kind == AND || kind == OR || kind == EQUALS || kind == NOT_EQUALS || kind == LESS_THAN
      || kind == GREATER_THAN || kind == GREATER_THAN_OR_EQUAL || kind == LESS_THAN_OR_EQUAL;
}
...
public static Value getRexNodeColumn(RexNode expr, RelNode node)
{
  if (expr instanceof RexInputRef rexInputRef)
  {
    ...
    if (node instanceof Join join)
    {
      ...
      {
        return getRexNodeColumn(
            new RexInputRef(index - getOutputSizeOfRelNode(join.getLeft()), rexInputRef.getType()),
            join.getRight());
      }
      else
      {
    ...
    if (node instanceof SetOp setop)
    {
      // since the schema of two tables that use SetOp must be the same, so it only needs to select
      // a random one.
