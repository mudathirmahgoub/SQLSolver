    ...
    implements BiConsumer<L, P>
{
  ...
  private ParserAndLexerATNCaches<L, P> buildATNCache()
  {
    ...
    return new ParserAndLexerATNCaches(
        new AntlrATNCacheFields(lexerATN), new AntlrATNCacheFields(parserATN));
  }
  // ================= 泛型处理 =====================
  ...
  public static final class AntlrATNCacheFields
  {
    ...
    public void configureLexer(Lexer lexer)
    {
      ...
      checkArgument(
          atn == lexer.getATN(), "Lexer ATN mismatch: expected %s, found %s", atn, lexer.getATN());
      lexer.setInterpreter(
          new LexerATNSimulator(lexer, atn, decisionToDFA, predictionContextCache));
    }
    ...
    public void configureParser(Parser parser)
    {
      ...
      // Intentional identity equals comparison
      checkArgument(atn == parser.getATN(),
          "Parser ATN mismatch: expected %s, found %s",
          atn,
          parser.getATN());
