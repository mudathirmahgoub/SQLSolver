...
public class CastRemover
{
  ...
  {
    sql = sql.replaceAll(
        "CAST\\(NULL AS (SIGNED|CHAR\\(\\d+\\)|VARCHAR\\(\\d+\\)|TIMESTAMP\\(\\d+\\)|BOOLEAN)\\)",
        "NULL");
    return sql;
  }
  ...
  private void removeCastOfField(
      PlanContext plan, Expression expr, SqlNode node, FieldKey<SqlNode> key)
  {
  ...
  private boolean isConversionSafe(SqlDataType t1, SqlDataType t2)
  {
    ...
    if (t1.category().equals(Category.INTEGRAL) && t2.name().equals(DataTypeName.DECIMAL))
    {
  ...
  private boolean checkType(PlanContext plan, Expression expr, SqlNode node, SqlDataType type)
  {
    ...
    else if (ExprKind.Case.equals(kind))
    {
      ...
      for (SqlNode when : whens)
      {
        ...
        if (!checkType(plan, expr, whenExpr, type))
          return false;
      }
