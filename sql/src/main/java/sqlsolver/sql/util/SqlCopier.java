...
public class SqlCopier
{
  ...
  public SqlNode go()
  {
    if (toCtx == null)
      toCtx = root.context();
    if (tracks != null)
      destination = new int[tracks.length];

    final SqlNode copied = copy0(root);
    if (tracks != null)
      System.arraycopy(destination, 0, tracks, 0, destination.length);
    return copied;
  }
  ...
  private SqlNode copy0(SqlNode node)
  {
    ...
    if (destination != null)
    {
      ...
      if (idx >= 0)
        destination[idx] = newNodeId;
    }

    if (SqlKind.TableSource.isInstance(node))
    {
      toCtx.setFieldOf(
          newNodeId, SqlNodeFields.TableSource_Kind, node.$(SqlNodeFields.TableSource_Kind));
    }
