...
public class ParamInterpolator
{
  ...
  private void interpolateOne(ParamDesc param)
  {
    ...
    if (!ExprKind.Param.isInstance(paramNode))
      return;

    ParamModifier modifier = tail(param.modifiers());
    if (modifier == null)
      return;
    if (modifier.type() == ParamModifier.Type.TUPLE_ELEMENT
        || modifier.type() == ParamModifier.Type.ARRAY_ELEMENT)
      modifier = elemAt(param.modifiers(), -2);
    if (modifier == null || modifier.type() != ParamModifier.Type.COLUMN_VALUE)
      return;
  ...
  private SqlNode mkValue(Column column)
  {
    ...
    switch (column.dataType().category())
    {
      ...
      case TIME:
        ...
        break;
      default: value.$(ExprFields.Literal_Kind, LiteralKind.NULL); break;
    }
    return value;
  }
}
