...
public class ColumnAliasCoRewriter extends SqlNodeCoPreprocess
{
  ...
  private boolean isSelectStar(SqlNodeList list)
  {
    if (list.size() == 1)
    {
      if (list.get(0) instanceof SqlIdentifier id && id.names.size() == 1)
  ...
  private String getAlias(SqlNode node)
  {
    ...
    else if (node instanceof SqlBasicCall call && call.getOperator() instanceof SqlAsOperator)
    {
  ...
  private boolean needsHandle(SqlNodeList list0, SqlNodeList list1)
  {
    ...
    if (isSelectStar(list0) || isSelectStar(list1))
      return false;
  ...
  {
    if (nodes[0].toString().contains("VALUES"))
      return nodes;
    if (nodes[1].toString().contains("VALUES"))
      return nodes;
