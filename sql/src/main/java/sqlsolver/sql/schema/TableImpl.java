...
class TableImpl implements Table
{
  ...
  static TableImpl build(SqlNode tableDef)
  {
    ...
    final String engine = DbSupport.PostgreSQL.equals(tableDef.dbType())
  ...
  void addPatch(SchemaPatch patch)
  {
    for (String name : patch.columns())
    {
      ...
      if (column != null)
        column.addPatch(patch);
    }

    if (patch.type() == SchemaPatch.Type.UNIQUE)
    {
      final List<Column> columns = ListSupport.map((Iterable<String>) patch.columns(),
          (Function<? super String, ? extends Column>) this::column);
    ...
    if (patch.type() == SchemaPatch.Type.FOREIGN_KEY)
    {
      ...
      if (split.length != 2)
        throw new IllegalArgumentException("illegal patch: " + patch);
