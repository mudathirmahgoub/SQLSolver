  ...
  default String toDDL(String dbType)
  {
    switch (type())
    {
      case FOREIGN_KEY:
        ...
        {
          return ("ALTER TABLE \"%s\" ADD CONSTRAINT \"fk_%s_%s_refs_%s\" FOREIGN KEY (%s) "
                  + "REFERENCES \"%s\"(\"%s\")").formatted(table(),
                     table(),
                     joining("_", columns()),
                     reference().replace('.', '_'),
        ...
        {
          return ("ALTER TABLE `%s` ADD CONSTRAINT `fk_%s_%s_refs_%s` FOREIGN KEY (%s) REFERENCES "
                  + "`%s`(`%s`)").formatted(table(),
                     table(),
                     joining("_", columns()),
                     reference().replace('.', '_'),
      ...
      case UNIQUE:
        if (DbSupport.PostgreSQL.equals(dbType))
        {
          return "CREATE %sINDEX \"index_%s\" ON \"%s\"(%s)".formatted(
