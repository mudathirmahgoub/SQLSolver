...
class SchemaImpl implements Schema
{
  ...
  static Schema build(String dbType, Iterable<SqlNode> defs)
  {
    ...
    for (SqlNode def : defs)
    {
      ...
      if (type == SqlKind.CreateTable)
        addCreateTable(def, builders);
      else if (type == SqlKind.AlterSeq)
        addAlterSequence(def, builders);
      else if (type == SqlKind.AlterTable)
        addAlterTable(def, builders);
      else if (type == SqlKind.IndexDef)
        addIndexDef(def, builders);
    }
  ...
  private static void addAlterTable(SqlNode node, Map<String, TableBuilder> builders)
  {
    final TableBuilder builder =
        builders.get(node.$(SqlNodeFields.AlterTable_Name).$(SqlNodeFields.TableName_Table));
    if (builder != null)
      builder.fromAlterTable(node);
  }

  private static void addIndexDef(SqlNode node, Map<String, TableBuilder> builders)
  {
    final TableBuilder builder =
        builders.get(node.$(SqlNodeFields.IndexDef_Table).$(SqlNodeFields.TableName_Table));
    if (builder != null)
      builder.fromCreateIndex(node);
  }
      ...
      if (table.constraints() != null)
        for (Constraint constraint0 : table.constraints())
        {
          ...
          if (refTableName != null)
          {
            ...
            constraint.setRefColumns(ListSupport.map((Iterable<SqlNode>) constraint.refColNames(),
                (Function<? super SqlNode, ? extends Column>)
                    it -> ref.column(it.$(SqlNodeFields.ColName_Col))));
          }
        }
  }
