...
class ColumnImpl implements Column
{
  ...
  ColumnImpl(String table, String name, String rawDataType, SqlDataType dataType)
  {
    ...
    if (dataType.category() == Category.BOOLEAN)
      flags.add(Flag.IS_BOOLEAN);
    else if (dataType.category() == Category.ENUM)
      flags.add(Flag.IS_ENUM);
  }

  static ColumnImpl build(String table, SqlNode colDef)
  {
    final String colName =
        SqlSupport.simpleName(colDef.$(SqlNodeFields.ColDef_Name).$(SqlNodeFields.ColName_Col));
    ...
    if (colDef.isFlag(SqlNodeFields.ColDef_Generated))
      column.flag(Flag.GENERATED);
    if (colDef.isFlag(SqlNodeFields.ColDef_Default))
      column.flag(Flag.HAS_DEFAULT);
    if (colDef.isFlag(SqlNodeFields.ColDef_AutoInc))
      column.flag(Flag.AUTO_INCREMENT);
  ...
  void addConstraint(Constraint constraint)
  {
    ...
    if (constraint.kind() == null)
      ...
      {
        case PRIMARY: flags.add(Flag.PRIMARY); flags.add(Flag.NOT_NULL);
        case UNIQUE:
          flags.add(Flag.UNIQUE);
          flags.add(Flag.INDEXED);
          break;
        case NOT_NULL: flags.add(Flag.NOT_NULL); break;
        case FOREIGN:
          flags.add(Flag.FOREIGN_KEY);
          flags.add(Flag.INDEXED);
          break;
        case CHECK: flags.add(Flag.HAS_CHECK); break;
      }
  }
