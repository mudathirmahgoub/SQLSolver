...
public class AggSelectGroupRewriter extends RecursiveRewriter
{
  ...
  private void getColListAndAlias(
      SqlNodeList nodeList, SqlNodeList selectColList, SqlNodeList selectColAliasList)
  {
  ...
  private SqlNodeList getGroupByNotInNodeList(
      SqlNodeList groupBys, SqlNodeList selectColList, SqlNodeList selectColAliasList)
  {
    SqlNodeList result = new SqlNodeList(SqlParserPos.ZERO);
    final List<String> selectColStrList =
        selectColList.getList().stream().map(Object::toString).toList();
    final List<String> selectColStrAliasList =
        selectColAliasList.getList().stream().map(Object::toString).toList();
    for (final SqlNode groupBy : groupBys)
    {
      if (groupBy instanceof SqlIdentifier)
      {
        if (!selectColStrList.contains(groupBy.toString())
            && !selectColStrAliasList.contains(groupBy.toString()))
          result.add(groupBy);
      }
    }
    return result;
  ...
  private SqlNodeList getSelectOuter(SqlNodeList selectList)
  {
    ...
    for (int i = 0, bound = selectList.size(); i < bound; i++)
    {
      ...
      else
      {
        ...
        SqlBasicCall as = new SqlBasicCall(
            new SqlAsOperator(), new SqlNode[] {toAddItem, newAs}, SqlParserPos.ZERO);
        selectList.set(i, as);
        toAddItem = newAs;
      }
  ...
  public SqlNode handleNode(SqlNode node)
  {
    if (node instanceof SqlSelect select && isTheCase(node))
    {
      ...
      SqlNodeList groupByOwns =
          getGroupByNotInNodeList(groupBys, selectColList, selectColAliasList);
      ...
      SqlBasicCall as = new SqlBasicCall(
          new SqlAsOperator(), new SqlNode[] {select, newTableAlias()}, SqlParserPos.ZERO);
