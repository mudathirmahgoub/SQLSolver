...
// ->
// SELECT a + b, c, d FROM (SELECT agg(x) AS a, agg(y) AS b, agg(z) AS c, d FROM r GROUP BY ...) AS
// t
//
...
public class CompoundAggRewriter extends SqlNodePreprocess
{
  ...
  {
    return !op.equals(SqlStdOperatorTable.SINGLE_VALUE) && (op instanceof SqlAggFunction);
  }
    ...
    if (node instanceof SqlSelect select)
    {
      ...
      if (needsHandle(select.getSelectList()))
      {
        ...
        for (SqlNode item : select.getSelectList())
        {
          ...
          else if (item instanceof SqlBasicCall call && call.getOperator() instanceof SqlAsOperator)
          {
  ...
  private void handleItem(SqlNode item,
      Set<String> aliases,
      SqlNodeList outerList,
      SqlNodeList innerList,
      Map<String, String> aggAlias)
  {
    if (item instanceof SqlIdentifier id)
    {
  ...
  // update <innerList>
  private SqlNode aliasAggs(
      SqlNode item, Set<String> aliases, SqlNodeList innerList, Map<String, String> aggAlias)
  {
    if (item instanceof SqlBasicCall call)
    {
