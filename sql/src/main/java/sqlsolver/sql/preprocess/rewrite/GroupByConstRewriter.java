...
public class GroupByConstRewriter extends RecursiveRewriter
{
  ...
  public SqlNode handleNode(SqlNode node)
  {
    if (node instanceof SqlSelect select && select.getGroup() != null)
    {
      ...
      {
        if (groupNode instanceof SqlCharStringLiteral)
          continue;
        newGroupList.add(groupNode);
      }
      if (newGroupList.isEmpty())
        select.setGroupBy(SqlNodeList.EMPTY);
      select.setGroupBy(newGroupList.isEmpty() ? SqlNodeList.EMPTY
                                               : new SqlNodeList(newGroupList, SqlParserPos.ZERO));
    }
    return node;
  }
}
