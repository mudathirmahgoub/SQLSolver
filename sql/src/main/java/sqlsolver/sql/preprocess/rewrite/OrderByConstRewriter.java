...
public class OrderByConstRewriter extends RecursiveRewriter
{
  ...
  public SqlNode handleNode(SqlNode node)
  {
    if (node instanceof SqlOrderBy orderBy)
    {
      ...
      {
        if (orderNode instanceof SqlCharStringLiteral)
          continue;
        newOrderList.add(orderNode);
      }
      if (newOrderList.isEmpty() && orderBy.offset == null && orderBy.fetch == null)
        return orderBy.getOperandList().get(0);
      return new SqlOrderBy(SqlParserPos.ZERO,
          orderBy.getOperandList().get(0),
          newOrderList.isEmpty() ? SqlNodeList.EMPTY
                                 : new SqlNodeList(newOrderList, SqlParserPos.ZERO),
          orderBy.offset,
          orderBy.fetch);
    }
    return node;
