...
public class ExtractYearRewriter extends RecursiveRewriter
{
  ...
  {
    if (node instanceof SqlBasicCall call && call.getOperator() instanceof SqlExtractFunction
        && call.operand(0).toString().equalsIgnoreCase("YEAR"))
    {
      return SqlStdOperatorTable.YEAR.createCall(
          SqlParserPos.ZERO, Collections.singletonList(call.operand(1)));
    }
    return node;
  }
}
