...
public class BetweenAndRewriter extends RecursiveRewriter
{
  ...
  {
    if (node instanceof SqlBasicCall call && call.getOperator().equals(SqlStdOperatorTable.BETWEEN))
    {
      SqlNode x = call.operand(0);
      SqlNode l = call.operand(1);
      SqlNode r = call.operand(2);
      // x BETWEEN l AND r -> x >= l AND x <= r
      SqlCall left = SqlStdOperatorTable.GREATER_THAN_OR_EQUAL.createCall(SqlParserPos.ZERO, x, l);
      SqlCall right = SqlStdOperatorTable.LESS_THAN_OR_EQUAL.createCall(SqlParserPos.ZERO, x, r);
