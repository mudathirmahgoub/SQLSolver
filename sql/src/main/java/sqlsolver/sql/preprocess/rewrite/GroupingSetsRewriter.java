...
public class GroupingSetsRewriter extends RecursiveRewriter
{
  ...
  private boolean isDesiredPattern(SqlSelect select)
  {
    ...
    return !(from == null || group == null || select.getHaving() != null
        || !select.getWindowList().isEmpty() || select.getQualify() != null
        || select.getOrderList() != null || select.getOffset() != null
  ...
  private int groupType(SqlNodeList group)
  {
    ...
    {
      if (group.size() == 1 && gsets.getOperator().equals(SqlStdOperatorTable.GROUPING_SETS)
    ...
    if (groupList != null)
    {
      ...
      {
        if (operand instanceof SqlBasicCall row && row.getOperator() instanceof SqlRowOperator)
        {
  ...
  private boolean containsAgg(SqlNodeList list)
  {
    for (SqlNode node : list)
    {
      if (node instanceof SqlBasicCall call && call.getOperator() instanceof SqlAsOperator)
      {
        node = call.operand(0);
      }
      if (node instanceof SqlBasicCall call
  ...
  private boolean emptyGroupingSetHasAgg(SqlSelect select)
  {
    ...
    if (!hasEmptyGroupingSet(gsets.getOperandList()))
      return true;
  ...
  {
    SqlNodeList group = select.getGroup();
    return isDesiredPattern(select) && groupType(group) != GROUP_UNSUPPORT
  ...
  private SqlNode queryFromGroupingSet(SqlNodeList selectList,
      SqlNode from,
      SqlNode where,
      Set<String> groupingSet,
  ...
  private SqlNode convertSelectItem(SqlNode item, Set<String> groupingSet, Set<String> groupColumns)
  {
    ...
    if (item instanceof SqlBasicCall call && call.getOperator() instanceof SqlAsOperator)
    {
  ...
  private SqlNode convertNode(SqlNode node, Set<String> groupingSet, Set<String> groupColumns)
  {
    if (node instanceof SqlBasicCall call)
    {
      if (call.getOperator().getName().equals("GROUPING"))
      {
        ...
        {
          if (!groupingSet.contains(arg.toString()))
            value |= mask;
          mask >>= 1;
        }
  ...
  private SqlNode unionAll(List<SqlNode> nodes)
  {
    ...
    {
      union = SqlStdOperatorTable.UNION_ALL.createCall(SqlParserPos.ZERO, union, nodes.get(i));
    }
    return union;
  }
}
