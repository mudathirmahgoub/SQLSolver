...
public class PredExprRewriter extends SqlNodePreprocess
{
  ...
  private static final SqlLiteral LITERAL_FALSE =
  ...
  {
    SqlNode node = unionAll(handlePredExpr(select, predItem, LITERAL_TRUE),
        handlePredExpr(select, predItem, LITERAL_FALSE));
    // EXISTS(...) must not be NULL
    if (type == TYPE_IN_EXPR)
      node = unionAll(node, handlePredExpr(select, predItem, LITERAL_NULL));
    return node;
  }
