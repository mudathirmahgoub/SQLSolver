...
public class SemiAntiJoinRewriter extends RecursiveRewriter
{
  ...
  private boolean isSpecialJoinSign(SqlNode node, String sign)
  {
    ...
    {
      if (call.getOperator().toString().equals(sign) && call.operandCount() == 0)
      {
        return true;
      }
    }
      ...
      if (from instanceof SqlJoin join)
      {
        ...
        if (isSpecialJoin && join.getJoinType().equals(JoinType.LEFT))
        {
          ...
          SqlNode innerSelect = new SqlSelect(SqlParserPos.ZERO,
              ...
              null,
              null);
          SqlNode newWhere = SqlStdOperatorTable.EXISTS.createCall(
              SqlParserPos.ZERO, Collections.singletonList(innerSelect));
          if (isAntiJoin)
          {
            // negate
            newWhere = SqlStdOperatorTable.NOT.createCall(
                SqlParserPos.ZERO, Collections.singletonList(newWhere));
          }
          // FROM
          ...
          if (where != null)
            newWhere = SqlStdOperatorTable.AND.createCall(SqlParserPos.ZERO, where, newWhere);
