  ...
  private SqlNode handleFilter(SqlNode expr)
  {
    if (expr instanceof SqlBasicCall call)
    {
      if (call.getOperator() instanceof SqlFilterOperator
          ...
          && CalciteSupport.isAggOperator(agg.getOperator()) && agg.operandCount() >= 1)
      {
        ...
        for (SqlNode arg : args.subList(1, args.size()))
        {
          ...
          final SqlNode argIsNotNull =
              new SqlBasicCall(SqlStdOperatorTable.IS_NOT_NULL, isNotNullList, SqlParserPos.ZERO);
        ...
        return new SqlBasicCall(
            agg.getOperator(), newArgs, SqlParserPos.ZERO, agg.getFunctionQuantifier());
      }
      else
      {
        // recursion
