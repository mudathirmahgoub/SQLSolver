...
public class ParamModifier
{
      ...
      BinaryOpKind op, SqlNode target, boolean inverse, boolean not)
  {
    ...
    if (op == BinaryOpKind.NOT_EQUAL)
      return not ? KEEP_STILL : modifier(NEQ);
    ...
    if (op == BinaryOpKind.IS)
      return fromIs(target, not);

    if (op.toStandardOp() == BinaryOpKind.REGEXP)
      return not ? modifier(NEQ) : modifier(REGEX);

    if (op == BinaryOpKind.PLUS)
      return modifier(SUBTRACT);
    if (op == BinaryOpKind.MINUS)
      return modifier(ADD);
    if (op == BinaryOpKind.MULT)
      return modifier(DIVIDE);
    if (op == BinaryOpKind.DIV)
      return modifier(TIMES);
