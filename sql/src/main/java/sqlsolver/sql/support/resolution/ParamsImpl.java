...
class ParamsImpl implements Params
{
  ...
  private TIntObjectMap<ParamDesc> params()
  {
    if (params == null)
    {
      ...
      {
        for (SqlNode limitNode :
            LocatorSupport.clauseLocator().accept(SqlNodeFields.Query_Limit).gather(rootNode))
          params.put(limitNode.nodeId(), mkLimitParam(limitNode));

        for (SqlNode offsetNode :
            LocatorSupport.clauseLocator().accept(SqlNodeFields.Query_Offset).gather(rootNode))
      ...
      for (SqlNode predicate : LocatorSupport.predicateLocator().primitive().gather(rootNode))
        for (ParamDesc paramDesc : new ResolveParam().resolve(predicate))
          if (paramDesc != null)
            params.put(paramDesc.node().nodeId(), paramDesc);

      this.params = params;

      int index = 0;
      for (SqlNode paramNode :
          LocatorSupport.nodeLocator().accept(it -> paramOf(it) != null).gather(rootNode))
      {
        final ParamDesc param = paramOf(paramNode);
        if (!ResolutionSupport.isCheckNull(param))
          param.setIndex(index++);
        if (ResolutionSupport.isElementParam(param))
          ++index;
      }
    }
  ...
  {
    return new ParamDescImpl(
        null, offsetNode, singletonList(ParamModifier.modifier(ParamModifier.Type.OFFSET_VAL)));
  }

  private static ParamDesc mkLimitParam(SqlNode paramNode)
  {
    return new ParamDescImpl(
        null, paramNode, singletonList(ParamModifier.modifier(ParamModifier.Type.LIMIT_VAL)));
  }
