...
public abstract class ResolutionSupport
{
  ...
  public static boolean isServedAsInput(Relation relation)
  {
    ...
    return SqlKind.TableSource.isInstance(node) || SqlKind.TableSource.isInstance(parent)
  ...
  {
    if (!isDirectTable(relation))
      return null;
    final Schema schema = relation.rootNode().context().schema();
    final String tableName =
        relation.rootNode().$(TableSourceFields.Simple_Table).$(SqlNodeFields.TableName_Table);
  ...
  public static SqlNode tableSourceOf(Relation relation)
  {
    ...
    if (SqlKind.TableSource.isInstance(node))
      return node;
    final SqlNode parent = node.parent();
    if (SqlKind.TableSource.isInstance(parent))
      return parent;
    return null;
  }
  ...
  {
    final SqlNode node = relation.rootNode();
    if (SqlKind.Query.isInstance(node))
      return node;
    else
      return null;
  }
  ...
  {
    if (!ExprKind.ColRef.isInstance(colRef))
      return null;
    ...
    if (qualification == null)
    {
      ...
      if (clause == SqlNodeFields.Query_OrderBy || clause == SqlNodeFields.QuerySpec_GroupBy
          || clause == SqlNodeFields.QuerySpec_Having)
      {
  ...
  {
    return any(param.modifiers(),
        it
        -> it.type() == ParamModifier.Type.ARRAY_ELEMENT
            || it.type() == ParamModifier.Type.TUPLE_ELEMENT);
  }

  public static boolean isCheckNull(ParamDesc param)
  {
    final ParamModifier.Type lastModifierType = tail(param.modifiers()).type();
    return lastModifierType == ParamModifier.Type.CHECK_NULL
        || lastModifierType == ParamModifier.Type.CHECK_NULL_NOT;
  }
  ...
  private static FieldKey<?> getClauseOfExpr(SqlNode expr)
  {
    ...
    if (SqlKind.OrderItem.isInstance(parent))
      return SqlNodeFields.Query_OrderBy;
    if (SqlKind.GroupItem.isInstance(parent))
      return SqlNodeFields.QuerySpec_GroupBy;
    if (SqlKind.SelectItem.isInstance(parent))
      return SqlNodeFields.QuerySpec_SelectItems;
    if (TableSourceKind.JoinedSource.isInstance(parent))
      return TableSourceFields.Joined_On;
    if (nodeEquals(parent.$(SqlNodeFields.Query_Offset), child))
      return SqlNodeFields.Query_Offset;
    if (nodeEquals(parent.$(SqlNodeFields.Query_Limit), child))
      return SqlNodeFields.Query_Offset;
    if (nodeEquals(parent.$(SqlNodeFields.QuerySpec_Where), child))
      return SqlNodeFields.QuerySpec_Where;
    if (nodeEquals(parent.$(SqlNodeFields.QuerySpec_Having), child))
      return SqlNodeFields.QuerySpec_Having;
