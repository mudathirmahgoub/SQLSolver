...
class JoinGraphBuilder
{
  ...
  {
    if (isNegated(joinCond))
      return;

    final Attribute lhsKey = ResolutionSupport.resolveAttribute(joinCond.$(ExprFields.Binary_Left));
    final Attribute rhsKey =
        ResolutionSupport.resolveAttribute(joinCond.$(ExprFields.Binary_Right));
    if (lhsKey == null || rhsKey == null)
      return;

    final Column lhsCol = ResolutionSupport.traceRef(lhsKey).column(),
                 rhsCol = ResolutionSupport.traceRef(rhsKey).column();
    if (lhsCol == null || rhsCol == null)
      return;

    graph.addJoin(ResolutionSupport.traceRef(lhsKey).owner(),
        lhsCol,
        ResolutionSupport.traceRef(rhsKey).owner(),
        rhsCol);
  }
  ...
  private static void addInSubquery(JoinGraph graph, SqlNode inSub)
  {
    ...
    final Relation subqueryRel = ResolutionSupport.getEnclosingRelation(
        inSub.$(ExprFields.Binary_Right).$(ExprFields.QueryExpr_Query));
  ...
  private static boolean isNegated(SqlNode node)
  {
    ...
    {
      if (node.$(ExprFields.Unary_Op) == UnaryOpKind.NOT)
        negated = !negated;
      node = node.parent();
    }
    return negated;
