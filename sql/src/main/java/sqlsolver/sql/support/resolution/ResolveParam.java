...
class ResolveParam
{
  ...
  List<ParamDesc> resolve(SqlNode expr)
  {
    ...
    final List<SqlNode> params = LocatorSupport.nodeLocator()
                                     .accept(ResolveParam::isParam)
  ...
  private ParamDesc resolve(SqlNode expr, SqlNode paramNode)
  {
    ...
    { // trace back to expr root
      if (parent.get(ExprFields.Unary_Op) == NOT)
        negated = !negated;
  ...
  private boolean deduce(SqlNode target)
  {
    ...
    switch (exprKind)
    {
      case Unary:
      {
        ...
        if (op == UNARY_MINUS)
          stack.offerFirst(modifier(INVERSE));
      ...
      case Binary:
      {
        ...
        if (modifier.type() != KEEP)
          stack.offerFirst(modifier);

        return induce(otherSide);
      }

      case Tuple: stack.offerFirst(modifier(TUPLE_ELEMENT)); return true;

      case Array: stack.offerFirst(modifier(ARRAY_ELEMENT)); return true;
      ...
      case FuncCall:
      {
        final String funcName =
            parent.$(ExprFields.FuncCall_Name).$(SqlNodeFields.Name2_1).toLowerCase();
  ...
  private boolean induce(SqlNode target)
  {
    ...
    switch (exprKind)
    {
      case ColRef:
      {
        final Attribute reference =
            ResolutionSupport.traceRef(ResolutionSupport.resolveAttribute(target));
      ...
      case FuncCall:
      {
        ...
        for (SqlNode arg : Lists.reverse(args))
          if (!induce(arg))
            return false;
        return true;
      }
      ...
      case Binary:
      {
        ...
        {
          case PLUS: stack.offerFirst(modifier(ADD)); break;
          case MINUS: stack.offerFirst(modifier(SUBTRACT)); break;
          case MULT: stack.offerFirst(modifier(TIMES)); break;
          case DIV: stack.offerFirst(modifier(DIVIDE)); break;
          default: return false;
        }
        return induce(target.$(ExprFields.Binary_Right))
            && induce(target.$(ExprFields.Binary_Left));
      }
      ...
      case Cast:
        return target.$(ExprFields.Cast_Type).category() != Category.INTERVAL
            && induce(target.$(ExprFields.Cast_Expr));
      ...
      case QueryExpr: stack.offerFirst(modifier(GUESS)); return true;
  ...
  private static boolean isParam(SqlNode expr)
  {
    return (ExprKind.Literal.isInstance(expr) && !ExprKind.FuncCall.isInstance(expr.parent()))
        ...
        || ExprKind.FuncCall.isInstance(expr)
        && "now".equalsIgnoreCase(expr.$(ExprFields.FuncCall_Name).$(SqlNodeFields.Name2_1));
  }
}
