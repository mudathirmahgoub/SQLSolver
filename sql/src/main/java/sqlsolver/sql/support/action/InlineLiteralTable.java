...
class InlineLiteralTable
{
  ...
  {
    final SqlNodes literalTables =
        LocatorSupport.nodeLocator().accept(InlineLiteralTable::canInline).gather(node);
  ...
  private static boolean canInline(SqlNode node)
  {
    return TableSourceKind.DerivedSource.isInstance(node)
        && TableSourceKind.JoinedSource.isInstance(node.parent()) && isLiteralTable(node);
  }
  ...
  private static boolean isLiteralTable(SqlNode derived)
  {
    ...
    return !SqlKind.SetOp.isInstance(body) && body.$(SqlNodeFields.QuerySpec_From) == null
        && all(body.$(SqlNodeFields.QuerySpec_SelectItems),
            it -> ExprKind.Literal.isInstance(it.$(SqlNodeFields.SelectItem_Expr)));
  }
  ...
  private static void inlineExprs(SqlNode rootQuery, SqlNode tableSource)
  {
    ...
    final Relation targetRelation =
        ResolutionSupport.getEnclosingRelation(tableSource.$(TableSourceFields.Derived_Subquery));
    ...
    for (SqlNode colRef : colRefs)
    {
      ...
      if (baseRef == null || baseRef.owner() != targetRelation)
        continue;
      ...
      if (SqlKind.OrderItem.isInstance(parent))
      {
        ...
        if (SqlKind.Query.isInstance(q) && q.$(SqlNodeFields.Query_OrderBy).isEmpty())
          q.remove(SqlNodeFields.Query_OrderBy);
      }
      else
      {
  ...
  private static void reduceTable(SqlNode deriveTableSource)
  {
    ...
    final SqlNode body = ResolutionSupport.getEnclosingRelation(deriveTableSource)
                             .rootNode()
                             .$(SqlNodeFields.Query_Body);
    ...
    if (lhs == deriveTableSource)
      ctx.displaceNode(joinNode.nodeId(), rhs.nodeId());
    else
      ctx.displaceNode(joinNode.nodeId(), lhs.nodeId());
