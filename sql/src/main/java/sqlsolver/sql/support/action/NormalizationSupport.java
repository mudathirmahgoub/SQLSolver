...
public abstract class NormalizationSupport
{
  ...
  static void detachExpr(SqlNode node)
  {
    ...
    {
      if (nodeEquals(node, parent.$(SqlNodeFields.QuerySpec_Where)))
        parent.remove(SqlNodeFields.QuerySpec_Where);
      else if (nodeEquals(node, parent.$(SqlNodeFields.QuerySpec_Having)))
        parent.remove(SqlNodeFields.QuerySpec_Having);
      else
    ...
    if (!ExprKind.Binary.isInstance(parent))
      return;
  ...
  static boolean isConstant(SqlNode node)
  {
    ...
    switch (exprKind)
    {
      ...
      case Cast: return isConstant(node.$(ExprFields.Cast_Expr));
      case Collate: return isConstant(node.$(ExprFields.Collate_Expr));
      case Interval: return isConstant(node.$(ExprFields.Interval_Expr));
      case ConvertUsing: return isConstant(node.$(ExprFields.ConvertUsing_Expr));
      case Default: return isConstant(node.$(ExprFields.Default_Col));
      case Values: return isConstant(node.$(ExprFields.Values_Expr));
      case Unary: return isConstant(node.$(ExprFields.Unary_Expr));
      case Binary:
        return isConstant(node.$(ExprFields.Binary_Left))
            && isConstant(node.$(ExprFields.Binary_Right));
      case Ternary:
      ...
      case Tuple: return all(node.$(ExprFields.Tuple_Exprs), NormalizationSupport::isConstant);
      case FuncCall:
      ...
      case Case:
        ...
        return (cond == null || isConstant(cond)) && (else_ == null || isConstant(else_))
