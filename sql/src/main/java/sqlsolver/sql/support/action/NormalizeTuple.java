...
class NormalizeTuple
{
  ...
  private static boolean isTuple(SqlNode node)
  {
    ...
    return (op == BinaryOpKind.ARRAY_CONTAINS || op == BinaryOpKind.IN_LIST)
        && nodeEquals(rhs, node);
  }
  ...
  private static void normalizeTuple(SqlNode node)
  {
    ...
    if (ExprKind.Array.isInstance(node))
      node.$(ExprFields.Array_Elements, elements);
    else if (ExprKind.Tuple.isInstance(node))
      node.setField(ExprFields.Tuple_Exprs, elements);
    else
      assert false;
  }
}
