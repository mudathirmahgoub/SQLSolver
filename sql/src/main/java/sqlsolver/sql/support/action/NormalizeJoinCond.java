...
class NormalizeJoinCond
{
  ...
  {
    for (SqlNode querySpec :
        LocatorSupport.nodeLocator().accept(SqlKind.QuerySpec).bottomUp(true).gather(root))
    {
      process(querySpec);
    }
  }
  ...
  {
    final SqlNodes onConditions = LocatorSupport.clauseLocator()
                                      .accept(TableSourceFields.Joined_On)
                                      .scoped()
                                      .gather(querySpec);
    ...
    {
      if (!onCond.parent().$(TableSourceFields.Joined_Kind).isInner())
        continue;

      final List<SqlNode> terms = SqlSupport.linearizeConjunction(onCond);
      for (SqlNode term : terms)
        if (isPlainCondition(term))
          plainConditions.add(term.nodeId());
    }
  ...
  private static boolean isPlainCondition(SqlNode expr)
  {
    if (ExprKind.Unary.isInstance(expr))
      return expr.$(ExprFields.Unary_Op) == UnaryOpKind.NOT
          && isPlainCondition(expr.$(ExprFields.Unary_Expr));

    if (ExprKind.Binary.isInstance(expr))
      return !expr.$(ExprFields.Binary_Op).isLogic()
          && (!ExprKind.ColRef.isInstance(expr.$(ExprFields.Binary_Left))
              || !ExprKind.ColRef.isInstance(expr.$(ExprFields.Binary_Right)));
