...
class Clean
{
  static void clean(SqlNode root)
  {
    SqlNode target;
    boolean flag = true;
    while ((target = LocatorSupport.nodeLocator().accept(Clean::isBoolConstant).find(root)) != null
        && flag)
  ...
  private static boolean deleteBoolConstant(SqlNode node)
  {
    ...
    if (nodeEquals(node, parent.$(QuerySpec_Where)) && node.$(ExprFields.Literal_Value) != null
        && (Boolean) node.$(ExprFields.Literal_Value))
    {
    ...
    else if (ExprKind.Binary.isInstance(parent) && parent.$(ExprFields.Binary_Op).isLogic())
    {
      ...
      if (ExprKind.Binary.isInstance(grandParent)
          && grandParent.$(ExprFields.Binary_Op).precedence()
              == BinaryOpKind.NULL_SAFE_EQUAL.precedence())
      {
        final SqlNode glhs = grandParent.$(ExprFields.Binary_Left),
                      grhs = grandParent.$(ExprFields.Binary_Right);
        if ((nodeEquals(glhs, parent) && grhs.$(ExprFields.Literal_Value) != null
                && (Boolean) grhs.$(ExprFields.Literal_Value))
            || (nodeEquals(grhs, parent) && glhs.$(ExprFields.Literal_Value) != null
                && (Boolean) glhs.$(ExprFields.Literal_Value)))
        {
          // replacement
  ...
  private static boolean isTextFunc(SqlNode node)
  {
    ...
    if (name == null || name.$(Name2_0) != null)
      return false; // UDF
    ...
    switch (funcName)
    {
      case "concat": return all(args, pred(ExprKind.Literal::isInstance).or(Clean::isTextFunc));
      case "lower":
      case "upper":
        return args.size() == 1
            && (ExprKind.Literal.isInstance(args.get(0)) || isTextFunc(args.get(0)));
  ...
  private static void inlineTextConstant(SqlNode funcCall)
  {
    final SqlNode literal =
        SqlSupport.mkLiteral(funcCall.context(), LiteralKind.TEXT, stringify(funcCall));
    funcCall.context().displaceNode(funcCall.nodeId(), literal.nodeId());
  }

  private static String stringify(SqlNode node)
  {
    if (ExprKind.Literal.isInstance(node))
      return String.valueOf(node.$(ExprFields.Literal_Value));
    ...
    switch (funcName)
    {
      case "concat": return joining("", args, Clean::stringify);
      case "upper": return stringify(args.get(0)).toUpperCase();
      case "lower": return stringify(args.get(0)).toLowerCase();
