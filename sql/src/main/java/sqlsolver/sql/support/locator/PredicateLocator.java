...
class PredicateLocator implements SqlVisitor, SqlGatherer, SqlFinder
{
  ...
  protected PredicateLocator(boolean scoped,
      boolean bottomUp,
      boolean primitive,
  ...
  {
    if (!bottomUp && when != null)
      traversePredicate(when.$(ExprFields.When_Cond));
    return false;
  }
  ...
  {
    if (!bottomUp && child != null
        && (key == TableSourceFields.Joined_On || key == SqlNodeFields.QuerySpec_Where
            || key == SqlNodeFields.QuerySpec_Having))
    {
  ...
  {
    if (bottomUp && when != null)
      nodes.add(when.$(ExprFields.When_Cond).nodeId());
  }
