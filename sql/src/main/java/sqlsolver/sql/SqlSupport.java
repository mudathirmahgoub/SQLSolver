...
public abstract class SqlSupport
{
  ...
  private static StringBuilder dumpAst(SqlNode root, StringBuilder builder, int level)
  {
    ...
    for (Map.Entry<FieldKey<?>, Object> pair : fields.entrySet())
    {
      ...
      if (value instanceof SqlNode || value instanceof SqlNodes)
        continue;
  ...
  public static SqlNode mkName2(SqlContext ctx, String piece0, String piece1)
  {
    ...
    if (piece0 != null)
      node.$(SqlNodeFields.Name2_0, piece0);
  ...
  public static SqlNode mkWildcard(SqlContext ctx, String tableName)
  {
    ...
    if (tableName != null)
      wildcard.$(ExprFields.Wildcard_Table, mkTableName(ctx, tableName));
    return wildcard;
  }
  ...
  public static SqlNode mkConjunction(SqlContext ctx, Iterable<SqlNode> filters)
  {
    ...
    for (SqlNode filter : filters)
    {
      ...
      if (expr == null)
        ...
        expr = mkBinary(ctx, BinaryOpKind.AND, expr, filter);
    }
    return expr;
  }
  ...
  {
    return ExprKind.Binary.isInstance(ast) && BinaryOpKind.EQUAL == ast.$(ExprFields.Binary_Op)
  ...
  public static boolean isEquiConstPredicate(SqlNode ast)
  {
    ...
    if (ExprKind.ColRef.isInstance(lhs) && ExprKind.Literal.isInstance(rhs))
      literal = rhs;
    else if (ExprKind.ColRef.isInstance(rhs) && ExprKind.Literal.isInstance(lhs))
      literal = lhs;
    else
      return false;
  ...
  {
    if (!ExprKind.Binary.isInstance(ast))
      return false;
    final BinaryOpKind op = ast.$(ExprFields.Binary_Op);
