...
class NotNullInference
{
  ...
  {
    if (!ctx.valuesReg().valuesOf(surfaceId).contains(toCheck))
      return false;

    final PlanKind kind = ctx.kindOf(surfaceId);
    if (kind == PlanKind.Input)
      return onInput(toCheck);
    else if (kind == PlanKind.Join)
      return onJoin(toCheck, surfaceId);
    else if (kind == PlanKind.Filter)
      return onFilter(toCheck, surfaceId);
    else if (kind == PlanKind.Proj)
      return onProj(toCheck, surfaceId);
    else if (kind == PlanKind.Agg)
      return onAgg(toCheck, surfaceId);
    else
