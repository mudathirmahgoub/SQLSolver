...
class ValuesRegistryImpl implements ValuesRegistry
{
  ...
  public Values valuesOf(int nodeId)
  {
    ...
    switch (ctx.kindOf(nodeId))
    {
      ...
      case SetOp:
      case Limit:
      case Sort: return valuesOf(ctx.childOf(nodeId, 0));
      ...
      case Input: values = mkValuesOfInput((InputNode) ctx.nodeAt(nodeId)); break;

      default: throw new PlanException("unknown plan node kind: " + ctx.kindOf(nodeId));
    }

    bindValues(nodeId, values);
    if (exprs != null)
      zip(values, exprs, this::bindExpr);
  ...
  {
    final Values vs;
    if (valueRefs instanceof Values)
      vs = (Values) valueRefs;
    else
      vs = Values.mk(valueRefs);
  ...
  {
    final Values values;
    if (rawValues instanceof Values)
      values = (Values) rawValues;
    else
      values = Values.mk(rawValues);
  ...
  {
    if (valueColumns.forRead().containsKey(value))
      valueColumns.forWrite().remove(value);
    if (valueExprs.forRead().containsKey(value))
    {
      final Expression expr = valueExprs.forWrite().remove(value);
      if (!valueExprs.forRead().containsValue(expr))
        exprRefs.forWrite().remove(expr);
    }
  }
