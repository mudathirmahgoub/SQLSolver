...
public abstract class PlanSupport
{
  ...
  public static boolean isSupported(SqlNode ast)
  {
    ...
    for (int i = 1; i <= ctx.maxNodeId(); i++)
    {
      ...
      if (ctx.kindOf(i) == SqlKind.QuerySpec
          && ctx.fieldOf(i, SqlNodeFields.QuerySpec_From) == null)
        return false;
    }
    return true;
  }
  ...
  {
    PlanContext plan;
    if ((plan = buildPlan(ast, schema)) == null)
      return null;
    if ((plan = resolvePlan(plan)) == null)
      return null;
  ...
  public static int locateNode(PlanContext ctx, int startPoint, int... pathExpr)
  {
    ...
    for (int direction : pathExpr)
    {
      if (direction < 0)
        ...
        node = ctx.childOf(node, direction);
    }
    return node;
  }
  ...
  public static boolean isEqualTree(PlanContext ctx0, int tree0, PlanContext ctx1, int tree1)
  {
    ...
    if (node0.kind() != PlanKind.Input || node1.kind() != PlanKind.Input)
      return false;
  ...
  public static List<Column> tryResolveColumns(
      PlanContext ctx, List<Value> values, boolean allowsNullColumn)
  {
  ...
  public static Column tryResolveColumn(PlanContext ctx, Value value)
  {
    ...
    final Expression expr = valueReg.exprOf(value);
    if (expr == null)
      return valueReg.columnOf(value);
    if (!isColRef(expr))
      return null;
  ...
  {
    if (ctx.kindOf(nodeId) != PlanKind.Join)
      return null;
  ...
  public static boolean isDedup(PlanContext ctx, int nodeId)
  {
    ...
    if (nodeKind == PlanKind.SetOp)
      return ((SetOpNode) ctx.nodeAt(nodeId)).deduplicated();
    if (nodeKind == PlanKind.Proj)
    {
      ...
      if (assigned != null)
        ...
        return ((ProjNode) ctx.nodeAt(nodeId)).deduplicated();
    }
    return false;
  }
  ...
  public static PlanContext disambiguateQualification(PlanContext ctx)
  {
    final List<PlanNode> nodes =
        gatherNodes(ctx, EnumSet.of(PlanKind.Proj, PlanKind.Agg, PlanKind.Input));
    ...
    for (PlanNode node : nodes)
    {
      ...
      if (!mustBeQualified(ctx, ctx.nodeIdOf(node)))
        continue;
      final String oldQualification = qualified.qualification();
      if (oldQualification != null && knownQualifications.add(qualified.qualification()))
        continue;
  ...
  public static List<Expression> getExprsIn(PlanContext plan, int treeRoot)
  {
    ...
    switch (kind)
    {
      ...
      case Exists:
      case SetOp:
      case Limit: return emptyList();
      case Filter: return singletonList(((SimpleFilterNode) plan.nodeAt(treeRoot)).predicate());
      case InSub: return singletonList(((InSubNode) plan.nodeAt(treeRoot)).expr());
      case Proj: return ((ProjNode) plan.nodeAt(treeRoot)).attrExprs();
      case Agg:
      {
        ...
        if (agg.havingExpr() != null)
          exprs.add(agg.havingExpr());
        return exprs;
      }
      case Sort: return ((SortNode) plan.nodeAt(treeRoot)).sortSpec();
      case Join: return singletonList(((JoinNode) plan.nodeAt(treeRoot)).joinCond());
      default: throw new IllegalArgumentException("unsupported node: " + kind);
    }
  }
  ...
  public static List<Value> getRefBindingForeignLookup(PlanContext plan, int nodeId)
  {
    ...
    while (plan.isPresent(parent))
    {
      if (plan.kindOf(parent).isSubqueryFilter() && indexOfChild(plan, child) == 1)
      {
        ...
        if (foreignLookup == null)
          foreignLookup = foreignLookup0;
        else
          foreignLookup = ListSupport.join(foreignLookup, foreignLookup0);
      }
      child = parent;
  ...
  public static boolean setupSubqueryExprOf(PlanContext plan, int nodeId)
  {
    ...
    final PlanKind kind = plan.kindOf(nodeId);
    if (kind == PlanKind.InSub)
      return setupInSub(plan, nodeId);
    else if (kind == PlanKind.Exists)
      return setupExists(plan, nodeId);
    else
      throw new IllegalArgumentException("not a subquery filter: " + nodeId + " in " + plan);
  }

  private static boolean setupInSub(PlanContext plan, int nodeId)
  {
    final SqlNode inSubExprAst = mkInSubExpr(plan, nodeId);
    if (inSubExprAst == null)
      return false;
  ...
  public static boolean isSimpleExprDetermineNotNull(Expression expr)
  {
    ...
    if (expr == null)
      return false;
    if (!isSingleParamExpr(expr))
      return false;
    if (!isSimpleBinaryExpression(expr.template(), false))
      return false;
  ...
  private static boolean isSimpleExprDetermineNotNull0(SqlNode pred)
  {
    if (!ExprKind.Binary.isInstance(pred))
      return false; // Do not consider other kinds of predicate

    // In binary predicate, cannot determine NotNull iff encountering `col IS NULL`
    if (colIsNullPredicate(pred))
      return false;
  ...
  public static boolean isSingleParamExpr(Expression expr) {
    ...
    {
      if (ExprKind.ColRef.isInstance(pred))
        return true;
      ...
      if (ExprKind.Binary.isInstance(pred))
      {
        ...
        if (colIsNullPredicate(pred))
          return ExprKind.ColRef.isInstance(lhs); // col IS NULL

        return isSimpleBinaryExpression(lhs, intArithmetic)
            && isSimpleBinaryExpression(rhs, intArithmetic);
      }
      return false;
    }
    ...
    {
      return ExprKind.Binary.isInstance(pred) && pred.$(ExprFields.Binary_Op) == BinaryOpKind.IS
    ...
    public static boolean isColRefs(Expression expr)
    {
      ...
      if (ExprKind.ColRef.isInstance(ast))
        return true;
      if (!ExprKind.Tuple.isInstance(ast))
        return false;
    ...
    public static Expression mkJoinCond(int numKeys)
    {
      ...
      for (int i = 0; i < numKeys; i++)
      {
        ...
        if (expr == null)
          ...
          expr = mkBinary(sqlCtx, BinaryOpKind.AND, expr, eq);
      }
    ...
    static boolean isBoolConstant(SqlNode exprAst)
    {
      return ExprKind.Binary.isInstance(exprAst)
          ...
          && !(exprAst.$(ExprFields.Binary_Left).$(ExprFields.Literal_Kind) == LiteralKind.NULL
              && exprAst.$(ExprFields.Binary_Right).$(ExprFields.Literal_Kind)
                  == LiteralKind.INTEGER
              && exprAst.$(ExprFields.Binary_Right)
                     .$(ExprFields.Literal_Value)
                     .equals((Integer) 1));
    }
    ...
    {
      if (ctx.kindOf(nodeId) == PlanKind.Input)
        return true;
      ...
      while (ctx.isPresent(parent))
      {
        ...
        if (parentKind == PlanKind.SetOp)
          return false;
        if (parentKind == PlanKind.Proj || parentKind == PlanKind.Join)
          return true;
        if (parentKind.isFilter())
          return ctx.childOf(parent, 0) == child;
        child = parent;
