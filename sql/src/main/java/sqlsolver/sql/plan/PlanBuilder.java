...
class PlanBuilder
{
  ...
  private int build0(SqlNode ast)
  {
    ...
    switch (kind)
    {
      case TableSource: return buildTableSource(ast);
      case SetOp: return buildSetOp(ast);
      case Query:
      {
        int nodeId = build0(ast.$(SqlNodeFields.Query_Body));
        nodeId = buildSort(ast.$(SqlNodeFields.Query_OrderBy), nodeId);
        nodeId =
            buildLimit(ast.$(SqlNodeFields.Query_Limit), ast.$(SqlNodeFields.Query_Offset), nodeId);
        return nodeId;
      }
      case QuerySpec:
      {
      ...
      default: return onError(PlanSupport.FAILURE_INVALID_QUERY + ast);
    }
  }
  ...
  private int buildProjection(SqlNode querySpec, int child)
  {
    ...
    final SqlNodes groupBys =
        coalesce(querySpec.$(SqlNodeFields.QuerySpec_GroupBy), SqlNodes.mkEmpty());
    ...
    if (!containsAgg(items) && groupBys.isEmpty())
    {
      ...
      if (child != NO_SUCH_NODE)
        plan.setChild(projNodeId, 0, child);
    ...
    else
    {
      ...
      */
      ...
      if (having != null)
        colRefs.addAll(LocatorSupport.gatherColRefs(having));

      // 3. build Proj node
      final Set<String> aggItemNames = map(items, it -> it.$(SqlNodeFields.SelectItem_Alias));
      final ProjNode proj =
          mkForwardProj(colRefs, containsDeduplicatedAgg(items), aggItemNames, numRefInProj);
  ...
  private TIntList buildFilters0(SqlNode expr, TIntList filters)
  {
    ...
    else if (ExprKind.Exists.isInstance(expr))
    {
      ...
      final int subqueryId =
          build0(expr.$(ExprFields.Exists_Subquery).$(ExprFields.QueryExpr_Query));
      if (subqueryId == FAIL)
        return null;
      final int nodeId = plan.bindNode(filter);
      plan.setChild(nodeId, 1, subqueryId);
  ...
  private int buildLimit(SqlNode limit, SqlNode offset, int child)
  {
    ...
    final LimitNode limitNode = LimitNode.mk(
        limit == null ? null : Expression.mk(limit), offset == null ? null : Expression.mk(offset));
  ...
  private int buildJoinedTableSource(SqlNode tableSource) {
    ...
    private int buildDerivedTableSource(SqlNode tableSource)
    {
      ...
      if (alias == null)
        return onError(PlanSupport.FAILURE_MISSING_QUALIFICATION + tableSource);
        ...
        int inputNodeId, SqlNode wildcard, List<String> attrNames, List<Expression> attrExprs)
    {
      ...
      if (wildcard.$(ExprFields.Wildcard_Table) == null)
        qualification = null;
      else
        qualification = wildcard.$(ExprFields.Wildcard_Table).$(SqlNodeFields.TableName_Table);
    ...
    private ProjNode mkForwardProj(
        List<SqlNode> colRefs, boolean deduplicated, Set<String> aggItemNames, int numRefsInProj)
    {
      ...
      for (int i = 0, bound = colRefs.size(); i < bound; i++)
      {
        ...
        if (i >= numRefsInProj)
        {
          ...
          if (qualification == null && aggItemNames.contains(refName))
            continue;
        }
