...
class ValueRefBinder
{
  ...
  private boolean bindAgg(int nodeId) {
    ...
    for (Expression attr : aggNode.attrExprs())
        {
          final List<Value> refs =
              ListSupport.map(attr.colRefs(), it -> bindRef(it, secondaryLookup));
    ...
    private boolean bindProj(int nodeId)
    {
      ...
      for (int i = 1; i < childrenIds.length; i++)
      {
        ...
        if (childrenIds[i] == NO_SUCH_NODE)
          continue;
        if (!bind0(plan.childOf(nodeId, i), lookup))
          return false;
      }
    ...
    private boolean bindExists(int nodeId, List<Value> secondaryLookup)
    {
      final List<Value> primaryLookup = PlanSupport.getRefBindingLookup(plan, nodeId);
      final List<Value> lookup = ListSupport.join(primaryLookup, secondaryLookup);
      if (!bind0(plan.childOf(nodeId, 1), lookup))
        return false;

      if (PlanSupport.setupSubqueryExprOf(plan, nodeId))
        return true;
      else
        return onError(PlanSupport.FAILURE_BAD_SUBQUERY_EXPR);
    }
    ...
    private boolean bindInSub(int nodeId, List<Value> secondaryLookup)
    {
      ...
      if (valueRefs.contains(null) || !bind0(plan.childOf(nodeId, 1), lookup))
        return false;
      ...
      if (PlanSupport.setupSubqueryExprOf(plan, nodeId))
        ...
        return onError(PlanSupport.FAILURE_BAD_SUBQUERY_EXPR + inSub);
    }
      ...
      for (Expression sortSpec : sortSpecs)
      {
        for (Value ref : valuesReg.valueRefsOf(sortSpec))
        {
          for (int i = 0, bound = inValues.size(); i < bound; i++)
          {
            ...
            if (ref == value)
              indexedRefs.add(i);
            else if (PlanSupport.deRef(plan, value) == ref)
              indexedRefs.add(i);
            else
              indexedRefs.add(-1);
          }
