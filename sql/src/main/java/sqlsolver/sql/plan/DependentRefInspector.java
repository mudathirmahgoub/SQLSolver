  ...
  private void inspect0(int nodeId)
  {
    ...
    switch (kind)
    {
      ...
      case Filter:
        ...
        break;
      case InSub: isDependentNode = inspectExpr(((InSubNode) plan.nodeAt(nodeId)).expr()); break;
      case Join: isDependentNode = inspectExpr(((JoinNode) plan.nodeAt(nodeId)).joinCond()); break;
    }

    if (isDependentNode)
      depNodes.get().add(nodeId);
