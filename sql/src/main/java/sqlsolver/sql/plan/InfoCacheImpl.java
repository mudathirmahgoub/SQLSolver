...
public class InfoCacheImpl implements InfoCache
{
  ...
  {
    if (joinKeys.forRead().containsKey(nodeId))
      joinKeys.forWrite().remove(nodeId);
    if (joinKinds.forRead().containsKey(nodeId))
      joinKinds.forWrite().remove(nodeId);
    if (subqueryExprs.forRead().containsKey(nodeId))
      subqueryExprs.forWrite().remove(nodeId);
    if (deduplicated.forRead().containsKey(nodeId))
      deduplicated.forWrite().remove(nodeId);
  }
