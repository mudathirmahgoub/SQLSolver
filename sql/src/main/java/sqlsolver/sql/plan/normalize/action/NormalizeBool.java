    ...
    switch (exprKind)
    {
      case Binary
        ...
        if (binaryOpKind.isComparison()) {
          /* for comparison filter,  a {=,<>,<,>,<=,>=} CONST, when a's type is different from CONST
          => FALSE e.g a = 'test' and a's type is INTEGER => FALSE
          */

          final SqlNode cstNode = ExprKind.Literal.isInstance(rhs) ? rhs:
        ExprKind.Literal.isInstance(lhs) ? lhs : null;
        final SqlNode refNode = ExprKind.ColRef.isInstance(rhs) ? rhs
            : ExprKind.ColRef.isInstance(lhs)                   ? lhs
                                                                : null;

        if (cstNode != null && refNode != null)
        {
          if (cstNode.$(ExprFields.Literal_Kind) == LiteralKind.NULL)
            break;
