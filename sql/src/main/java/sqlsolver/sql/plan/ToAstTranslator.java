...
class ToAstTranslator
{
  ...
  private QueryBuilder onNode(int node)
  {
    ...
    {
      case Input: return onInput(node);
      case Join: return onJoin(node);
      case Filter: return onFilter(node);
      case InSub: return onInSub(node);
      case Exists: return onExists(node);
      case Proj: return onProj(node);
      case Agg: return onAgg(node);
      case Sort: return onSort(node);
      case Limit: return onLimit(node);
      case SetOp: return onSetOp(node);
      default: throw new IllegalArgumentException("unknown op kind: " + plan.kindOf(node));
    }
  }
  ...
  private QueryBuilder onInput(int nodeId)
  {
    ...
    final SqlNode tableSource =
        SqlSupport.mkSimpleSource(sql, input.table().name(), input.qualification());
  ...
  private QueryBuilder onJoin(int nodeId)
  {
    ...
    if (tableSource0 == null || tableSource1 == null)
  ...
  private QueryBuilder onInSub(int nodeId)
  {
    ...
    final SqlNode filterNode =
        SqlSupport.mkBinary(sql, BinaryOpKind.IN_SUBQUERY, mkExpr(inSub.expr()), subqueryNode);
  ...
  private QueryBuilder onProj(int nodeId)
  {
    ...
    return query.setSelectItems(selectItems, false)
  ...
  private QueryBuilder onAgg(int nodeId)
  {
    ...
    return query.setSelectItems(selectItems, true)
  ...
  private QueryBuilder onSetOp(int nodeId)
  {
    ...
    unionNode.$(
        SqlNodeFields.SetOp_Option, setOp.deduplicated() ? SetOpOption.DISTINCT : SetOpOption.ALL);
    return mkBuilder()
        .setSource(SqlSupport.mkQuery(sql, unionNode))
        .setQualification(mkQualification(nodeId));
  }
  ...
  private String mkQualification(int nodeId)
  {
    ...
    if (node instanceof Exporter)
    {
      ...
      if (exporter.qualification() != null)
        return exporter.qualification();
    }
    ...
    for (Value value : values)
    {
      ...
      if (qualification != null)
        if (guess == null)
          guess = qualification;
        else if (!guess.equals(qualification))
          return null;
    }
  ...
  private static class QueryBuilder
  {
    ...
    SqlNode asTableSource()
    {
      ...
      if (isPureSource() && SqlKind.TableSource.isInstance(tableSource))
        return tableSource;
      if (qualification == null)
        return translator.onError(PlanSupport.FAILURE_MISSING_QUALIFICATION);
    ...
    SqlNode asQuery()
    {
      ...
      else
      {
        ...
        if (tableSource != null)
          spec.$(SqlNodeFields.QuerySpec_From, tableSource);
        if (!isNullOrEmpty(filters))
          spec.$(SqlNodeFields.QuerySpec_Where, SqlSupport.mkConjunction(sql, filters));
        if (!isNullOrEmpty(groupBys))
          spec.$(SqlNodeFields.QuerySpec_GroupBy, SqlNodes.mk(sql, groupBys));
        if (having != null)
          spec.$(SqlNodeFields.QuerySpec_Having, having);

        final SqlNodes selections;
        if (selectItems != null)
          selections = SqlNodes.mk(sql, selectItems);
        else
          selections = SqlNodes.mk(sql, singletonList(SqlSupport.mkWildcard(sql, null)));
        spec.$(SqlNodeFields.QuerySpec_SelectItems, selections);

        if (!isAgg && deduplicated)
          spec.$(SqlNodeFields.QuerySpec_Distinct, true);
        if (isAgg && selectItems != null)
        {
          for (SqlNode selectItem : selectItems)
          {
            final SqlNode expr = selectItem.$(SqlNodeFields.SelectItem_Expr);
            if (ExprKind.Aggregate.isInstance(expr))
              expr.flag(ExprFields.Aggregate_Distinct, deduplicated);
          }
        }
      ...
      if (orderBys != null)
        q.$(SqlNodeFields.Query_OrderBy, SqlNodes.mk(sql, orderBys));
      if (offset != null)
        q.$(SqlNodeFields.Query_Offset, offset);
      if (limit != null)
        q.$(SqlNodeFields.Query_Limit, limit);
    ...
    QueryBuilder pushFilter(SqlNode filter)
    {
      if (isInvalid())
        return INVALID;
      if (isFullQuery() && !collapseAsTableSource())
        return INVALID;
    ...
    QueryBuilder setSelectItems(List<SqlNode> selectItems, boolean isAgg)
    {
      if (isInvalid())
        return INVALID;
      if (isAgg && (this.isAgg || this.selectItems == null))
        return INVALID;
      if (!isAgg && isFullQuery() && !collapseAsTableSource())
        return INVALID;
    ...
    QueryBuilder setOrderBy(List<SqlNode> orderBys)
    {
      ...
      if (this.orderBys != null && !collapseAsQuery())
        return INVALID;
    ...
    QueryBuilder setLimit(SqlNode limit, SqlNode offset)
    {
      ...
      if ((this.limit != null || this.offset != null) && !collapseAsQuery())
        return INVALID;
    ...
    {
      return filters == null && selectItems == null && groupBys == null && orderBys == null
          && having == null && limit == null && offset == null;
    }
