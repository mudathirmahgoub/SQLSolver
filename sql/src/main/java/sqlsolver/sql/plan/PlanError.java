...
public class PlanError
{
  ...
  private boolean isSemanticError(int node, PlanContext plan)
  {
    ...
    {
      if (isSemanticError(plan0.childOf(node, i), plan))
        return true;
    }
  ...
  private boolean isErrorAgg(int node, PlanContext plan) {
    ...
    for(Expression group : groups)
        {
          if (group.template().$(GroupItem_Expr).$(ExprFields.Literal_Value) != null
              ...
              && (Integer) group.template().$(GroupItem_Expr).$(ExprFields.Literal_Value)
                  > exprs.size())
            return true;
        }
        return false;
    ...
    private boolean isErrorFilter(int node, PlanContext plan)
    {
      return false;
    }

    private boolean isErrorLimit(int node, PlanContext plan)
    {
      return false;
    }

    private boolean isErrorSort(int node, PlanContext plan)
    {
      return false;
    }

    private boolean isErrorSetOp(int node, PlanContext plan)
    {
      return false;
    }
  }
