...
public interface PgAstHelper
{
  ...
  static String[] stringifyIdentifier(PGParser.Schema_qualified_name_nontypeContext ctx)
  {
    ...
    if (ctx.schema != null)
      pair[0] = stringifyIdentifier(ctx.schema);
  ...
  static String stringifyText(PGParser.Character_stringContext ctx)
  {
    ...
    if (ctx.Text_between_Dollar() != null && !ctx.Text_between_Dollar().isEmpty())
      return String.join("",
          ListSupport.<TerminalNode, String>map((Iterable<TerminalNode>) ctx.Text_between_Dollar(),
              (Function<? super TerminalNode, ? extends String>) TerminalNode::getText));
    else if (ctx.Character_String_Literal() != null)
  ...
  static SqlDataType parseDataType(PGParser.Data_typeContext ctx) {
    ...
    final String typeString =
        predefinedCtx.type != null
            ...
            :
        stringifyIdentifier(predefinedCtx.schema_qualified_name_nontype().identifier_nontype())
            .toLowerCase();
        ...
        else if (typeString.contains("bit"))
        {
          ...
          name = (predefinedCtx.VARYING() != null || typeString.contains("var"))
        ...
        else if (typeString.startsWith("int") && !typeString.equals("interval"))
        {
          ...
          switch (typeString)
          {
            case "int2": name = DataTypeName.SMALLINT; break;
            case "int4": name = DataTypeName.INTEGER; break;
            case "int8": name = DataTypeName.BIGINT; break;
        ...
        else if (typeString.startsWith("serial"))
        {
          ...
          switch (typeString)
          {
            case "serial2": name = DataTypeName.SMALLSERIAL; break;
            case "serial4": name = DataTypeName.SERIAL; break;
            case "serial8": name = DataTypeName.BIGSERIAL; break;
            default: return assertFalse();
          }
        }
        ...
        else if (typeString.startsWith("float"))
        {
          ...
          switch (typeString)
          {
            case "float4": name = DataTypeName.FLOAT; break;
            case "float8": name = DataTypeName.DOUBLE; break;
            default: return assertFalse();
          }
        }
        ...
        {
          category = Category.STRING;
          name = (typeString.contains("var") || predefinedCtx.VARYING() != null)
        ...
        for (int i = 0; i < arrayDimsCtx.size(); i++)
        {
          ...
          arrayDims[i] = arrayDimCtx.NUMBER_LITERAL() == null
              ? 0
    ...
    static Object parseUnsignedValue(PGParser.Unsigned_value_specificationContext ctx)
    {
      ...
      else if (ctx.character_string() != null)
        return stringifyText(ctx.character_string());
      else if (ctx.truth_value() != null)
        return parseTruthValue(ctx.truth_value());
      else
        return assertFalse();
    }
    ...
    static JoinKind parseJoinKind(PGParser.From_itemContext ctx)
    {
      ...
      {
        if (ctx.LEFT() != null)
          return JoinKind.NATURAL_LEFT_JOIN;
        else if (ctx.RIGHT() != null)
          return JoinKind.NATURAL_RIGHT_JOIN;
        else
          return JoinKind.NATURAL_INNER_JOIN;
      }

      if (ctx.CROSS() != null)
        return JoinKind.CROSS_JOIN;
      if (ctx.INNER() != null)
        return JoinKind.INNER_JOIN;
      if (ctx.LEFT() != null)
        return JoinKind.LEFT_JOIN;
      if (ctx.RIGHT() != null)
        return JoinKind.RIGHT_JOIN;
      if (ctx.FULL() != null)
        return JoinKind.FULL_JOIN;
    ...
    Set<String> KNOWN_AGG_BASIC = Set.of("array_agg",
        "avg",
        "bit_and",
        "bit_or",
        "bool_and",
    ...
    Set<String> KNOWN_AGG_STATISTIC = Set.of("corr",
        "covar_pop",
        "covar_samp",
        "regr_avgx",
        "regr_count",
