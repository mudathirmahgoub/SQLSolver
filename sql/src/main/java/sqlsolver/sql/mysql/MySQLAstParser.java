...
public class MySQLAstParser implements AstParser
{
  ...
  public SqlNode parse(String str, Function<MySQLParser, ParserRuleContext> rule)
  {
    ...
    if (str.contains("OVER ("))
      lexer.setServerVersion(Integer.MAX_VALUE);
    ...
    if (str.contains("OVER ("))
      parser.setServerVersion(Integer.MAX_VALUE);
    ...
    // 注意，此处需要构造一个匿名类
    BiConsumer<MySQLLexer, MySQLParser> initializer =
        new RefreshableParserInitializer<MySQLLexer, MySQLParser>() {};
