...
public abstract class ListSupport
{
  ...
  public static <X, Y> List<Y> map(Iterable<? extends X> xs, Function<? super X, ? extends Y> func)
  {
    if (xs instanceof Collection)
      return map((Collection<X>) xs, func);
  ...
  public static <T> List<T> join(List<T> xs, List<T> ys)
  {
    if (xs.isEmpty())
      ...
      return new BinaryJoinedList<>(xs, ys);
  }
  ...
  public static <P0, P1, R> List<R> zipMap(Iterable<? extends P0> l0,
