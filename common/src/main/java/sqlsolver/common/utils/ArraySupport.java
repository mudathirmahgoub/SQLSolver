...
public interface ArraySupport
{
  ...
  static PartialOrder compareBools(boolean[] bs0, boolean[] bs1)
  {
    ...
    {
      if (bs0[i] && !bs1[i])
        if (cmp == PartialOrder.LESS_THAN)
          return PartialOrder.INCOMPARABLE;
        else
          cmp = PartialOrder.GREATER_THAN;
      else if (!bs0[i] && bs1[i])
        if (cmp == PartialOrder.GREATER_THAN)
          return PartialOrder.INCOMPARABLE;
        else
          cmp = PartialOrder.LESS_THAN;
    }
  ...
  static int linearFind(int[] arr, int target, int fromIndex)
  {
    if (arr == null)
      return -1;
    for (int i = fromIndex, bound = arr.length; i < bound; i++)
      if (arr[i] == target)
        return i;
    return -1;
  }
  ...
  {
    final T[] ret = (T[]) Array.newInstance(
