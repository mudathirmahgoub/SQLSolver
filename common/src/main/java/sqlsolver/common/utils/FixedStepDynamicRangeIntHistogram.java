...
class FixedStepDynamicRangeIntHistogram implements IntHistogram
{
  ...
  private FixedStepDynamicRangeIntHistogram(int step, int from, TIntList indices, TIntList numbers)
  {
    this.step = step;
    this.from = from;
  ...
  public int findCoveringRange(int fromInclusive, int toExclusive)
  {
    ...
    if (rangeIdxOfTo != rangeIdxOfFrom)
      return -1;
    else
      return rangeIdxOfTo;
  }
  ...
  public String toString()
  {
    ...
    for (int i = 0, bound = indicesMap.size(); i < bound; ++i)
    {
      ...
      if (logicIndex != expectedLogicIdx)
        appendAbsentRange(expectedLogicIdx, logicIndex, builder);
  ...
  private void appendAbsentRange(int fromLogicIdx, int toLogicIdx, StringBuilder builder)
  {
    ...
    if (fromLogicIdx == 0)
      ...
      builder.append('[').append(beginOfRange(fromLogicIdx)).append(',');
