...
public class DbSchemaPatchDao extends DbDao implements SchemaPatchDao
{
  ...
  private static final String FIND_BY_APP =
      String.format("SELECT patch_app AS %s, patch_type AS %s, patch_table_name AS %s, "
                    + "patch_columns_name AS %s, patch_source AS %s, patch_reference AS %s "
  ...
  private static final String INSERT =
      "INSERT OR REPLACE INTO sqlsolver_schema_patches (patch_app, patch_type, patch_table_name, "
      + "patch_columns_name, patch_source, patch_reference) "
      + "VALUES (?, ?, ?, ?, ?, ?)";
