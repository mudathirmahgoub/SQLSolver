...
final public class VerificationImpl implements Verification
{
  ...
  private static VerificationResult getVerifyLiaStarResult(QueryPair pair)
  {
    try
    {
      return LogicSupport.proveEqByLIAStarConcrete(
          pair.getPlan0(), pair.getPlan1(), pair.getSchema());
    }
    catch (Throwable e)
    {
      if (LogicSupport.dumpLiaFormulas)
   ...
   */
  private static List<VerificationResult> getVerifyResult(
      List<QueryPair> pairs, String schemaString, long timeout)
  {
    ...
    {
      Printer.output.println("Verifying pair " + ++count);
      Cvc5LiaStarSolver.fileName = "query" + String.format("%03d", count);
      ...
      if (timeout > 0)
      {
        ...
        final Thread worker = new Thread(() -> {
          ...
          Printer.output.println("Thread here ");
        ...
        try
        {
          ...
          Printer.output.println("Verification time: " + timeVerify + " ms");
          final VerificationResult result = atomicResult.get();
          results.add(result);
          // if (result == VerificationResult.TIMEOUT)
          //   Printer.output.println("Timeout delay: " + (timeEnd - timeInterrupt.get()) + " ms");
        }
        catch (InterruptedException e)
        {
          // should not be interrupted
          Printer.output.println("Verification is interrupted");
          results.add(VerificationResult.UNKNOWN);
        }
        Printer.output.println(pair.pairId() + " " + results.get(count - 1));
      }
      else
      {
        final long timeStart = System.currentTimeMillis();
        ...
        Printer.output.println("Verification time: " + timeVerify + " ms");
        results.add(result);
        Printer.output.println(pair.pairId() + " " + result);
      }
    }

    /*Printer.output.println("===== Time stats (ms) =====");
    for (long time : times) {
      Printer.output.println(time);
    }*/
  ...
  static List<VerificationResult> verify(
      List<String> sqlList0, List<String> sqlList1, String schemaString, long timeout)
  {
    final Schema schema = CalciteSupport.getSchema(schemaString);
