...
public class LiaProveTest
{
  ...
  void testMultipleCases()
  {
    ...
    try
    {
      ...
      final Set<String> haveTested =
          Set.of("network_1", "student_transcripts_tracking", "real_estate_properties", "flight_2");
      for (final String fileName : fileNames)
      {
        if (haveTested.contains(fileName))
          continue;
        Printer.output.println("Handling case for: " + fileName);
    ...
    for (String appName : appNames)
          {
            ...
            sqlExecResults.put(appName + ".expect.txt",
                Files.readAllLines(expectPath)
                    .stream()
          ...
          for (String expectException : expectExceptions)
          {
            ...
            {
              expectExceptionCases.computeIfAbsent(appName, k -> new ArrayList<>())
                  .add(Integer.valueOf(parts[i]));
            }
          }
          ...
          {
            try (
                FileWriter fileWriter = new FileWriter(dataDir().resolve("unsound.txt").toString()))
            {
              for (String appName : appNames)
              {
                Printer.output.println(appName + " begin");
                if (targetApp != null && !targetApp.equals(appName))
                  continue;
                Path testCasesPath =
                    dataDir().resolve("spider/cases").resolve("rules." + appName + ".sql.txt");
                ...
                String schema = Files.readString(
                    dataDir().resolve("schemas").resolve(appName + ".base.schema.sql"));
                ...
                Printer.output.println(appName + "\n" + sqlsolverResult);
                Printer.output.println(sqlExecResults.get(appName + ".expect.txt"));
                for (int i = 0; i < sqlsolverResult.size(); i++)
                {
                  if (!sqlsolverResult.get(i).equals(
                          sqlExecResults.get(appName + ".expect.txt").get(i))
                      && sqlsolverResult.get(i) >= 0)
                  {
                    Printer.output.println("mismatch at " + (i + 1));
                    fileWriter.append(appName)
                        .append("\t")
                        .append(String.valueOf(i + 1))
                        .append("\t")
                        .append(sqlList0.get(i))
                        .append("\t")
                        .append(sqlList1.get(i))
                        .append("\t")
                        .append(String.valueOf(sqlExecResults.get(appName + ".expect.txt").get(i)))
                        .append("\t")
                        .append(String.valueOf(sqlsolverResult.get(i)))
                        .append("\n");
                  }
                  if (sqlsolverResult.get(i) < 0)
                  {
                    finalExceptionCases.computeIfAbsent(appName, k -> new ArrayList<>()).add(i + 1);
                    fileWriter1.append(appName)
                        .append("\t")
                        .append(String.valueOf(i + 1))
                        .append("\t")
                        .append(sqlList0.get(i))
      ...
      void testLiaOnRulesConcrete(
          Path testCasesPath, int targetId, int[] eqCases, Set<Integer> timeouts, String appName)
          throws IOException
      {
        testLiaOnRulesConcrete(testCasesPath, targetId, eqCases, timeouts, appName, -1);
      }

      void testLiaOnRulesConcrete(Path testCasesPath,
          int targetId,
          int[] eqCases,
          Set<Integer> timeouts,
          String appName,
          long timeout) throws IOException
      {
        ...
        List<String> sqlList1 = new ArrayList<>();
        String schema =
            Files.readString(dataDir().resolve("schemas").resolve(appName + ".base.schema.sql"));
        ...
        for (int t : timeouts)
        {
          results.set(t - 1, VerificationResult.TIMEOUT);
        }
        ...
        for (int eqCase : eqCases)
        {
          ...
          {
            Printer.output.println("warning: case " + eqCase + " becomes TIMEOUT!\n");
          }
          else if (results.get(eqCase - 1) != VerificationResult.EQ)
            Printer.output.println("warning: case " + eqCase + " becomes NEQ!\n");
        }
        ...
        Printer.output.println("Total EQs: " + totalEqs);
      }
      ...
      void testLiaOnCraftRules() throws IOException
      {
        ...
        {
          rule.toString();
          if (targetId > 0 && rule.id() != targetId)
            continue;
          if (targetId < 0 && startId > 0 && rule.id() < startId)
            continue;
          final UExprTranslationResult uExprs = UExprSupport.translateToUExpr(
              rule, UExprSupport.UEXPR_FLAG_INTEGRITY_CONSTRAINT_REWRITE);
          if (targetId > 0)
          {
            Printer.output.println("Rewritten UExpressions: ");
            Printer.output.println("[[q0]](" + uExprs.sourceOutVar() + ") := " + uExprs.sourceExpr());
            Printer.output.println("[[q1]](" + uExprs.targetOutVar() + ") := " + uExprs.targetExpr());
            LogicSupport.setDumpLiaFormulas(true);
          }
          final VerificationResult result = LogicSupport.proveEqByLIAStar(rule);
          Printer.output.println("Rule id: " + rule.id() + " is " + result.toString());
        }
      }
    }