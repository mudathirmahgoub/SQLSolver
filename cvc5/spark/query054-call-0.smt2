(set-logic HO_ALL)
(declare-const u5 Int)
(declare-const u6 Int)
(declare-const u7 Int)
(declare-const u8 Int)
(declare-const u9 Int)
(declare-const u10 Int)
(declare-const u0 Int)
(declare-const u1 Int)
(declare-const u2 Int)
(assert (and (not (= u0 u1)) (int.star-contains (lambda ((u5 Int) (u8 Int) (u6 Int) (u7 Int) (u9 Int) (u10 Int) (u2 Int) ) (and (= u5 (ite (= u2 u6) u7 0)) (= u8 (ite (= u2 u9) u10 0)))) u1 u0 u6 u7 u9 u10 u2 )))
(check-sat)
